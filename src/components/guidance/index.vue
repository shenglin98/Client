<template>
  <div class="guidance">
    <!-- 分诊弹框 -->
    <div class="triage_ejectbox" v-if="showGuidance">
      <!-- 标题与隐藏 -->
      <div class="title_btn">
        <div class="tb_title">排队详情</div>
        <div class="tb_btn" @click="handleHide">
      ...
      <div class="top_btn1">
        <div class="sex_box" style="display: flex; align-items: center;">
          ...
          <el-select style="width: 100px;" v-model="isShowSex" class="isShowSex" @change="handleTriagSex" size="mini" placeholder="请选择">
      ...
      <div class="eject_table_box">
        ...
        <div class="ejecttable_box">
          <vxe-table ref="ejecttable" border="default" stripe class="ejecttable" round height="395px" :loading="ejectLoading" highlight-hover-row :data="ejectStatus == '待检' ? queuePaiDuiList : ejectStatus == '已检' ? completeList : guohaoList" :checkbox-config="{ trigger: 'row' }" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName">
            ...
            <vxe-table-column field="name" title="姓名" show-header-overflow show-overflow="title" width="90px" show-footer-overflow>
              <template slot-scope="scope">
                <span v-if="scope.row.overflag > 1" style="color: red;">{{ scope.row.name }}</span>
    ...
    <!-- 点击配置科室弹框 -->
    <el-dialog title="" class="tedepart_dia" :modal="false" :visible.sync="showDepart" width="50%" :show-close="false" :close-on-click-modal="false">
    ...
    <!-- 点击完成弹出提示弹框 - 分诊 -->
    <el-dialog title="" class="tefinish_dia" :modal="false" :visible.sync="showFinish" width="34%" :show-close="false" :close-on-click-modal="false">
      <span>
...
export default {
  ...
  methods: {
    ...
    handleHide() {
      ...
      this.$emit("hide", this.queuePaiDuiList.length);
    },
    handleTriagChangeType(status) {
      ...
      ChangeRoomStopStatus({
        ...
        this.$emit("update", this.queuePaiDuiList.length);
      });
    },
    handleTriagSex(sex) {
      if (!this.triagData.roomcode) {
      ...
      that.$global.ws.onmessage = function (msg) {
        ...
        if (msg.data != "连接成功" && JSON.parse(msg.data)) {
          ...
          if (data.isLoginOut) {
            ...
            that.$emit("update", that.queuePaiDuiList.length);
          }
        }
      };
    },
    ...
    handleTriagDetailNew() {
      TriagDetail().then((res) => {
        ...
        this.isShowSex = res.result.roomsex;
    ...
    socketObj: {
      handler(newVal, oldVal) {
        ...
        if (data.isLoginOut) {
          ...
          that.$emit("update", data.RoomPaiDuiList.length);
        }
      },
      deep: true,
    },
