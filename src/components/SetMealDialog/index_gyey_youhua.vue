<template>
  <div class="setmeal_dialog">
    <!-- 主体套餐弹框 -->
    <el-dialog class="page_dialog" :title="diatitle" :visible.sync="dialogVisible" width="90%" :close-on-click-modal="false" :close-on-press-escape="false" @close="handleCloseSetMeal">
      <div class="content">
        <div class="content_top">
          <div class="search_box">
            <el-input ref="searchInput" size="mini" placeholder="请输入套餐或组合名称" v-model="searchName" clearable>
            </el-input>
          ...
          <div class="btn_template">
            <el-button size="mini" plain type="primary" style="margin-right:30px" :loading="loadingBtn" @click="handleSubmitSetMeal">保 存</el-button>
            <el-button size="mini" plain type="primary" @click="handleDeptSeqnoOrdinamento">组合集中</el-button>
            <el-button size="mini" plain type="primary" @click="handleSaveTemplate">保存模板</el-button>
            <el-button size="mini" plain type="primary" @click="handleResetPage">刷新</el-button>
            <el-button size="mini" plain type="primary" @click="handleEmptyChooseData">清空</el-button>
            <el-button size="mini" plain type="primary" @click="handleCloseSetMeal">关 闭</el-button>
          </div>
        </div>
        <div class="table_box">
          <!-- 套餐表格 -->
          ...
          <div class="combination_table widthClas" style="height: 100%;">
            <vxe-table ref="setmealitems" height="100%" border="inner" :loading="loading" highlight-hover-row :data="setmealitemsData" :checkbox-config="{ reserve: true, trigger: 'row' }" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: true, gt: 0 }" @checkbox-change="handleSelectSetMeal" row-id="setmealcode">
              <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
              <vxe-table-column field="setmealname" show-overflow="title" width="200" title="套餐名称"></vxe-table-column>
              <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
            </vxe-table>
          </div>
          <div class="combination_table widthClas" style="height: 100%;">
            <vxe-table ref="combinationOne" v-if="showTableFlag" height="100%" border="inner" :loading="loading" highlight-hover-row :data="combinationOneData" :checkbox-config="{ reserve: true, trigger: 'row' }" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: true, gt: 0 }" row-id="combinecode" @checkbox-change="handleSelectcomOne">
              <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
              <vxe-table-column field="combinename" show-overflow="title" title="组合名称" width="200"></vxe-table-column>
              <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
            </vxe-table>
          </div>
          <div class="combination_table widthClas" style="height: 100%;">
            <vxe-table ref="combinationTwo" height="100%" border="inner" :loading="loading" highlight-hover-row :data="combinationTwoData" :checkbox-config="{ reserve: true, trigger: 'row' }" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: true, gt: 0 }" row-id="combinecode" @checkbox-change="handleSelectcomTwo">
              <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
              <vxe-table-column field="combinename" title="组合名称" width="200" show-overflow="title"></vxe-table-column>
              <vxe-table-column field="price" title="价格" width="80px">
              </vxe-table-column>
              <vxe-table-column title="查看明细" width="100">
                <template v-slot="params">
                  <el-link type="primary" @click.stop="handlePopoverShow(params.row)">组合明细</el-link>
                </template>
                <!-- <template v-slot="params">
                <el-popover
                  placement="left"
          ...
          <div class="combination_table widthClas" style="height: 100%;">
            <vxe-table ref="combinationThree" height="100%" border="inner" :loading="loading" highlight-hover-row :data="combinationThreeData" :checkbox-config="{ reserve: true, trigger: 'row' }" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: true, gt: 0 }" row-id="combinecode" @checkbox-change="handleSelectcomThree">
              <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
              <vxe-table-column field="combinename" show-overflow="title" title="组合名称" width="200"></vxe-table-column>
              <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
            </vxe-table>
    ...
    <!-- 保存模板弹框 -->
    <el-dialog class="temp_dialog" title="套餐模板维护" :visible.sync="showTemplate" width="45%" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <div class="content">
    ...
    <!-- 查看明细 -->
    <el-dialog class="temp_dialog" title="查看明细" :visible.sync="popverFlag" width="45%" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <el-table :data="combinationDetailData" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: false }" v-loading="popverLoading" height="300px">
        <el-table-column width="100" property="itemcode" label="项目编号"></el-table-column>
        <el-table-column property="itemname" width="500" label="项目名称"></el-table-column>
        <el-table-column property="deptname" label="科室"></el-table-column>
      </el-table>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" plain type="primary" @click="handlePopoverHide">关 闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handlePopoverShow(row) {
      this.popverFlag = true;
      getItemList([row.combinecode]).then((res) => {
        res.code == 200 && (this.combinationDetailData = res.result);
      });
    },
    // 隐藏回调
    handlePopoverHide(row) {
      this.combinationDetailData = [];
      this.popverFlag = false; // 控制弹框
    },
    handleOpenSetMeal() {
    ...
    handleSubmitSetMeal() {
      ...
      if (this.selectSetMeal.length === 1) {
        ...
        chooseObj && (setmealcode = chooseObj.setmealcode || "");
      } else if (this.groupTableData.length > 0) {
        for (let i = 0; i < this.groupTableData.length; i++) {
          if (this.groupTableData[i].setmealcode) {
            setmealcode = this.groupTableData[i].setmealcode;
          }
        }
      }
      let combinecodes = data.map((k) => k.combinecode);
      SureCombines({ setmealcode, combinecodes }).then((res) => {
        if (!res.result) {
          this.$message.warning("暂无数据!");
          return;
        }
        let dataarr = res.result.map((item) => {
          return {
            detid: "",
            regid: "",
            setmealcode:
              (chooseObj && chooseObj.setmealcode) || item.setmealcode || "",
            setmealname:
              (chooseObj && chooseObj.setmealname) || item.setmealname || "",
        ...
        this.$emit("submit", dataarr); // 发送数据父组件
      });
    },
    // 关闭套餐选择弹框回调
    handleCloseSetMeal() {
    ...
    handleSelectSetMeal({ records, reserves, row }) {
      ...
      if (this.selectSetMeal.length > 0) {
        let setmealcode = [];
        let setmealtype = [];
        for (let i = 0; i < this.selectSetMeal.length; i++) {
          setmealcode.push(this.selectSetMeal[i].setmealcode);
          setmealtype.push(this.selectSetMeal[i].setmealtype);
        }
        LoadSetmealCombineList({
          ...
          res.result.forEach((item) => {
            this.combinationOne.forEach((val) => {
              if (item.combinecode == val.combinecode) {
                // 手动添加到选中项
    ...
    handleGetDicSetmealList() {
      ...
      LoadSimpleSetmeals({
        ...
        if (!res.result) {
          this.$message.warning("暂无套餐数据!");
          return;
        }
        this.setmealitems = res.result.setmealitems;
      });
      LoadSimpleCombines({ sex: this.sex, keyword: "" })
        .then((res) => {
          this.loading = false;
          if (!res.result) {
          ...
          if (this.filterData && this.filterData.length > 0) {
            let tempArr = this.filterData.map((k) => k.combinecode);
            this.combinationOne = res.result[0].combineitems.filter((k) => {
              return !tempArr.includes(k.combinecode);
            });
            this.combinationTwo = res.result[1].combineitems.filter((k) => {
              return !tempArr.includes(k.combinecode);
            });
            this.combinationThree = res.result[2].combineitems.filter((k) => {
              return !tempArr.includes(k.combinecode);
            });
          } else {
            this.combinationOne = res.result[0].combineitems;
            this.combinationTwo = res.result[1].combineitems;
            this.combinationThree = res.result[2].combineitems;
          }
          this.$nextTick(() => {
