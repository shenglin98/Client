<template>
  <div class="setmeal_dialog">
    ...
    <el-dialog class="page_dialog" title="组合选择" :visible.sync="dialogVisible" width="50%" :close-on-click-modal="false" :close-on-press-escape="false" @close="handleCloseCombine">
      <div class="top_box">
        <el-input ref="searchInput" class="searchName" placeholder="请输入套餐名称" v-model="searchName" clearable @keydown.down.native="handleSearchKeyDown" @keydown.up.native="handleSearchKeyUp" @keydown.enter.native="handleSubmitCombine">
        </el-input>
        <div class="btn_group">
          <el-button type="primary" @click="handleSubmitCombine">确定</el-button>
      ...
      <div class="table_box">
        <vxe-table class="combine_table color_table" ref="combine" height="auto" border="inner" show-overflow :loading="loading" highlight-hover-row highlight-current-row :data="combineitemsData" :checkbox-config="{ reserve: true }" :sort-config="{ trigger: 'cell' }" :keyboard-config="{ isArrow: true }" @keydown="handleCombineTableKeyDown" @cell-dblclick="handleSubmitCombine" row-id="setmealcode">
