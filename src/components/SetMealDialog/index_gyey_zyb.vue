<template>
  <div class="setmeal_dialog">
    <!-- 主体套餐弹框 -->
    <el-dialog class="page_dialog" title="套餐组合选择" :visible.sync="dialogVisible" width="90%" :close-on-click-modal="false" :close-on-press-escape="false" @close="handleCloseSetMeal">
      <div class="content">
        <div class="content_top">
          ...
          <div class="btn_template">
            ...
            <el-button size="mini" plain type="primary" @click="handleSubmitSetMeal">保 存</el-button>
        ...
        <div class="table_box">
          ...
          <vxe-table class="setmeal_table" ref="setmealitems" height="auto" border="inner" :loading="loading" highlight-hover-row :data="setmealitemsData" :checkbox-config="{ reserve: true }" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: false }" @checkbox-change="handleSelectSetMeal" row-id="setmealcode">
            <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
            <vxe-table-column field="setmealname" width="200" title="套餐名称"></vxe-table-column>
            <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
          </vxe-table>
          <vxe-table class="combination_table" ref="combinationOne" v-if="showTableFlag" height="auto" border="inner" :loading="loading" highlight-hover-row :data="combinationOneData" :checkbox-config="{ reserve: true }" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: false }" row-id="combinecode" @checkbox-change="handleSelectcomOne">
            <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
            <vxe-table-column field="combinename" title="组合名称" width="200"></vxe-table-column>
            <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
          </vxe-table>
          <vxe-table class="combination_table" ref="combinationTwo" height="auto" border="inner" :loading="loading" highlight-hover-row :data="combinationTwoData" :checkbox-config="{ reserve: true }" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: false }" row-id="combinecode" @checkbox-change="handleSelectcomTwo">
            <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
            <vxe-table-column field="combinename" title="组合名称" width="200"></vxe-table-column>
            <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
          </vxe-table>
          <vxe-table class="combination_table" ref="combinationThree" height="auto" border="inner" :loading="loading" highlight-hover-row :data="combinationThreeData" :checkbox-config="{ reserve: true }" :row-class-name="handleReturnIsChecked" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: false }" row-id="combinecode" @checkbox-change="handleSelectcomThree">
            <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
            <vxe-table-column field="combinename" title="组合名称" width="200"></vxe-table-column>
            <vxe-table-column field="price" title="价格" width="80px"></vxe-table-column>
          </vxe-table>
...
export default {
  ...
  methods: {
    ...
    handleGetDicSetmealList() {
      ...
      // console.log(setmealAndCombine.setmealitems, "setmealAndCombine 查看类型");
      if (this.sex == 0) {
        this.setmealitems = [];
         setmealAndCombine.setmealitems.forEach((u) => {
          if (u.checkType == "2") {
            this.setmealitems.push(u) ;
          }
        }); // 返回职业病类型数据
        // console.log(this.setmealitems, "处理过的");
    ...
    handleGetDicSetmealListbeiyon() {
      ...
      getDicSetmealList({ tp: "", sex: this.sex }).then((res) => {
        // console.log(res.result.setmealitems, "查看checkType");
        this.loading = false;
        let arr1 = [];
        res.result.setmealitems.forEach((u) => {
          if (u.checkType == "2") {
            arr1.push(u);
          }
        });
        this.setmealitems = arr1;
        ...
        res.result.setmealitems = arr2;
        let setmealAndCombine = res.result;
        console.log(setmealAndCombine);
