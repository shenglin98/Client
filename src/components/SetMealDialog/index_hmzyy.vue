    ...
    >
      <div class="content">
        ...
        <div class="table_box">
          ...
          <vxe-table
            class="setmeal_table"
            :tree-config="{ children: 'children', showIcon: true }"
            ref="setmealitems"
            resizable
            height="auto"
            border="inner"
...
export default {
  ...
  computed: {
    ...
    // 套餐表格搜索数据
    setmealitemsData() {
      const search = this.$XEUtils.toValueString(this.searchName).trim();
      if (search) {
        let options = { children: "children" };
        let searchProps = ["setname", "setmealname", "setmealpx", "setmealwb"];
        let temp = this.$XEUtils.searchTree(
          this.setmealitems,
          (item) =>
            searchProps.some(
              (key) =>
                this.$XEUtils.toValueString(item[key]).indexOf(search) > -1
            ),
          options
        );
        // 搜索之后默认展开所有子节点
        this.$nextTick(() => {
          this.$refs.setmealitems.setAllTreeExpand(true);
        });
        return temp;
  ...
  methods: {
    ...
    handleSubmitSetMeal() {
      ...
      data = data.map((item) => {
        return {
          ...
          setmealcode:
            (chooseObj &&
              chooseObj.setmealtype != "Person" &&
              chooseObj.setmealcode) ||
            "",
          setmealname:
            (chooseObj &&
              chooseObj.setmealtype != "Person" &&
              chooseObj.setmealname) ||
            "",
    ...
    handleGetDicSetmealList() {
      ...
      if (
        ...
        this.groupTableData[0].setmealcode
      ) {
        this.checkRowKeys.push(this.groupTableData[0].setmealcode);
        // 搜索之后默认展开所有子节点
        this.$nextTick(() => {
          this.$refs.setmealitems.setAllTreeExpand(true);
        });
      ...
      if (this.sex == 0) {
        ...
        let tempMeali = this.setmealitems.map((k) => {
          return {
            settype: k.settype,
            setname: k.setname,
            setmealname: k.setname,
            children: [],
          };
        });
        let map = new Map();
        for (let item of tempMeali) {
          map.set(item.setname, item);
        }
        tempMeali = [...map.values()];
        for (let i = 0; i < tempMeali.length; i++) {
          tempMeali[i].children = this.setmealitems.filter((k) => {
            return tempMeali[i].setname == k.setname;
          });
          if (!tempMeali[i].setname) {
            tempMeali[i].setmealname = "未分组";
          }
        }
        this.setmealitems = tempMeali;
