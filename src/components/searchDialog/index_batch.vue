    ...
    <el-dialog class="search_dialog" :close-on-click-modal="false" :visible.sync="showDialog" width="85%" top="2%" :show-close="false">
      ...
      <div class="search_form">
        ...
        <el-form ref="form" class="form_box" :model="searchForm" label-width="80px">
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item class="select" size="mini" label="审核医生">
                <el-select class="check_select" style="width: 100%" v-model="searchForm.checkdoctorcode" clearable @change="handleSearch">
                  <el-option v-for="(item, index) in checkdoctorItems" :key="index" :label="item.name" :value="item.account"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <div class="tableBtnBoxClas">
        <el-button-group>
          <el-button size="mini" type="primary" plain @click="(searchForm.orderstatus = ''), handleSearch()">全部</el-button>
          <el-button size="mini" type="primary" plain @click="(searchForm.orderstatus = '0'), handleSearch()">未总检</el-button>
          <el-button size="mini" type="primary" plain @click="(searchForm.orderstatus = '1'), handleSearch()">已总检</el-button>
          <el-button size="mini" type="primary" plain @click="(searchForm.orderstatus = '2'), handleSearch()">已审核</el-button>
        </el-button-group>
        <div class="text">
          <div>未总检：{{ unhandletotal }}</div>
          <div>已总检：{{ handletotal }}</div>
          <div>已审核：{{ completetotal }}</div>
        </div>
        <div>
          <el-button size="mini" type="primary" plain @click="handleBatchSave">批量总检</el-button>
          <el-button size="mini" type="primary" plain @click="handleBatchCheck">批量审核</el-button>
        </div>
      </div>
      <!-- 主体表格数据 -->
...
export default {
  ...
  async mounted() {
    ...
    this.checkdoctorItems = JSON.parse(JSON.stringify(res.data));
  ...
  methods: {
    ...
    handleBatchCheck() {
      ...
      BatchCheck(this.chooseList.map((k) => k.regid)).then((res) => {
        this.$message.success(res.message);
        this.handleSearch();
      });
    },
