<template>
  <el-dialog class="search_dialog" :close-on-click-modal="false" :visible.sync="showDialog" width="60%" top="5%" :show-close="false">
    <!-- 头部按钮组 -->
    <div class="btn_group">
      ...
      <el-button type="primary" v-if="pageFlag == 'totalcheck'" plain size="mini" @click="handleAllReturn">全部返回</el-button>
    ...
    <div class="search_form">
      ...
      <el-form ref="form" class="form_box" :model="searchForm" label-width="80px">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item class="check_select" size="mini" label="状态">
              <el-select clearable size="mini" @change="handleSearch" v-model="searchForm.status" placeholder="">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item class="check_select" size="mini" label="团检批次">
              <el-select @change="handleSelectBatch" style="width: 100%" v-model="searchForm.batch" placeholder="" clearable>
          ...
          <el-col :span="8">
            <el-form-item class="check_select" size="mini" label="团检分组">
              <el-select @change="handleCheckGroup" style="width: 100%" v-model="searchForm.groupcode" placeholder="" clearable>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item class="check_select" size="mini" label="检查医生">
              <el-select v-model="searchForm.doctorcode" @change="handleSearch" clearable>
                <el-option v-for="item in operatorItems" :key="item.id" :label="`${item.account}　${item.name}`" :value="item.account"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <!-- 主体表格数据 -->
    <EasyTable class="page_table" height="100%" :data="pageData" :tableOptions="pageOptions" :radio-config="{ rowClick: true }" @row-dblclick="handleCheckPerson" @selection-change="handleSelectIntment" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="{ height: '32px' }" :cell-style="{ padding: '0' }" />
...
export default {
  ...
  async mounted() {
    ...
    usernameOrId &&
      getAllUser({ meccode: usernameOrId.meccode }).then((res) => {
