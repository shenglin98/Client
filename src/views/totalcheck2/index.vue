              ...
              <el-row>
                ...
                <el-col :span="4">
                  <el-form-item size="mini" label="体检日期">
                    <el-input v-model="currentPeople.registercheckdate"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
          ...
          <div class="body_box">
            ...
            <div class="check_form">
              <el-form ref="form" :disabled="currentPeople.updatestatus == 0" :model="currentPeople" label-width="80px">
                ...
                <el-form-item size="mini" label="二审医生">
                  <el-select v-model="currentPeople.checkcode2" clearable disabled @change="handleChangeCheckCode">
                    <el-option v-for="(item, index) in currentPeople.doctoritems" :key="index" :label="item.name" :value="item.code"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item size="mini" label="二审时间">
                  <el-date-picker v-model="currentPeople.checkdate2" value-format="yyyy-MM-dd hh:mm:ss" type="datetime">
                  </el-date-picker>
                </el-form-item>
              </el-form>
            </div>
...
import SearchDialogBacth from "@/components/searchDialog/index_batch2.vue";
...
export default {
  ...
  methods: {
    handleIsLastProcess() {
      LastProcess({ regid: this.currentPeople.regid, pt: "PrintReport" }).then(
        (res) => {
          ...
          if (!res.result) {
            BatchCheck2Cancel({
              regiditems: [this.currentPeople.regid],
              reason: "未打印",
            }).then((res) => {
    ...
    // 获取总检结果回调
    handleGetTotalCheck() {
      TotalCheck2Detail({ regid: this.checkupWork.trim() }).then((res) => {
        let data = res.result;
