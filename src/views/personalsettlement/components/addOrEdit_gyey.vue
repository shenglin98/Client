    ...
    <div class="head_box">
      <div class="title_box">
        ...
        <div class="btn_group">
          ...
          <el-button type="primary" size="mini" plain @click="handleUnSettlementCustomers">查询</el-button>
          <!-- :disabled="pageFlag == 'edit'"  结算-->
      ...
      <el-form ref="form" class="search_form" :model="searchForm" label-width="80px">
        ...
        <el-form-item size="mini" class="date_box">
      ...
      <!-- 主体数据表格 -->
      <el-row class="page_table_box">
        <!-- 人员列表 -->
        <el-col :span="8" class="page_table_box_l">
        ...
        <!-- 项目列表 -->
        <el-col :span="16" class="page_table_box_r">
      ...
      <el-dialog class="dialog-mini discount_dialog" width="50%" top="20%" :close-on-click-modal="false" title="单位名称" :visible.sync="showUnitDia" @close="handleCloseUnitDia">
        <div class="discount_box">
          <el-input style="width: 80%" placeholder="请输入单位名称" max="100" min="1" v-model="unitname" clearable>
          </el-input>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      departitems: [],
      departitem2s: [],
      checkupDateArr: ["登记时间", "体检时间", "总检时间"], // 检索条件身份证等数组
      conditionDateText: "登记时间",
      conditionDate: [],
      pickerOptions: {
        shortcuts: [
          {
  ...
  methods: {
    ...
    handleChangeDate(val) {
      ...
      this.handleUnSettlementCustomers();
    },
    // 点击选择检索体检条件回调
    handleSelectConditionDate(val) {
      if (val == "登记时间") {
        this.searchForm.timetype = "regdate";
      } else if (val == "体检时间") {
        this.searchForm.timetype = "checkdate";
      } else {
        this.searchForm.timetype = "totalcheckdate";
      }
    ...
    handleSelectStaff(arr) {
      ...
      this.selectstaffData = arr.map((k) => k.regid);
      CalcPayMoneyForChecked(arr.map((k) => k.regid)).then((res) => {
