    ...
    <div class="main_cs">
        ...
        <div class="main_right_menu">
          ...
          <el-row v-if="switchhazards" class="tablehazards">
            <el-col :span="11" class="tableClas">
              <!-- 职业危害因素表格 -->
              <el-table :data="hazards" class="HazardsClas" height="calc(100vh - 152px)" style="border-radius: 5px; width:50%;" @row-click='hazardscellClick' @selection-change="handleSelectChange" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                ...
                <el-table-column label="关联">
                  <template slot-scope="param">
                    <el-button type="primary" size="mini" @click="RelevancyFlagFun(param.row)">关联</el-button>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="typesOfGuardianship" label="监护种类">
                </el-table-column>
            ...
            <el-dialog title="关联危害" class="relevancyClass" :close-on-click-modal="false" style="margin-top: 5vh; height: 100%;" :visible.sync="RelevancyVisible" width="80%" :before-close="handleRelevancyClose">
              <el-row>
                <el-col :span="11" style="padding-right:10px;">
                  <!-- 搜索框 1-->
                  <div class="filter-container" style="white-space: nowrap; overflow-x: auto">
                    <el-input @keyup.enter.native="handlesetmealSearch" size="mini" style="width: 200px" class="filter-item" :placeholder="'名称'" v-model="setmealSearch">
                    </el-input>
                    <el-button class="filter-item" size="mini" icon="el-icon-search" @click="handlesetmealSearch">搜索</el-button>
                  </div>
                  <div>套餐: </div>
                  <el-table height="340px" :row-key="rowKey1" ref="setmealRef" @row-dblclick='parthandleSelectionChange' :data="setmeal" v-loading="!setmeal" show-overflow-tooltip fit highlight-current-row style="width: 100%" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                    <!-- <el-table-column type="selection" align="center" width="55"></el-table-column> -->
                    <el-table-column prop="setmealcode" label="套餐编码" placeholder="系统自动生成" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column width="200" prop="setmealname" label="套餐名称" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column prop="price" label="套餐价格" show-overflow-tooltip>
                    </el-table-column>
                  </el-table>
                </el-col>
                <el-col :span="11">
                  <div>已选择套餐: <el-button type="primary" size="mini" @click="DelSetmeaBtnFun">删除</el-button></div>
                  <el-table height="340px" :row-key="rowKey2" :data="setmealItemData" show-overflow-tooltip :default-sort="{ prop: 'seqno', order: 'ascending' }" fit highlight-current-row style="width: 100%" @selection-change="chilhandleSelectionChange" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                    <el-table-column type="selection" align="center" width="55"></el-table-column>
                    <el-table-column prop="setmealcode" label="套餐编码" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column width="200" prop="setmealname" label="套餐名称" show-overflow-tooltip>
                    </el-table-column>
                  </el-table>
              ...
              <el-row>
                <el-col :span="11" style="padding-right:10px;">
                  <!-- 搜索框 2-->
                  <div class="filter-container" style="white-space: nowrap; overflow-x: auto">
                    <el-input @keyup.enter.native="handlecombineSearch" size="mini" style="width: 200px" class="filter-item" placeholder="名称" v-model="combineSearch">
                    </el-input>
                    <el-button class="filter-item" size="mini" icon="el-icon-search" @click="handlecombineSearch">搜索</el-button>
                  </div>
                  <div>组合: </div>
                  <el-table :data="combine" :row-key="rowKey3" ref="combineRef" @row-dblclick="combinehandleSelectionChange" v-loading="!combine" height="340px" fit highlight-current-row style="width: 100%" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                    <!-- <el-table-column type="selection" align="center" width="55"></el-table-column> -->
                ...
                <el-col :span="11">
                  <div>已选择组合: <el-button type="primary" size="mini" @click="DelcombineBtnFun">删除</el-button></div>
                  <el-table :data="combineItemData" :row-key="rowKey3" height="340px" ref="chilmainTable" :default-sort="{ prop: 'seqno', order: 'ascending' }" fit highlight-current-row style="width: 100%" @selection-change="CombinerowClick" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                    <el-table-column type="selection" align="center" width="55"></el-table-column>
                    <el-table-column prop="combinecode" label="组合编码" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column width="200" prop="combinename" label="组合名称" show-overflow-tooltip>
                    </el-table-column>
                  </el-table>
                </el-col>
              </el-row>
            ...
            <el-col :span='12'>
              <el-row class="table_Packag">
                <el-table :data="setmealItemData" height="calc(50vh - 90px)" style="border-radius: 5px;" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                  <el-table-column prop='setmealcode' label="编码">
                  </el-table-column>
                  <el-table-column prop='setmealname' label="套餐名称">
                  </el-table-column>
                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-button type="primary" size="mini" @click="DelSetmeaBtnFun(scope.row)">删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              </el-row>
              <el-row class="table_Combination">
                <el-table :data="combineItemData" height="calc(50vh - 90px)" style="border-radius: 5px;" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                  <el-table-column prop='combinecode' label="编码">
                  </el-table-column>
                  <el-table-column prop='combinename' label="组合名称">
                  </el-table-column>
                  <!-- <el-table-column prop='code' label="组合价格">
                  </el-table-column> -->
                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-button type="primary" size="mini" @click="DelcombineBtnFun(scope.row)">删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              </el-row>
          ...
          <el-table :data="evaluate" height="calc(100vh - 152px)" v-if="switchevaluate" @selection-change="handleSelectChange" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
            ...
            <el-table-column prop="reviewflag" label="复查标志">
              <template slot-scope="scope">
                <span>
                  {{ scope.row.reviewflag == 0 ? "不需要" : scope.row.reviewflag == 1 ? "需复查" : "" }}
                </span>
              </template>
            </el-table-column>
          ...
          <el-table :data="Industry" height="calc(100vh - 152px)" v-if="switchIndustry" @selection-change="handleSelectChange" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
            ...
            <el-table-column prop="name" label="行业名称">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="industryRelevancyFun(scope.row)">关联</el-button>
              </template>
            </el-table-column>
            <el-table-column prop="pycode" label="拼音码"></el-table-column>
            <el-table-column prop="wbcode" label="五笔码"></el-table-column>
          ...
          <el-dialog title="行业关联工种" label-position="right" top="4vh" :visible.sync="industryRelevancyDialogFlag" width="70%">
            <div class="industryDialogClas">
              <div class="left">
                <p>
                  未选择:
                </p>
                <el-table :data="industryNotSelectedData" height="calc(100vh - 250px)" @row-dblclick="handlIndustryRowDblclick" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
                  <el-table-column prop="code" label="工种编码"></el-table-column>
              ...
              <div class="right">
                <div class="rightBtnClas">
                  <p>
                    已选择:
                  </p>
                  <div>
                    <el-button type="primary" size="mini" @click="handelDeletIndustrySelect">删除</el-button>
                  </div>
                </div>
                <el-table :data="industrySelectedData" height="calc(100vh - 250px)" :border="false" @selection-change="handleIndustrySelect" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
          ...
          <el-table :data="Jobs" height="calc(100vh - 152px)" v-if="switchJobs" @selection-change="handleSelectChange" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
            <el-table-column type="selection" width="50"></el-table-column>
            <el-table-column prop="code" label="工种编码">
              <template slot-scope="scope">
      ...
      <el-dialog title="添加" label-position="right" :visible.sync="dialogFormVisible1" width="30%">
        <el-form :model="addhazards" ref="addhazards">
          ...
          <el-form-item label="监护种类" prop="typesOfGuardianship" :label-width="formLabelWidth" size="small">
            <el-select v-model="addhazards.typesOfGuardianship" @change="$forceUpdate()">
      ...
      <el-dialog title="编辑" label-position="right" :visible.sync="editFormVisible1" width="30%">
        <el-form :model="editcodeset">
          ...
          <el-form-item label="监护种类" prop="typesOfGuardianship" :label-width="formLabelWidth" size="small">
            <el-select v-model="editcodeset.typesOfGuardianship">
              <el-option :label="'上岗前'" :value="'上岗前'"></el-option>
              <el-option :label="'在岗期间'" :value="'在岗期间'"></el-option>
              <el-option :label="'离岗时'" :value="'离岗时'"></el-option>
              <el-option :label="'应急检查'" :value="'应急检查'"></el-option>
...
export default {
  ...
  data() {
    return {
      ...
      rowKey3: "id3",
      rowKey4: "id4",
      selectedRows1: [], // 默认勾选项 套餐
      selectedRows2: [], // 默认勾选项 组合
      harmcode: "", // 危害code
      ...
      // 组合
      combineItemData: [],
      setmealItemDataSelect: [], // 套餐
      combineItemDataSelect: [], // 组合
      // dialog 危害因素对应 体检套餐
      setmeal: [],
      setmealSearch: "", // 搜索
      // dialog 危害因素对应 体检组合
      ...
      // 行业关联--------------------
      industryRelevancyDialogFlag: false, // 行业关联的dialog开关
      industryNotSelectedData: [], //未选择数据
      industrySelectedData: [], // 已选择数据
      linkedSelectData: [], // 已勾选的数据 --- 待删除的
      occupationCode: "", // 保存当前点击数据的code
    };
  },
  computed: {
  ...
  mounted() {
    ...
    // 获取组合/套餐数据
    GetSetmealLoad().then((res) => {
      console.log(res, "获取套餐");
      if (res.code == 200) {
        this.setmeal = res.result;
      }
    });
    GetDicCombineLoad({ tp: "", sex: 1 }).then((res) => {
      // console.log(res, "获取组合");
      if (res.code == 200) {
        this.combine = res.data;
      }
    });
  },
  ...
  methods: {
    // 行业关联-----------------------------------
    // 已关联表格 删除回调
    handelDeletIndustrySelect() {
      if (this.linkedSelectData.length == 0) {
        this.$message.error("最少选择一条");
        return;
      }
      this.$confirm("是否确认删除?", "提示", {
        ...
        .then(() => {
          const result = this.linkedSelectData.map((item) => item.id);
          DelRelevance(result).then((res) => {
            console.log(res);
            if (res.code == 200) {
              this.$message.success("删除成功");
              // 获取行业对应的工种
              this.LoadIndustryRelevanceFun();
            } else {
    ...
    handleIndustrySelect(row) {
      this.linkedSelectData = row;
      console.log(row);
    },
    // 点击关联按钮回调
    industryRelevancyFun(row) {
      this.occupationCode = row.code; // 保存当前数据code
      getOccupationalDiseaseLoad({ type: "2" }).then((res) => {
        if (res.code == 200) {
          this.industryNotSelectedData = res.data;
        }
      });
      // 获取行业对应的工种
      this.LoadIndustryRelevanceFun();
    ...
    // 获取行业对应的工种
    LoadIndustryRelevanceFun() {
      // 获取行业对应的工种
      LoadIndustryRelevance({ code: this.occupationCode }).then((res) => {
        !!res.result && (this.industrySelectedData = res.result);
        this.industryRelevancyDialogFlag = true; // 打开dialog
      });
    },
    // 未选择的表格双击回调
    handlIndustryRowDblclick(row) {
      // 判断是否已经存在于数组中
      var exists = this.industrySelectedData.some((k) => k?.code === row.code);
      if (!exists) {
        this.industrySelectedData.push({
        ...
        this.$message.error("已存在该行业");
      }
    },
    // 职业危害因素关联----------------------------------
    // 套餐字典搜索回调
    handlesetmealSearch() {
      GetSetmealLoad({ searchInfo: this.setmealSearch?.trim() }).then((res) => {
        console.log(res, "获取套餐");
        if (res.code == 200) {
          this.setmeal = res.result;
        }
    ...
    handlecombineSearch() {
      GetDicCombineLoad({
        tp: "",
        sex: 1,
        combineName: this.combineSearch?.trim(),
      }).then((res) => {
        // console.log(res, "获取组合");
        if (res.code == 200) {
          this.combine = res.data;
    ...
    //  点击关联按钮回调
    RelevancyFlagFun(row) {
      !!this.$refs.setmealRef && this.$refs.setmealRef.clearSelection();
      !!this.$refs.combineRef && this.$refs.combineRef.clearSelection();
      this.RelevancyVisible = true;
      this.harmcode = row.code;
      this.getHarmCombineData(row.code);
    },
    // 点击套餐项 push到已选框中
    parthandleSelectionChange(item) {
      this.selectedRows1.push(item);
      let row = [];
      row.push(item);
      const List = row.map((item) => {
        return {
          harmcode: this.harmcode,
          checktype: "",
          setmealcode: item.setmealcode,
          setmealname: item.setmealname,
          seqno: "0",
        };
      });
      AddSetmealApi(List).then((res) => {
        if (res.code == 200) {
          this.getHarmCombineData(this.harmcode);
        }
      });
    },
    // 点击组合项 push到已选框中
    combinehandleSelectionChange(item) {
      console.log(item, "// 点击组合项 push到已选框中");
      this.selectedRows2.push(item);
      let row = [];
      row.push(item);
      const List = row.map((item) => {
        return {
          harmcode: this.harmcode,
          checktype: "",
          combinecode: item.code,
          combinename: item.name,
          seqno: "0",
        };
      });
      AddcombineApi(List).then((res) => {
        if (res.code == 200) {
          this.getHarmCombineData(this.harmcode);
        }
      });
    ...
    // 多选删除  套餐
    DelSetmeaBtnFun(item) {
      let row = undefined;
      if (!!item.setmealname && !!item.id) {
        // 如果外面按钮接受有值就走这里
        row = [item];
      } else {
        row = this.setmealItemDataSelect;
        if (this.setmealItemDataSelect.length <= 0) {
          return this.$message.error("请最少勾选一项");
        }
      }
      this.$confirm("此操作将永久删除该文件, 是否继续?")
        .then(() => {
          const List = row.map((k) => {
            return k.id;
          });
          List.length > 0 &&
            DelSetmealApi(List).then((res) => {
              res.code == 200 &&
                this.$message({
                  type: "success",
                  message: "删除成功!",
    ...
    // 组合
    DelcombineBtnFun(item) {
      let row = undefined;
      if (!!item.combinename && !!item.id) {
        // 如果外面按钮接受有值就走这里
        row = [item];
      } else {
        row = this.combineItemDataSelect;
        if (this.combineItemDataSelect.length <= 0) {
          return this.$message.error("请最少勾选一项");
        }
      }
      this.$confirm("此操作将永久删除该文件, 是否继续?")
    ...
    // 添加套餐 已选框勾选回调
    chilhandleSelectionChange(row) {
      this.setmealItemDataSelect = row;
    },
    // 添加组合 已选框勾选回调
    CombinerowClick(row) {
      this.combineItemDataSelect = row;
    },
    // 点击危害因素展示右侧数据
    hazardscellClick(row) {
      console.log(row);
      this.harmcode = row.code;
      this.getHarmCombineData(this.harmcode);
    },
    getHarmCombineData(code) {
      GetOccupationHarm({ harmcode: code }).then((res) => {
        if (res.code == 200) {
          this.setmealItemData = res.result.setmeal;
          this.combineItemData = res.result.combine;
        }
      });
    },
    // -------------------点击表格名字回调
