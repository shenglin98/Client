<template>
  <div class="full_search" v-loading="loadPage" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
    <!-- 头部操作控件 -->
      ...
      <el-row class="bg_white">
        ...
        <el-col class="form_box" :span="5">
          <div class="head">
            ...
            <el-switch style="margin-left: 30px" v-model="autoPrintReport" active-color="#13ce66" inactive-color="#909399" active-text="打印报告书">
            </el-switch>
          </div>
          <el-form ref="form" :model="searchForm">
            <el-form-item class="checkup_code" size="mini">
              ...
              <el-checkbox v-model="isSuperpose"></el-checkbox>
            </el-form-item>
            ...
            <el-form-item class="checkup_date" size="mini">
              <el-button plain type="primary" size="mini">检查时间</el-button>
              <el-date-picker @change="handleChangeCheckDate" @clear="handleSearchPageTable" size="mini" v-model="checkDate" type="daterange" align="right" unlink-panels range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd" :picker-options="pickerOptions">
              </el-date-picker>
            </el-form-item>
            <el-form-item class="checkup_date" size="mini">
              <el-button plain type="primary" size="mini">审核时间</el-button>
              <el-date-picker @change="handleChangeTotalcheckDate" @clear="handleSearchPageTable" size="mini" v-model="totalcheckDate" type="daterange" align="right" unlink-panels range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd" :picker-options="pickerOptions">
              </el-date-picker>
            </el-form-item>
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="dataSources" clearable @change="handleSearchPageTable">
            ...
            <el-form-item class="input" size="mini">
              ...
              <el-input v-model="searchForm.hiscode" clearable @clear="handleSearchPageTable" @keyup.native.enter="handleSearchPageTable"></el-input>
            </el-form-item>
            <el-form-item class="input" size="mini">
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="searchForm.deparment2" clearable filterable @change="handleSearchPageTable">
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="searchForm.reportprintflag" clearable @change="handleSearchPageTable">
                <el-option label="全部" :value="''"></el-option>
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="searchForm.statusin" clearable multiple @change="handleSearchPageTable">
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="reporttype" clearable @change="handleSearchReporttype">
                <el-option v-for="(item, index) in reporttypeitems" :key="index" :label="item.name" :value="item.code"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item class="select" size="mini">
              <div class="form_label">总检医生</div>
              <el-select v-model="searchForm.doctorcode" clearable filterable @change="handleSearchPageTable">
                <el-option v-for="item in operatorItems" :key="item.account" :label="item.name" :value="item.account"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item class="select" size="mini">
              <div class="form_label">审核医生</div>
              <el-select v-model="searchForm.checkcode" clearable filterable @change="handleSearchPageTable">
                <el-option v-for="item in operatorItems" :key="item.account" :label="item.name" :value="item.account"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item class="select" size="mini">
              <div class="form_label">客户级别</div>
              <el-select v-model="searchForm.level" clearable filterable @change="handleSearchPageTable">
        ...
        <el-col class="table_box" :span="19">
          ...
          <div class="page_table">
            <vxe-table ref="pageTable" height="auto" border="inner" :loading="loading" highlight-hover-row :data="pageData" :sort-config="{ trigger: 'cell' }" @checkbox-change="handleSelectPeople" @checkbox-all="handleSelectPeopleAll" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="regid">
              ...
              <vxe-table-column field="statusname" title="状态" min-width="90px" show-header-overflow show-overflow="title" show-footer-overflow>
                <template slot-scope="scope">
                  <div>
                    ...
                    <span style="color: red" v-show="scope.row.status == 8">
                      已二审
                    </span>
                  </div>
                </template>
              ...
              <vxe-table-column field="payamount" title="超额费用" width="90px" show-header-overflow show-overflow="title" show-footer-overflow>
                <template slot-scope="scope">
                  <span v-if="scope.row.chargeflag == 1" style="color: green">{{
                    scope.row.payamount
                  }}</span>
                  <span v-if="scope.row.chargeflag == 0">{{
                    scope.row.payamount
                  }}</span>
                </template>
              </vxe-table-column>
              <vxe-table-column field="setmealname" title="套餐" width="110px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              ...
              <vxe-table-column field="department" title="部门" min-width="120px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              <vxe-table-column field="seqno" title="序号" min-width="80px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              <vxe-table-column field="chargeflagcn" title="收费状态" min-width="80px" show-header-overflow show-overflow="title" show-footer-overflow>
              </vxe-table-column>
              <vxe-table-column field="discountprice" title="总费用" min-width="90px" show-header-overflow show-overflow="title" show-footer-overflow>
              </vxe-table-column>
              ...
              <vxe-table-column field="totalcheckdoctorname" title="总检医生" min-width="100px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              <vxe-table-column field="totalcheckcheckname" title="审核医生" min-width="100px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              <vxe-table-column field="reportprintflag" title="打印状态" min-width="100px" show-header-overflow show-overflow="title" show-footer-overflow>
                <template slot-scope="scope">
                  <span v-if="scope.row.reportprintflag == 0">未打印</span>
                  <span v-if="scope.row.reportprintflag == 1">已打印</span>
                </template>
              </vxe-table-column>
              <vxe-table-column field="reportprintdate" title="打印时间" min-width="180px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              ...
              <vxe-table-column field="totalcheckinputdate" title="总检时间" min-width="180px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              <vxe-table-column field="totalcheckcheckdate" title="审核时间" min-width="180px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              ...
              <!-- <vxe-table-column
                ...
                show-footer-overflow
              ></vxe-table-column> -->
              <vxe-table-column title="操作" min-width="220px" show-header-overflow show-overflow="title" show-footer-overflow>
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
      ...
      FileExportExport({
        ...
        .then((res) => {
          if (res.result) {
            ...
            link.setAttribute("download", "综合查询"); // 设置文件名
      ...
      this.$confirm("此操作将删除当前勾选人员, 是否继续?", "提示", {
        ...
        .then((status) => {
          if (status == "confirm") {
            ...
            if (delArr.length == 0) {
              this.$message.warning("暂无符合删除条件人员!");
              return;
            }
    ...
    // 失去光标检索框
    handleBlurcheckupWorkInput() {
      this.$nextTick(() => {
        this.$refs.checkupWorkInput.blur();
      });
    },
    handleEnterRegid() {
    ...
    handleChangeTotalcheckDate(arr) {
      if (arr) {
        this.searchForm.starttotalcheckdate = arr[0];
        this.searchForm.endtotalcheckdate = arr[1];
      } else {
        this.searchForm.starttotalcheckdate = "";
        this.searchForm.endtotalcheckdate = "";
      }
      this.handleSearchPageTable();
    },
    handleGetPrintDate() {
          ...
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "报告书") {
              this.downloadList.forEach((printItem) => {
                ...
                //   if (printItem.checktype == 10104) {
                //     repname = `${indexP}F报告书招工入职`;
                //   } else if (printItem.checktype == 925) {
                ...
                let data = {
                  ...
                  isselectrepname: this.chooseReport ? true : false,
    ...
    handleGetExportFile() {
      ...
      Object.keys(this.searchForm).forEach((item) => {
        if (this.searchForm[item] && item != "statusin") {
          whereitems.push({
            key: item,
            value: this.searchForm[item],
          });
      ...
      if (this.searchForm.statusin.length > 0) {
        if (this.searchForm.statusin.includes("")) {
          whereitems.push({
            key: "status",
            value: "",
          });
          ...
          whereitems.push({
            key: "statusin",
            value: this.searchForm.statusin.join(","),
          });
        }
      }
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          ReportBook(data).then((res) => {
            ...
            // a.click();
            this.forceDownloadPDF(url, `${k.name}-${k.regid}`);
            return resolve("ok");
          });
        } catch (err) {
          return reject(err);
    ...
    async forceDownloadPDF(url, filename) {
      try {
        // 1. 使用fetch获取文件
        ...
        .then((res) => {
          ...
          if (res.data) {
            ...
            if (
              ...
              if (res.data[0].reportprintflag == 1) {
                this.$confirm("该用户已经打印, 是否继续打印?", "提示", {
                  ...
                  .then((status) => {
                    if (status == "confirm") {
                      this.handleBlurcheckupWorkInput();
    ...
    handleJumpPage(data, item) {
      ...
      } else if (item.code == "TotalCheck2") {
        jumpRouteLink(this, "totalcheck2");
        this.$router.push({
          path: `/totalcheck2/index`,
          query: { regid: data.regid },
        });
      }
    },
    ...
    handlePrintReport() {
      this.$refs.popoverRefIdc.forEach(async (d) => {
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          let repname = d.$children[0].$attrs["data-remark"];
          for (let i = 0; i < this.downloadList.length; i++) {
            if (this.downloadList[i].reporttake == "电子版报告") {
              this.$confirm(
                ` 该报告领取方式为电子版报告，是否继续打印?`,
                "提示",
                {
                  confirmButtonText: "是",
                  cancelButtonText: "否",
      ...
      return new Promise((resolve, reject) => {
        try {
          let data = {
            ...
            isselectrepname: repObj ? true : false,
            // repname: d.$children[0].$attrs["data-remark"],
            designtype: repObj
              ? repObj.unioncode
              : repname.includes("F")
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnExport1": // 导出
          ...
          break;
        case "btnOneKeyExport": // 一键导出
          this.handleOneKeyExportFile();
          break;
        case "btnDelete": // 删除个人回调
          this.handleDeletePersonnel();
          break;
