  ...
  <div class="full_search">
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  ...
                  <el-link
                    style="margin-right: 8px; color: #bd10e0"
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="(showDetails = false), (combData = [])"
          >关 闭</el-button
        >
      </span>
    </el-dialog>
...
import { setPrintOrderDetail } from "@/api/totalcheck.js"; // 引入接口文件
// 引入页面接口
  ...
  methods: {
    ...
    handleGetPrintDate() {
      getSysConfigInfo({ cc: "Sys_FullSearch_DateRange" }).then((res) => {
        ...
        this.checkDate = [
          `${this.gettime(start)} 00:00:00`,
          `${this.gettime(end)} 23:59:59`,
        ];
        this.searchForm.startcheckdate = `${this.gettime(start)} 00:00:00`;
        this.searchForm.endcheckdate = `${this.gettime(end)} 23:59:59`;
        ...
        case "btnGuide": // 指引单
          this.$refs.popoverRefIdc.forEach((d) => {
            ...
            if (d.$children[0].$el.innerText == "指引单") {
              ...
              let repname = "";
              if ([2, 7].includes(+this.downloadList[0].checktype)) {
          ...
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "报告书") {
              this.downloadList.forEach((printItem) => {
                ...
                let repname = "";
                if (
                  printItem.checktype == 1 ||
                  printItem.checktype == 3 ||
                  printItem.checktype == 8
                ) {
                  repname = `${indexP}Fvip报告书`;
                } else if (
                  printItem.checktype == 2 ||
                  printItem.checktype == 7
                ) {
                  repname = `${indexP}F报告书`;
                } else if (printItem.checktype == 6) {
                  repname = `${indexP}F健康教师报告书`;
                } else if (printItem.checktype == 11) {
                  repname = `${indexP}F职业体检表`;
                } else if (printItem.checktype == 12) {
    ...
    handleGetExportFile() {
      ...
      Object.keys(this.searchForm).forEach((item) => {
        if (this.searchForm[item] && item != "batch" && item != "status") {
          whereitems.push({
            key: item,
      ...
      if (
        ...
        !this.searchForm.status.includes("")
      ) {
        whereitems.push({
          key: "statusin",
          value: this.searchForm.status.join(","),
        });
      }
      getExportFile({
    ...
    handleExportPdf(k) {
      return new Promise((resolve, reject) => {
        try {
          let repname = "";
          if (k.checktype == 1 || k.checktype == 3 || k.checktype == 8) {
            repname = `${indexP}Fvip报告书`;
          } else if (k.checktype == 2 || k.checktype == 7) {
            repname = `${indexP}F报告书`;
          } else if (k.checktype == 6) {
            repname = `${indexP}F健康教师报告书`;
          } else if (k.checktype == 11) {
            repname = `${indexP}F职业体检表`;
          } else if (k.checktype == 12) {
            repname = `${indexP}F放射体检表`;
          } else {
            repname = d.$children[0].$attrs["data-remark"];
          }
    ...
    handlePrintReport() {
      this.$refs.popoverRefIdc.forEach(async (d) => {
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          let printArr = [];
          for (let i = 0; i < this.downloadList.length; i++) {
            if (this.downloadList[i].status < 7) {
              tipsArr.push(this.downloadList[i]);
            } else {
              printArr.push(this.downloadList[i]);
            }
          }
          if (tipsArr.length > 0) {
            this.$message.warning(`已为你跳过未审核人员!`);
          }
          for (let i = 0; i < printArr.length; i++) {
            if (
              printArr[i].checktype == 1 ||
              printArr[i].checktype == 3 ||
              printArr[i].checktype == 8
            ) {
              repname = `${indexP}Fvip报告书`;
            } else if (
              printArr[i].checktype == 2 ||
              printArr[i].checktype == 7
            ) {
              repname = `${indexP}F报告书`;
            } else if (printArr[i].checktype == 6) {
              repname = `${indexP}F健康教师报告书`;
            } else if (printArr[i].checktype == 11) {
              repname = `${indexP}F职业体检表`;
            } else if (printArr[i].checktype == 12) {
              repname = `${indexP}F放射体检表`;
            } else {
              repname = d.$children[0].$attrs["data-remark"];
            }
            await this.handlePrintReportAsync(repname, printArr[i]);
          }
        }
      });
    },
    // 弹框打印申请单
    handlePrintReportAsync(repname, item) {
      return new Promise((resolve, reject) => {
        try {
          let data = {
            fingercode: localStorage.getItem("md5code"),
          ...
          timeId = setTimeout(() => {
            printBase64(data).then((res) => {
              ...
              //   printname == "" ? this.loddp.GET_PRINTER_NAME(-1) : printname
              // );
