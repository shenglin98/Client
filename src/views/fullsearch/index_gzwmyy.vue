          ...
          <div class="page_table">
            <vxe-table ref="pageTable" height="auto" border="inner" :loading="loading" highlight-hover-row :data="pageData" :sort-config="{ trigger: 'cell' }" @checkbox-change="handleSelectPeople" @checkbox-all="handleSelectPeopleAll" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="regid">
              ...
              <vxe-table-column title="操作" min-width="150px" show-header-overflow show-overflow="title" show-footer-overflow>
                <template slot-scope="scope">
                  <el-link v-for="(item, index) in scope.row.operateitems" :key="index" style="margin-right: 8px" :type="index == 0 ? 'primary' : index == 1 ? 'warning' : 'success'" @click="handleJumpPage(scope.row, item)">
                    {{ item.name }}
                  </el-link>
                </template>
              </vxe-table-column>
        ...
        case "btnGuide": // 指引单
          this.$refs.popoverRefIdc.forEach((d) => {
            ...
            if (d.$children[0].$el.innerText == "指引单") {
              ...
              } else if (this.downloadList[0].meccode == 10007) {
                if (this.downloadList[0].personalflag == 1) {
                  repname = `${indexP}汕头第一健康团体指引单`;
                } else {
                  repname = `${indexP}汕头第一健康指引单`;
                }
              }
              let data = {
          ...
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "报告书") {
              ...
              this.downloadList.forEach((printItem) => {
                ...
                } else if (printItem.meccode == 10007) {
                  repname = `${indexP}F汕头第一健康报告书`;
                } else {
                  repname = d.$children[0].$attrs["data-remark"];
                }
                let data = {
      ...
      this.$refs.popoverRefIdc.forEach((d) => {
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          if (this.downloadList[this.downloadList.length - 1].meccode == 1) {
            ...
            this.downloadList[this.downloadList.length - 1].meccode == 10007
          ) {
            repname = `${indexP}F汕头第一健康报告书`;
          } else {
            repname = d.$children[0].$attrs["data-remark"];
    ...
    handleExportPdf(k) {
      return new Promise((resolve, reject) => {
        try {
          ...
          } else if (k.meccode == 10007) {
            repname = `1、F汕头第一健康报告书`;
          } else {
            repname = `1、报告书`;
          }
          let data = {
        ...
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          for (let i = 0; i < this.downloadList.length; i++) {
            if (this.downloadList[i].meccode == 1) {
              ...
              repname = `${indexP}F广州第一健康报告书`;
            } else if (this.downloadList[i].meccode == 10007) {
              repname = `${indexP}F汕头第一健康报告书`;
            } else {
              repname = d.$children[0].$attrs["data-remark"];
            }
