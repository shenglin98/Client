        ...
        <el-col class="form_box" :span="6">
          ...
          <el-form ref="form" :model="searchForm">
            <el-form-item class="select" size="mini">
              <div class="form_label">自动打印</div>
              <div class="auto_btn">
                <el-switch style="margin-left: 30px" v-model="autoPrintReport" active-color="#13ce66" inactive-color="#909399" active-text="报告书">
                </el-switch>
                <el-switch style="margin-left: 30px" v-model="autoPhysique" active-color="#13ce66" inactive-color="#909399" active-text="体格检查表">
                </el-switch>
              </div>
            </el-form-item>
            ...
            <el-form-item class="input" size="mini">
              <div class="form_label" style="width: 80px">身份证</div>
              <div class="idcard_btn">
                ...
                <el-button type="primary" size="mini" plain @click="handleSearchCardReading">读卡</el-button>
              </div>
            </el-form-item>
            <el-form-item class="input" size="mini">
              <div class="form_label">档案号</div>
              <el-input v-model="searchForm.checkno" clearable @clear="handleSearchPageTable" @keyup.native.enter="handleSearchPageTable"></el-input>
            </el-form-item>
            <el-form-item class="input" size="mini">
              <div class="form_label">门诊卡号</div>
        ...
        <el-col class="table_box" :span="18">
          ...
          <div class="page_table">
            <vxe-table ref="pageTable" height="auto" border="inner" :loading="loading" highlight-hover-row :data="pageData" :sort-config="{ trigger: 'cell' }" @checkbox-change="handleSelectPeople" @checkbox-all="handleSelectPeopleAll" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="regid">
              ...
              <vxe-table-column field="checktypename" title="体检类型" width="140px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              ...
              <vxe-table-column field="signoutdate" title="交表时间" min-width="150px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              ...
              <vxe-table-column field="department" title="部门" min-width="200px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              <vxe-table-column field="department2" title="二级部门" min-width="200px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
  ...
  methods: {
    ...
    handleExportXml() {
      ...
      for (let i = 0; i < this.downloadList.length; i++) {
        if (!this.downloadList[i].fileid) {
          tips.push(this.downloadList[i].name);
        }
      }
      if (tips.length > 0) {
        this.$message.warning(`${tips.join("、")}暂无照片信息,请取消勾选！`);
        return;
      }
      getExportExportXml({
        apicontrol: "FullSearch",
        codeitems: this.selectArr,
      }).then((res) => {
        if (!res.result) return;
        this.$message.success(res.systemWarn);
        const a = document.createElement("a");
        a.href = res.result;
        a.download = `xml文件`; // 下载文件的名字
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnDelete": // 删除个人回调
          ...
          break;
        case "btnPrintRegisList": // 体检登记清单
          this.handlePrintRegisList();
          break;
        case "btnGuide": // 指引单
