  ...
  <div class="full_search" v-loading="loadPage" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
    ...
    <div class="filter-container">
      <div v-for="(btn, index) of buttons" :key="index">
        ...
        <el-button :ref="btn.domId" type="primary" plain v-else :size="size" v-bind:key="btn.Id" v-bind:data-remark="btn.remark" class="filter-item" @click="hadnleOnBtnClicked(btn.domId)"><i :class="`iconfont icon-${btn.icon}`"></i>{{ btn.name }}</el-button>
      </div>
    </div>
    <!-- 主体内容区域 -->
    <div class="subject">
          ...
          <el-form ref="form" :model="searchForm">
            ...
            <el-form-item class="checkup_date" size="mini">
              ...
              <el-popover ref="popoverRef" placement="bottom-start" trigger="click">
                <div class="content">
                  <div class="content_item" v-for="item in dateCodeArr" :key="item.code" @click="handleSelectConditionText(item)">
                    {{ item.name }}
                  </div>
                </div>
                <el-button type="primary" plain style="width: 80px" size="mini" slot="reference">{{ conditionText }}</el-button>
              </el-popover>
            ...
            <el-form-item class="select" size="mini">
              <div class="form_label">体检套餐</div>
              <el-select v-model="searchForm.setmealcode" clearable filterable @change="handleSearchPageTable">
                <el-option v-for="(item, index) in packageItems" :key="index" :label="item.name" :value="item.code"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item class="select" size="mini">
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="searchForm.reporttype" clearable @change="handleSearchPageTable">
        ...
        <el-col class="table_box" :span="19">
          ...
          <div class="page_table">
            <vxe-table ref="pageTable" height="auto" border="inner" :loading="loading" highlight-hover-row :data="pageData" :sort-config="{ trigger: 'cell' }" @checkbox-change="handleSelectPeople" @checkbox-all="handleSelectPeopleAll" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="regid">
              ...
              <vxe-table-column field="statusname" title="状态" min-width="90px" show-header-overflow show-overflow="title" show-footer-overflow>
                <template slot-scope="scope">
                  <div>
                    ...
                    <span style="color: #ffcc00" v-show="scope.row.status == 3">体检中</span>
                    <span v-show="scope.row.status == 4">已交表</span>
                    <span v-show="scope.row.status == 5">可总检</span>
                    <span style="color: #4cd964" v-show="scope.row.status == 6">已总检</span>
              ...
              <vxe-table-column field="checktypename" title="体检类型" min-width="150px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
          ...
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[100, 200, 300, 1000,5000,10000]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="count">
          </el-pagination>
        </el-col>
      </el-row>
    </div>
...
export default {
  ...
  methods: {
        ...
        case "btnGuide": // 指引单
          this.$refs.popoverRefIdc.forEach((d) => {
            ...
            if (d.$children[0].$el.innerText == "指引单") {
              ...
              } else if (this.downloadList[0].checktype == 3) {
                repname = `${index}、食品从业指引单`;
              } else if (this.downloadList[0].checktype == 5) {
                repname = `${index}、普通员工指引单`;
              } else if (this.downloadList[0].checktype == 6) {
                repname = `${index}、自选项目指引单`;
              } else if (this.downloadList[0].checktype == 7) {
                repname = `${index}、驾驶证指引单`;
              } else if (this.downloadList[0].checktype == 8) {
    ...
    handleGetExportFile() {
      ...
      if (this.conditionDate && this.conditionDate.length > 0) {
        if (this.conditionDateCode == "登记日期") {
          whereitems.push({
            key: "startdate",
            value: this.conditionDate[0],
          });
    ...
    // 点击选择检索体检条件回调
    handleSelectConditionText(val) {
      this.conditionText = val.name;
      this.conditionDateCode = val.code;
      this.$refs.popoverRef.doClose(); // 关闭弹出层popover
      this.handleSearchPageTable();
    },
    handleHealthPrint(domId) {
      if (this.selectArr.length == 0) return;
      let d = this.$refs[domId];
      if (d[0].$el.innerText == "健康证打印") {
        let repname = d[0].$attrs["data-remark"] || "1、健康证";
        let data = {
          fingercode: localStorage.getItem("md5code"),
          codes: this.selectArr,
          repname,
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnExportPdf": // 导出PDF
          ...
          break;
        case "btnExportPdfEntry": // 导出PDF-入职
          this.handleGetExportPdfEntry();
          break;
        case "btnDownloadHead": // 导出照片
          this.handleDownloadHead();
          break;
        case "btnHealthPrint": // 导出照片
          this.handleHealthPrint(domId);
          break;
