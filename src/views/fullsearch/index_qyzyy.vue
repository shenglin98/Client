    ...
    <div class="filter-container">
      <div v-for="(btn, index) of buttons" :key="index">
        <el-popover ref="popoverRefIdc" v-if="domIdArr.includes(btn.domId)" class="print_box" placement="bottom" trigger="click">
          ...
          <div class="print_item" @click="hadnleOnBtnClicked(btn.domId)" v-if="btn.domId != 'btnPhysique'">
            打印
          </div>
          <div class="print_item" @click="handlePreview(btn.domId)" v-if="btn.domId != 'btnPhysique'">
            预览
          </div>
        ...
        <el-col class="table_box" :span="19">
          ...
          <div class="page_table">
            <vxe-table ref="pageTable" height="auto" border="inner" :loading="loading" highlight-hover-row :data="pageData" :sort-config="{ trigger: 'cell' }" @checkbox-change="handleSelectPeople" @checkbox-all="handleSelectPeopleAll" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="regid">
              ...
              <vxe-table-column field="department" title="部门" width="150px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    handleOneKeyExportFile() {
      ...
      // let url = `${this.baseConfigUrl}/FileExport/Export`;
      // this.$axios
      //   .post(
      //     url,
      //     {
    ...
    handleSelectPrintDia() {
      ...
      this.$refs.popoverRefIdc.forEach((d) => {
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          let repname = "";
          if (this.pageData[index].checktype == "12") {
          ...
          let data = {
            ...
            // repname: this.chooseReport
            //   ? this.chooseReport.name
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnReport": // 导出报告书
          ...
          break;
        case "btnPrintRegisList": // 导出报告书
          this.$refs.popoverRefIdc.forEach((k) => {
            k.doClose(); // 关闭弹出层popover
          });
          let temp = this.pageData.filter((k) => {
            return k.regid == this.chooseArr[0];
          })[0];
          if (!temp) return;
          if (temp.status >= 6) {
