        ...
        <el-col class="form_box" :span="5">
          ...
          <el-form ref="form" :model="searchForm">
            ...
            <el-form-item class="select" size="mini">
              ...
              <el-select v-model="searchForm.batch" clearable>
        ...
        <el-col class="table_box" :span="19">
          ...
          <div class="page_table">
            <vxe-table ref="pageTable" height="auto" border="inner" :loading="loading" highlight-hover-row :data="pageData" :sort-config="{ trigger: 'cell' }" @checkbox-change="handleSelectPeople" @checkbox-all="handleSelectPeopleAll" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="regid">
              ...
              <vxe-table-column field="checktypename" title="体检类型" width="110px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
            ...
            if (d.$children[0].$el.innerText == "报告书") {
              this.downloadList.forEach((printItem) => {
                ...
                if (printItem.meccode == 1) {
                  ...
                  } else if (printItem.checktype == 7) {
                    repname = `${indexP}F报告书招工入学`;
                  } else if (printItem.checktype == 6) {
    ...
    handleExportPdf(k) {
      return new Promise((resolve, reject) => {
        try {
          let repname = "";
          if (k.meccode == 1) {
            if (k.checktype == 10104) {
              repname = `1、F报告书招工入职`;
            } else if (k.checktype == 925) {
              repname = `1、F报告书教师体检`;
            } else if (k.checktype == 7) {
              repname = `1、F报告书招工入学`;
            } else if (k.checktype == 6) {
              repname = `1、F报告书健康证`;
            } else {
              repname = `1、F报告书`;
            }
          ...
          ReportBook(data).then((res) => {
            ...
            // a.click();
            this.forceDownloadPDF(url, `${k.regid}-${k.name}`);
            return resolve("ok");
          });
        } catch (err) {
          return reject(err);
    ...
    handlePrintReportAuto(item) {
      this.$refs.popoverRefIdc.forEach(async (d) => {
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          let repname = "";
          if (item.meccode == 1) {
            if (item.checktype == 10104) {
              repname = `${indexP}F报告书招工入职`;
            } else if (item.checktype == 925) {
              repname = `${indexP}F报告书教师体检`;
            } else if (item.checktype == 7) {
              repname = `${indexP}F报告书招工入学`;
        ...
        if (d.$children[0].$el.innerText == "报告书") {
          ...
          for (let i = 0; i < this.downloadList.length; i++) {
            if (this.downloadList[i].meccode == 1) {
              if (this.downloadList[i].checktype == 10104) {
                repname = `${indexP}F报告书招工入职`;
              } else if (this.downloadList[i].checktype == 925) {
                repname = `${indexP}F报告书教师体检`;
              } else if (this.downloadList[i].checktype == 7) {
                repname = `${indexP}F报告书招工入学`;
              } else if (this.downloadList[i].checktype == 6) {
                repname = `${indexP}F报告书健康证`;
              } else {
                repname = `${indexP}F报告书`;
              }
