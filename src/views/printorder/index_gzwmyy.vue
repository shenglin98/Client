...
export default {
  ...
  methods: {
    ...
    // 打印条码
    handlePrintBarCode() {
      console.log(this.chooseStaffList);
      if (this.chooseStaffList.length == 0) {
      ...
      this.$refs.printorder.$children.forEach((d) => {
        if (d.$el.innerText == "指引单模板") {
          ...
          if (this.meccode == 1) {
            ...
            repname = `${indexP}第一健康团体指引单模板`;
          } else if (this.meccode == 10006) {
            repname = `${indexP}广州第一健康团体指引单模板`;
          } else if (this.meccode == 10007) {
            repname = `${indexP}汕头第一健康团体指引单模板`;
          }
          let data = {
            fingercode: localStorage.getItem("md5code"),
            repname: repname,
