  ...
  <div class="print_order">
    ...
    <sticky :className="'sub-navbar'">
      <div class="filter-container" style="display: flex;justify-content: flex-end;">
        <el-upload style="margin-right: 10px" class="upload-demo" name="files" action="" accept=".rar,.zip" :show-file-list="false" :auto-upload="false" :on-change="handleChangeUploadPicture" :file-list="fileList">
          ...
          <el-row>
            ...
            <el-col :span="4">
              <el-form-item size="mini" label="分组">
                <el-select filterable v-model="searchForm.groupcode" clearable placeholder="请选择分组" @change="handleChangeGroup">
          ...
          <el-row>
            ...
            <el-col :span="4">
              <el-form-item size="mini" label="部门">
                <el-select v-model="searchForm.departcode" filterable clearable placeholder="请选择部门" @change="handleGetPrintOrderList">
    ...
    <div class="table_box">
      ...
      <el-card class="box-card table_left">
        ...
        <!-- 人员表格数据 -->
        <EasyTable class="staff_table" :loading="loading" height="100%" :data="staffTableData" ref="staffTable" :tableOptions="staffTableOption" @row-click="handleChoosePeople" :checkbox-config="{ fetch: true, rowClick: true }" @selection-change="handleSelectStaff" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="{ height: '32px' }" :cell-style="{ padding: '0' }" />
    ...
    <!-- 打印弹框 -->
    <el-dialog title="打印" :visible.sync="showPrintDialog" class="printDialog" width="40%" :show-close="false" :close-on-click-modal="false">
      <span>
        <div class="all_box">
          <el-checkbox v-model="checkAllFlag" @change="handleCheckAllFlag">全选</el-checkbox>
        </div>
        ...
        <el-checkbox v-model="singleKindergarten">幼儿园入学体检表</el-checkbox>
        <el-checkbox v-model="childrenCheckup">儿童健康体检检查表</el-checkbox>
      </span>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="handleClosePrintDia">取 消</el-button>
        <el-button size="mini" type="primary" @click="handlePrintDiaConfirm">打 印</el-button>
      </span>
    </el-dialog>
    <!-- 折扣 -->
    <el-dialog class="dialog-mini discount_dialog" width="20%" top="20%" :close-on-click-modal="false" title="修改标准金额" :visible.sync="showQuota" @close="handleCloseQuotat">
      <div class="discount_box">
        <el-input size="mini" placeholder="请输入金额" max="100" min="1" v-model="quota" clearable>
        </el-input>
      </div>
      <span slot="footer" class="dialog-footer history_table_footer">
        <el-button size="mini" type="primary" @click="handleConfirmQuota">确 认</el-button>
        <el-button size="mini" @click="handleCloseQuotat">取 消</el-button>
      </span>
    </el-dialog>
    <!-- 体检项目 -->
...
import { getFilesUpload, setUploadByZip } from "@/api/unitregistration.js";
export default {
  name: "printorder",
  ...
  data() {
    return {
      ...
      singleKindergarten: false, // 幼儿园入学体检表开关
      childrenCheckup: false, // 幼儿园入学体检表开关
      ...
      quota: 0,
      fileList: [], // 文件上传列表
      avatarImg: require("@/assets/lin.jpg"),
    };
  },
  computed: {
  ...
  methods: {
    ...
    handleChangeUploadPicture(file, fileList) {
      if (this.searchForm.companycode == "") {
        this.$message.warning("请先选择需要导入照片单位!");
        return false;
      }
      if (this.searchForm.batch == "") {
        this.$message.warning("请先选择需要导入照片批次!");
        return false;
      }
    ...
    handleChangeQuota() {
      if (this.chooseStaffList.length == 0) {
        this.$message.warning(`请勾选需要操作的人员信息!`);
        return false;
      }
      this.showQuota = true;
    },
    handleConfirmQuota() {
      if (+this.quota <= 0) {
        this.$message.warning(`请输入数字类型,且不小于0`);
        return false;
      }
      AlterQuota({
        regiditems: this.chooseStaffList.map((k) => k.regid),
        quota: this.quota,
      }).then((res) => {
    ...
    async handlePrintDiaConfirm() {
      ...
      for (let i = 0; i < this.chooseStaffList.length; i++) {
        ...
        this.censorShip &&
          (await this.handleDiaPrintCensorShip(this.chooseStaffList[i])); // 打印送检单
        this.singleKindergarten &&
          (await this.handleDiaPrintSingleKindergarten(
            this.chooseStaffList[i]
          )); // 打印送检单
        this.childrenCheckup &&
          (await this.handleDiaPrintChildrenCheckup(this.chooseStaffList[i])); // 打印儿童健康体检检查表
      }
    ...
    handleCheckAllFlag(value) {
      ...
      this.singleKindergarten = value; // 送检单开关
      this.childrenCheckup = value; // 送检单开关
    },
    // 弹框打印指引单
    ...
    // 弹框打印儿童健康体检检查表
    handleDiaPrintChildrenCheckup(item) {
      return new Promise((resolve, reject) => {
        try {
          let data = {
            fingercode: localStorage.getItem("md5code"),
    ...
    // 弹框打印幼儿园体格检查表
    handleDiaPrintSingleKindergarten(item) {
      return new Promise((resolve, reject) => {
        try {
          let data = {
            fingercode: localStorage.getItem("md5code"),
    ...
    handleClosePrintDia() {
      ...
      this.singleKindergarten = false; // 送检单开关
      this.childrenCheckup = false; // 送检单开关
    ...
    // id获取文件回调
    handleGetFiles() {
      getFiles({ Id: this.choosePeople.fileid }).then((res) => {
    ...
    handleChoosePeople(row) {
      this.choosePeople = Object.assign({}, row);
      this.handleSetPrintOrderDetail();
      if (this.choosePeople.fileid) {
        this.handleGetFiles(); // 获取头像
      } else {
        this.avatarImg = "";
      }
    },
    ...
    handlePrintGuidance(flag) {
      ...
      this.$refs.printorder.$children.forEach(async (d) => {
        if (
          d.$el.innerText == "打印指引单" ||
          d.$el.innerText == "导出指引单"
        ) {
          // this.chooseStaffList.forEach((k) => {
          ...
          for (let i = 0; i < this.chooseStaffList.length; i++) {
            let indexP = d.$attrs["data-remark"].split("、")[0] + "、";
            let repname = "";
            if (this.chooseStaffList[i].checktype == 12) {
              repname = `${indexP}专项指引单`;
            } else {
              repname = d.$attrs["data-remark"];
            }
            console.log("进来2");
            if (flag == "print") {
              console.log("进来打印");
              await this.handlePrintGuidanceApi(
                this.chooseStaffList[i],
                repname
              );
            } else {
              console.log("进来导出");
              await this.handlePrintGuidanceDownload(
                this.chooseStaffList[i],
                repname
              );
            }
    ...
    // 循环导出指引单
    handlePrintGuidanceDownload(k, repname) {
      console.log("下载");
      return new Promise((resolve, reject) => {
        try {
          let data = {
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnGuidance": // 打印指引单
          this.handlePrintGuidance("print");
          break;
        case "btnExportGuidance": // 导出指引单
          this.handlePrintGuidance("download");
          break;
