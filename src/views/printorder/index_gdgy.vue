<template>
  <div class="print_order">
    ...
    <div class="filter-container" style="height:55px;display: flex;align-items: center;background-color: #fff;justify-content: flex-end;padding-right: 12px;">
      <div class="take_c" style="display: flex;align-items: center;">
        <div class="take_c_title">修改客户级别：</div>
        <el-select v-model="level" size="mini" @change="handleChangeLevel" clearable @clear="handleChangeLevel" placeholder="修改客户级别">
    ...
    <el-card class="box-card search_box">
      ...
      <el-form ref="form" class="search_form" style="margin-top: 10px;" :model="searchForm" label-width="80px">
        <el-row>
          <el-col :span="8">
          ...
          <el-col :span="4">
            <el-form-item size="mini" label="婚姻状况">
              <el-select style="width: 100%;" v-model="searchForm.marriage" clearable placeholder="请选择婚姻" @change="handleGetPrintOrderList">
        ...
        <el-row>
          ...
          <el-col :span="4">
            <el-form-item size="mini" label="客户级别">
              <el-select style="width: 100%;" v-model="searchForm.level" clearable placeholder="请选择状态" @change="handleGetPrintOrderList">
                <el-option label="普通" :value="0"></el-option>
                <el-option label="VIP" :value="1"></el-option>
                <el-option label="保密" :value="2"></el-option>
              </el-select>
...
export default {
  ...
  methods: {
    ...
    handleChangeLevel(value) {
      ...
      this.$confirm("此操作将修改当前人员客户级别,请确认?", "提示", {
        ...
        .then(async () => {
          let data = [
            {
              key: "regid",
              valueList: this.chooseStaffList.map((k) => k.regid),
            },
            {
              key: "level",
              value,
          ...
          BatchAlterFields(data).then((res) => {
      ...
      this.$refs.printorder.$children.forEach(async (d) => {
        if (d.$el.innerText == "打印指引单") {
          ...
          for (let i = 0; i < this.chooseStaffList.length; i++) {
            ...
            if (this.chooseStaffList[i].checktype == 3) {
              repname = `${indexP}学生指引单`;
            } else {
              repname = d.$attrs["data-remark"];
            }
        ...
        if (d.$el.innerText == "打印指引单") {
          ...
          for (let i = 0; i < this.chooseStaffList.length; i++) {
            ...
            for (
              ...
              await this.handlePrintApplyBatchApi(
                this.chooseStaffList[i],
                this.chooseStaffList[i].batchprintdepts[j].deptcode
              );
            }
          }
          if (
    ...
    // 打印指引单
    handlePrintGuidanceDL(flag) {
      if (this.chooseStaffList.length == 0) {
        this.$message.warning("请先选择需要打印指引单人员!");
        return false;
      }
      this.$refs.printorder.$children.forEach(async (d) => {
        if (d.$el.innerText == "导出指引单") {
          this.chooseStaffList = this.chooseStaffList.sort(function (a, b) {
            return a.regid - b.regid;
          });
          let indexP = d.$attrs["data-remark"].split("、")[0] + "、";
    ...
    // 循环打印申请单
    handlePrintApplyBatchApi(k, deptcode) {
      return new Promise((resolve, reject) => {
        try {
          let data = {
            fingercode: localStorage.getItem("md5code"),
            codes: [k.regid],
            repname: k.meccode == 1 ? "1、科室申请单" : "1、科室申请单A5",
            // repname: d.$attrs["data-remark"],
            printflag: 1,
            whereitems: [
              {
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnGuideSingleAndApply": // 打印指引单+申请单
          this.handlePrintGuidanceAndApply();
          break;
        case "btnExportGuidance": // 导出指引单
          this.handlePrintGuidanceDL("download");
          break;
        case "btnOnekey": // 一键打单
