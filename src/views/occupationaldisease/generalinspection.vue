<template>
  <div class="total_check">
    ...
    <div class="filter-container">
      <div v-for="(btn, index) of buttons" :key="index">
        <el-popover ref="popoverRefIdc" v-if="domIdArr.includes(btn.domId)" class="print_box" placement="bottom" trigger="click">
          <el-button type="primary" plain slot="reference" :size="size" v-bind:key="btn.Id" v-bind:data-remark="btn.remark" class="filter-item"><i :class="`iconfont icon-${btn.icon}`"></i>{{
                            btn.name }}</el-button>
        ...
        <el-button type="primary" plain v-else :size="size" v-bind:key="btn.Id" v-bind:data-remark="btn.remark" :loading="saveLoading" class="filter-item" @click="hadnleOnBtnClicked(btn.domId)"><i :class="`iconfont icon-${btn.icon}`"></i>{{ btn.name }}</el-button>
      </div>
    </div>
    <!-- 主体内容区域 -->
    <div class="subject">
              ...
              <el-row>
                ...
                <el-col :span="3">
                  <el-form-item size="mini" label="年龄">
                    <el-input disabled v-model="currentPeople.age"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item size="mini" label="总检状态">
                    <el-input disabled class="font18" :class="[currentPeople.status >= 6 ? 'is_red' : '']" v-model="currentPeople.statuscn"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item size="mini" label="体检时间">
                    <el-input disabled v-model="currentPeople.registercheckdate"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="3">
              ...
              <el-row>
                <el-col :span="6">
                  <el-form-item size="mini" label="团检批次">
                    <el-input disabled v-model="currentPeople.batch"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
        ...
        <div class="personnel_box" :class="showLeftAnm ? '' : 'personnel_box_n'">
          <div class="show_box" v-show="showLeftAnm">
            <div class="pers_table_box">
              ...
              <EasyTable :loading="loading" class="pers_table" height="100%" :data="persData" @row-click="handleChoosePers" :tableOptions="persOptions" :radio-config="{
                                    rowClick: true,
            ...
            <div class="comb_table_box">
              ...
              <EasyTable :loading="loading" class="comb_table" height="100%" :data="combData" :tableOptions="combOptions" :radio-config="{
                                    rowClick: true,
                                }" @row-dblclick="handleChooseComb" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="{ height: '32px' }" :cell-style="{ padding: '0' }" />
            </div>
          </div>
          ...
          <div class="body_box">
            ...
            <div class="check_body">
              <div class="check_left">
                <div class="btn_top_box">
                  ...
                  <el-radio-group v-model="checkActive" @change="handleTabsChange" size="mini">
                    ...
                    <el-radio-button label="beginning">初检</el-radio-button>
                    <el-radio-button v-for="item,index in reviewTheProject" :label="index" :key="index">第{{index + 1}}次复检</el-radio-button>
                  </el-radio-group>
                  <!-- 生成综述 -->
                  <!-- <el-button type="primary" class="overview" size="mini" plain
                                        @click="handleResetOverView">生成综述</el-button> -->
                  <!-- <div class="handle_btn" @click="handleShowCommonWords">
                ...
                <div class="btn_top_box" style="margin-top:10px;">
                  <el-radio-group v-model="checkActiveItem" @change="handleTabsItemChange" size="mini">
                    <el-radio-button label="briefSummary">项目小结</el-radio-button>
                    <el-radio-button label="checkTheResults">检查结果</el-radio-button>
                    <el-radio-button label="contrast">历史对比</el-radio-button>
                  </el-radio-group>
                </div>
                ...
                <el-scrollbar class="check_b">
                  <div class="overflow" v-if="checkActiveItem=='briefSummary'">
                    <el-input v-if="checkActiveItem=='briefSummary'" class="combinename_text" type="textarea" resize="none" :autosize="true" v-model="conclusiontext"></el-input>
                  </div>
                  <div class="MarignTop combinename_text">
                    职业检查结论: &#x3000;<el-button type="primary" size="mini" @click="checkTheConclusionTitleFun">...</el-button>
                    <div>{{checkTheConclusionMsg}}</div>
                  </div>
                  <el-dialog title="职业检查结论" :visible.sync="checkTheConclusionVisible" width="25%">
                    <!-- @change="checkTheConclusionSelectFun"  选择回调 -->
                    <el-select ref="checkTheConclusionDom" v-model="checkTheConclusionMsg" placeholder="职业检查结论选择">
                      <el-option v-for="ite in checkTheConclusionData" :key="ite.code" :label="ite.name" :value="ite.name">
                      </el-option>
                    </el-select>
                    <el-button type="primary" size="mini" @click="checkTheConclusionVisible=false">确定</el-button>
                  </el-dialog>
                  <!-- 总检综述 dom1 describe   -->
                  <!-- <div v-if="checkActive == 'describe'" class="dom1">
                    <el-input class="combinename_text" type="textarea" resize="none" :autosize="true" v-model="conclusiontext"></el-input> -->
                  <!-- <div
                  ...
                  <!-- 以上打开需单个开启 -->
                  <!-- 历史对比 dom2 contrast   -->
                  <div v-if="checkActiveItem == 'contrast'" class="dom2">
                  ...
                  <!-- <div v-if="checkActive == 'depart'" class="dom3">
                    <EasyTable class="depart_table" height="100%" :data="departData" :tableOptions="departOptions" :radio-config="{ rowClick: true }" @row-click="handleDepartRow" :border="false" @selection-change="DialogSelectIntment" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }" />
              ...
              <div class="check_right">
                ...
                <div :class="['handle_box']" v-show="checkActiveItem != 'contrast'">
                  ...
                  <el-tabs v-model="activeName">
                    <el-tab-pane label="总检结论" name="zjjl">总检结论</el-tab-pane>
                    <el-tab-pane label="健康建议" name="jkjy">健康建议</el-tab-pane>
                  </el-tabs>

                  <div class="handle_btn_group">
                    <!-- 项目复查 - 旧逻辑 -->
                    <!-- <el-button type="primary" size="mini" style="width:30% ;position: relative;top: 7px !important;" @click="dialogProjectReviewFlag = true, Review()">项目复查</el-button> -->
                    <!-- 项目复查 -->
                    <el-dialog title="项目复查" :close-on-click-modal="false" :visible.sync="dialogProjectReviewFlag">
                      <!-- <div class="mask" v-show="dialogInsideFlag"></div> -->
                      <el-button type="primary" size="mini" @click="dialogInsideFlag = true, getProjectFun()">增加组合</el-button>
                      <!-- 添加项目组件 -->
                      <SetMealDialog v-if="dialogInsideFlag" :groupTableData="groupTableData" @submit="handleProjectSubmit" @close="InsideFlagClose" />

                      <el-table :data="DialogProjectReviewData" style="width: 100%">
                        <el-table-column prop="date" label="操作" width="180">
                          <template v-slot="param">
                            <el-button type="primary" icon="el-icon-delete" size="mini" @click="ProjectReviewDel(param.row)"></el-button>
                          </template>
                        </el-table-column>
                        <el-table-column prop="combinename" label="体检组合" width="180">
                        </el-table-column>
                        <el-table-column prop="remark" label="复查要求">
                          <template v-slot="param">
                            <el-input size="mini" v-model="param.row.remark"></el-input>
                          </template>
                        </el-table-column>
                      </el-table>
                      <div slot="footer" class="dialog-footer">
                        <el-button @click="dialogProjectReviewFlag = false">取 消</el-button>
                        <el-button type="primary" @click="dialogProjectReviewFlag = false, Setproject()">确 定</el-button>
                      </div>
                    </el-dialog>
                    ...
                    <div class="handle_btn">
                      <el-button type="primary" size="mini" @click="AddOpinionItems">新增小结</el-button>
                    </div>
                    <!-- <div class="handle_btn" @click="handleChangeIllnessstatus">
                      <el-tooltip class="item" effect="dark" content="疾病标志" placement="top">
                        <i class="el-icon-edit-outline"></i>
                      </el-tooltip>
                    </div>
                    <div class="handle_btn" @click="handleMoveConclusion">
                    ...
                    <div class="handle_btn" @click="handleMoveBottomping">
                ...
                <!-- 主体内容结构 -->
                <div class="check_b jkjyBom" ref="checkL">
                  <!-- 总检结论 -->
                  <div class="check_b_title" v-show="activeName == 'zjjl' && DiseaseFlag &&checkActiveItem != 'contrast'">
                    <div style="padding:10px 0; font-size:14px;">
                      <el-row v-if="zjjlData?.length > 0">
                        <el-col class="colTime" :span='3'>疾病或异常</el-col>
                        <el-col class="colTime" :span='4'>体检结论</el-col>
                        <el-col class="colTime" :span='3'>职业危害</el-col>
                        <el-col class="colTime" :span='3'>职业建议</el-col>
                        <el-col class="colTime" :span='3'>健康建议</el-col>
                        <el-col class="colTime" :span='3'>复查项目及要求</el-col>
                        <el-col class="colTime" :span='4'>检查组合</el-col>
                      </el-row>
                    </div>
                    <el-dialog title="职业危害选择" :close-on-click-modal="false" :visible.sync="HazardsVisible" width="30%">
                      <el-select @change="HazardsDialogFun" ref="HazardsSelectDOM" v-model="HazardsMsg.code" placeholder="职业危害选择">
                        <el-option v-for="ite in HazardsSelectData" :key="ite.code" :label="ite.name" :value="ite.code">
                        </el-option>
                      </el-select>
                    </el-dialog>
                    <el-dialog title="复查项目及要求" destroy-on-close :fullscreen="false" :visible.sync="TheProjectVisible" :close-on-click-modal='false' style=" height:95%;">
                      <el-col>
                        <div class="TheProjectTableClas">
                          <div class="TheProItem" v-for="item,index in commonlyUsedItem" :key="index" @click="combinationItemClick(item)">
                            <span class="index">{{index + 1}}、</span>
                            <div>{{item.reviewRequirement}}</div>
                          </div>
                        </div>
                      </el-col>
                      <el-col style="color:black;">
                        ...
                        <el-row style="margin:10px 0 ;">
                          <el-col :span="4" style="line-height:30px;">复检项目: </el-col>
                          <el-col :span="20" style="display:flex;"><el-input size="mini" v-model="retestItemMsg"></el-input>
                            <el-button type="primary" size="mini" @click="addTheProjectItem">...</el-button></el-col>
                        </el-row>
                        <el-row>
                          <el-col :span="4">
                            复查项目及要求:
                          ...
                          <el-col :span="20">
                            <el-input type="textarea" :rows="4" placeholder="请输入内容" v-model="textRequest">
                            </el-input>
                          </el-col>
                        </el-row>
                      </el-col>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="TheProjectDialogColse">取 消</el-button>
                        <el-button type="primary" @click="TheProjectDialogSubmit">确 定</el-button>
                      </span>
                      <!-- <el-select @change="TheProjectDialogFun" ref="TheProject" v-model="reviewTheProjectMsg" placeholder="项目复查选择">
                        <el-option v-for="ite in TheProjectSelectData" :key="ite.code" :label="ite.name" :value="ite.name">
                        </el-option>
                      </el-select> -->
                    </el-dialog>
                    <el-dialog title="复检项目" :close-on-press-escape='false' :show-close='false' :close-on-click-modal="false" destroy-on-close :visible.sync="InnerTheProjectDialog" width="45%">
                      <el-row style="margin:0 auto; width:80%;">
                        <el-transfer v-model="transferSelectData" @change="transferSelectFun" :titles="['未选', '已选']" :data="transferData" :props="transferprops"></el-transfer>
                      </el-row>
                      <span slot="footer" class="dialog-footer">
                        <!-- <el-button @click="InnerTheProjectDialog = false, TheProjectVisible = true">取 消</el-button> -->
                        <el-button type="primary" @click="InnerTheProjectDialog = false,InnerTheProjectDialogSubmit()">确 定</el-button>
                      </span>
                    </el-dialog>
                    <el-dialog title="职业选择" :close-on-click-modal="false" :visible.sync="SuggestionVisible" width="30%">
                      <el-select @change="SuggestionDialogFun" ref="SuggestionSelectDOM" v-model="SuggestionDialogMsg" placeholder="职业危害选择">
                        <el-option v-for="ite in SuggestionSelectData" :key="ite.code" :label="ite.name" :value="ite.name">
                        </el-option>
                      </el-select>
                    </el-dialog>
                    <!-- 总检结论主体 -->
                    <div class="marginButtom" v-for="item,index in zjjlData" :key="index">
                      <el-row>
                        <el-col class="colTime" :span='3'>
                          <!-- @input="diseaseFun(index,$event)" -->
                          <textarea class="border" ref="diseaseRef" rows="5" v-model="item.wordnameText"> </textarea>
                        </el-col>
                        <el-col class="colTime" :span='4'>
                          <!-- @input="conclusionFun(index,$event)" -->
                          <textarea class="border" ref="conclusionRef" rows="5" v-model="item.conclusion"></textarea>
                        </el-col>
                        <el-col class="colTime" :span='3'>
                          <div @click.stop="HazardsBoxFun(index)" class="border" v-html="item.harmname"></div>
                        </el-col>
                        <el-col class="colTime" :span='3'>
                          <div @click="SuggestionBoxFun(index)" class="border" v-html="item.occupationadvice"></div>
                        </el-col>
                        <!-- 健康建议 -->
                        <el-col class="colTime" :span='3'>
                          <!-- @input="healthAdviceFun(index,$event)"   -->
                          <textarea class="border" ref="healthAdviceRef" name="healthAdvice" rows="5" v-model="item.healthadvice"></textarea>
                        </el-col>
                        <!-- 复查项目及要求 -->
                        <el-col class="colTime" :span='3'>
                          <div @click="TheProjectBoxFun(index)" class="border" v-html="item.reviewexplain"></div>
                        </el-col>
                        <!-- 检验组合 -->
                        <el-col class="colTime" :span='4'>
                          <el-row>
                            <el-select @change="TheCombinationSelect(index)" size="mini" v-model="item.combinecode" placeholder="检查组合选择">
                              <el-option v-for="item in checkTheCombinationSelectData" :key="item.code" :label="item.name" :value="item.code">
                              </el-option>
                            </el-select>
                          </el-row>
                          <el-row>
                            <span>目标疾病</span>
                            <!-- DiseaseMsg -->
                            <el-cascader @change="handleDiseaseClick(item,index)" v-model="item.wordcode" :props="defaultProps" :options="DiseaseData" size="mini" :show-all-levels="false"></el-cascader>
                          </el-row>
                        </el-col>
                        <el-col class="colTime" :span='1'>
                          <el-button class="centerDelet" size='mini' type="danger" @click="zjjlDelet(item,index)">删除</el-button>
                        </el-col>
                      </el-row>
                    </div>
                  </div>
                  ...
                  <div v-for="(item, index) in worditems" :key="index">
                    <div class="check_b_title" v-if="item.wordname.length > 0 && activeName == 'jkjy'">
                      <el-tag type="danger" v-if="checkActive == 'describe' && item.level == 0">普通</el-tag>
                      <el-tag type="danger" v-if="checkActive == 'describe' && item.level == 1">进一步治疗</el-tag>
                      <el-tag type="danger" v-if="checkActive == 'describe' && item.level == 2">立马治疗</el-tag>
                    </div>
                    <!--    checkActive == 'describe' && -->
                    <el-checkbox-group v-if="item.wordname.length > 0 && activeName == 'jkjy'" class="dom1 item_drag" v-model="checkAdvise1">
                      <draggable v-model="item.wordname" chosenClass="chosen" :disabled="true" forceFallback="true" handle=".text_h_drag" :group="index" animation="500" @start="handleDragOnStart" @end="handleDragOnEnd">
                        <div class="div_drag" v-for="(val, ind) in item.wordname" :key="ind">
                          <div class="dra_t">{{ ind + 1 }}、</div>
                          <el-checkbox class="dom1_item" :label="val">
                            <!-- :class="val.conclusiontoword ? 'mb20' : ''" -->
                            <div class="dra_b">
                              <!-- <div class="dra_t">{{ ind + 1 }}、</div> -->
                              <div class="dra_cont">
                                <div class="item_top dom1_item_top">
                                  <div class="text text_h_drag">
                                    ...
                                    <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 8 }" resize="none" v-model="val.wordname"></el-input>
                                  </div>
                                </div>
                                <div class="item_top item_zhon dom1_item_top" v-if="val.conclusiontoword ||
                                                                    val.conclusiontoword == ''
                                                                    ">
                                  <div class="text text_h_drag">
                                    <!-- <el-input size="mini" v-model="item.wordname"></el-input> -->
                                    <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 8 }" resize="none" v-model="val.conclusiontoword"></el-input>
                                  </div>
                                </div>
                                <!-- <div class="line"></div> -->
                                <div class="item_body">
                                  <el-input type="textarea" :autosize="true" resize="none" v-model="val.healthadvice"></el-input>
                                </div>
                              </div>
                            </div>
                            <!-- <div
                                class="conclusiontoword"
                                v-if="val.conclusiontoword"
                              >
                                {{ val.conclusiontoword || "" }}
                              </div> -->
                          </el-checkbox>
                  ...
                  <!-- 饮食建议文本域 ysjy -->
                  <!-- <el-input v-if="activeName == 'ysjy' && dietadviceText != ''" class="ysjy" type="textarea" resize="none" v-model="dietadviceText"></el-input> -->
                  <!-- 运动建议文本域 ydjy -->
                  <!-- <el-input v-if="activeName == 'ydjy' && sportsadviceText != ''" class="ydjy" type="textarea" resize="none" v-model="sportsadviceText"></el-input> -->
                  <!-- 健康知识文本域 jkzs -->
                  <!-- <el-input v-if="activeName == 'jkzs' && healthknowledgeText != ''" class="jkzs" type="textarea" resize="none" v-model="healthknowledgeText"></el-input> -->
                  <!-- 历史对比 dom2 contrast   -->
                  <div v-if="checkActiveItem == 'contrast'" class="dom2">
                  ...
                  <!-- 科室检查 dom3 depart -->
                  <!-- <div v-if="checkActive == 'depart'" class="dom3">
                    <EasyTable class="departDt_table" height="100%" :data="departDtData" :tableOptions="departDtFlag === '3'
                                            ? departDtOptions1
                                            : departDtOptions
            ...
            <div class="check_form">
              <el-form ref="form" :disabled="currentPeople.updatestatus == 0" :model="currentPeople" label-width="80px">
    ...
    <el-dialog class="total_dialog" :visible.sync="showTotalCheck" width="40%" @close="handleTotalCheckClose">
      <div class="head_box">
        <div class="personalflag_box">
          <div class="time_title">类型:</div>
          <el-select style="width: 80px" v-model="personalflag" size="mini" clearable @change="handleGetDepartCheckList" placeholder="请选择">
        ...
        <el-radio-group @change="handleGetDepartCheckList" v-model="dialogActive" size="mini">
          ...
          <el-radio-button :label="2">已总检</el-radio-button>
        </el-radio-group>
      </div>
      <div class="table_box">
        ...
        <EasyTable v-if="dialogActive == 2" class="total_table" height="100%" :data="sureData" :tableOptions="sureOptions" :radio-config="{ rowClick: true }" @row-dblclick="handleCheckTotal" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }" />
      </div>
      <div class="tips">
        记录数：{{
                    dialogActive == 0
      ...
      <div style="text-align: center; margin-top: 15px">
        <el-button size="mini" type="primary" @click="handleAllCopy">全部返回</el-button>
        <el-button size="mini" @click="handleTotalCheckClose">关 闭</el-button>
      </div>
    </el-dialog>
    <!-- 新增结论词弹框 -->
    <el-dialog class="conc_dialog" :visible.sync="showConclusion" width="60%" :show-close="false" @open="handleAddConclusionOpen">
      <!-- 按钮组 -->
      <div class="btn_group">
      ...
      <el-row class="conc_body">
        ...
        <el-col class="conc_right" :span="17">
          ...
          <div class="conc_search_box">
            ...
            <el-input size="mini" v-model="concSearch" clearable ref="concSearchInput" placeholder="请输入搜索内容" @keyup.enter.native="handleGetConclusionsList" @input="handleChangeConclusionsList" @clear="handleGetConclusionsList"></el-input>
          </div>
          <!-- 结论词表格数据 -->
          <div class="conc_table_box">
            ...
            <EasyTable v-if="concActive == 'Person'" class="personal_table" height="100%" :data="personalData" :tableOptions="personalOptions" :radio-config="{ rowClick: true }" @row-click="handleChoosePsonConc" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }" />
          </div>
          <!-- 建议输入框文本域  -->
    ...
    <!-- 检索组件 -->
    <SearchDialog v-if="showSearch" :pageFlag="pageFlag" :showSearch="showSearch" @close="handleCloseSearch" @submit="handleSearchSubmit" />
    <!-- 取消审核原因弹框 -->
    ...
    <!-- 查看组合详情数据弹框 -->
    <el-dialog title="检查结果" class="comb_dialog" :visible.sync="dialogVisible" width="50%" top="2vh">
      <div class="comb_d_content">
        <div class="div_mess" v-if="currentPeople">
          <div class="div_mess_item">姓名：{{ currentPeople.name || "" }}</div>
          <div class="div_mess_item">
            性别：{{ currentPeople.sex || "未知" }}
          </div>
          <div class="div_mess_item">年龄：{{ currentPeople.age || "" }}</div>
          <div class="div_mess_item" v-if="chooseDepart">
            检查组合：{{ chooseDepart.combinename || "" }}
          </div>
        </div>
        <div v-if="chooseDepart" class="chooseDepart">
          ...
          <div class="summary-box">
            ...
            <div class="tinymce_box">
              <el-input class="conc_text" :rows="5" type="textarea" resize="none" disabled v-model="conclusion"></el-input>
            </div>
          </div>
        </div>
        <div class="div_mess" v-if="chooseDepart">
          <div class="div_mess_item">
            检查医生：{{ chooseDepart.doctorname || "" }}
          </div>
          <div class="div_mess_item">
            检查日期：{{ chooseDepart.inputdate || "" }}
          </div>
        </div>
              ...
              <div class="fruit_btn">
                <el-button-group class="nowrap">
                  <el-button
                    ...
                    plain
                    type="primary"
                    @click="(fruit = 'conclusion'), handleFruitChange()"
                    >小结</el-button
                  >
                  <el-button
                    size="mini"
                    plain
                    type="primary"
                    @click="(fruit = 'specialistadvice'), handleFruitChange()"
                    >专家建议</el-button
                  >
                </el-button-group>
                <el-link
    ...
    <!-- 个人常用语弹框 -->
    <el-dialog title="个人模板" class="summaryDia" top="5vh" :visible.sync="showCommonWords" width="800px" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <EasyTable :loading="loading" height="100%" ref="commonTable" class="table_100 move_conclusion_t" style="height: 400px" :data="commonWords" :tableOptions="commonWordsOptions" @checked-change="handleCheckCommon" :radio-config="{ fetch: true, selectedVal: setmealRadio }" :row-style="rowstyle" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }" row-id="dcdid" />
      <span slot="footer" class="dialog-footer" style="text">
        <el-button type="primary" size="mini" @click="handleAddSummarize">添加到综述</el-button>
        <el-button type="primary" size="mini" @click="handleAddPropose">添加到建议</el-button>
        <el-button type="primary" size="mini" @click="handleAddCommonWords">新 增</el-button>
        <el-button type="primary" size="mini" @click="handleEditCommonWords">编 辑</el-button>
        <el-button type="primary" size="mini" @click="handleSaveCommonWords">保 存</el-button>
        <el-button type="primary" size="mini" @click="handleCloseCommonWords">关 闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  GetdiseaseLoad,
  GetGeneralInspectionDetail,
  GetRegisterHarm,
  GetCombination,
  GetTheRecommendations,
  GetGeneralInspectionSave,
  GetdictionaryAll,
  CheckIn,
  GetDicCombineLoad,
  Load_Dic_evaluate_word,
} from "@/api/personalregister_zyb.js";
import SetMealDialog from "@/components/SetMealDialog/index_gyey.vue"; // 新项目弹框
import Sortable from "sortablejs";
export default {
  name: "personalregister",

  components: { Sticky, permissionBtn, SearchDialog, draggable, SetMealDialog },
  ...
  data() {
    return {
      ...
      timeCId: "",
      chooseCommonWords: undefined,
      setmealRadio: undefined,
      commonWords: [],
      ...
      tinymceFlag1: 1,
      tinymceFlag2: 1,
      checkActive: "beginning", // 左侧当前选中标签页 describe  contrast  depart  totalCheck  archives  beginning complex
      checkActiveItem: "briefSummary", //  初检 复检 下面的两个按钮  briefSummary=项目小结  checkTheResults = 检查结果    historyComparison = 历史对比
      checkText: "综述", // 综述 历史对比 科室检查 历史总检 健康档案
      activeName: "zjjl", // 右侧当前选中标签页
      ...
      meccode: "", //判断meccode(院区标识）判断为1，报告书报表调用‘高干报告书’模版
      showMoveConclusion: false, // 健康建议位置调整移动
      regId: "", // 登记ID 获取复查数据
      DialogSelectIntmentData: [], // 科室检查 选择项数组
      DialogProjectReviewData: [], // 项目复查 表格数据
      dialogProjectReviewFlag: false, //  项目复查Dialog 开关
      dialogInsideFlag: false, // dialog中的 套餐组合选择组件
      ...
      DiseaseFlag: false, // 总检结论开关
      DiseaseData: [], //  存放目标疾病下拉框数据
      DiseaseMsg: "", // 目标疾病展示字段

      HazardsSelectData: [], //下拉框
      HazardsMsg: { name: "", code: "" }, // 职业危害字段数据
      HazardsVisible: false, // dialog开关
      HazardsDialogMsg: "", //  dialog字段数据

      SuggestionMsg: "", //建议字段数据
      SuggestionVisible: false, //dialog开关
      SuggestionDialogMsg: "", // dialog 选中数据
      SuggestionSelectData: [], // 选择框数据

      reviewTheProjectMsg: "", //项目复查字段数据
      reviewTheProject: [], // 复检数据 个数判断次数
      TheProjectSelectData: [], //下拉框数据
      TheProjectVisible: false, // dialog 开关

      checkTheCombinationMsg: "", // 检查组合字段数据
      checkTheCombinationSelectData: [], // 检查组合下拉框
      olderRegId: "", // 旧的regid
      textRequest: "", // 项目及要求, 要求字段
      InnerTheProjectDialog: false, // 内层dialog的开关
      // transferValue: [], // ...按钮中 选择的数据
      transferData: [], //  ...按钮中(复检项目) 数据
      transferSelectData: [], // 选中的项目
      transferprops: {
        key: "code",
        label: "name",
        disabled: "disabledFlag",
      }, // 数据源别名
      retestItemMsg: "", // 选择完常用后  展示的字段
      commonlyUsedItem: [], // 复检项目及要求 常用数据
      userSignregId: "", // 签入签出regid
      checkTheConclusionVisible: false, // 职业检查结论
      checkTheConclusionMsg: "", // 保存字段
      checkTheConclusionData: [], // 职业检查结论数据
      olderName: "", // 复检项目及要求旧的 项目名
      olderRequire: "", // 旧的要求字段
    };
  },
  computed: {
            ...
            return (
              <el-link
                ...
                vOn:click_stop_prevent={() => {
                  this.handleDelCommonWords(data);
                }}
              >
                删 除
              </el-link>
    ...
    moveConclusionOptions() {
      return [
        ...
        },
        {
          ...
          width: 300,
          render: (h, data) => {
            return <div class="move_conclusion">{data.wordname}</div>;
          },
        },
        {
          title: "建议",
          render: (h, data) => {
            return <div class="move_conclusion">{data.healthadvice}</div>;
          },
        },
      ];
    },
    ...
    persOptions() {
      return [
        ...
        //         break;
        //       case 6:
        //         src = require("@/assets/images/可总检.png");
        //         break;
        //       case 7:
        ...
        },
        {
          ...
          render: (h, data) => {
            ...
            return (
              <div>
                <span v-show={data.status == 0} style="color:red;">
                  {Status}
                </span>
                <span v-show={data.status != 0}>{Status}</span>
              </div>
            );
          },
        },
      ...
      return [
        {
          ...
          render: (h, data) => {
            return (
              <div>
                <span style="color: red" v-show={data.status == 0}>
                  未完成
                </span>
                <span v-show={data.status == 1}>弃检</span>
                <span v-show={data.status != 1 && data.status != 0}>完成</span>
              </div>
            );
          },
        },
    ...
    sureDataSearch() {
      ...
      // 进入筛选
      if (search && search.length == 2) {
        return this.sureData.filter((data) => {
          if (data.regdate) {
            let condition1 = new Date(search[0]).getTime();
            let condition2 = new Date(search[1]).getTime();
            let returnData = new Date(data.regdate).getTime();
  ...
  mounted() {
    ...
    // };
    // 获取评价字典
    Load_Dic_evaluate_word({ tablename: "Dic_evaluate_word" }).then((res) => {
      console.log(res.data);
      this.checkTheConclusionData = res.data;
    });
  },
  destroyed() {
  ...
  methods: {
    // 疾病或异常输入回调
    diseaseFun(index, e) {
      this.zjjlData[index].wordnameText = e.target.innerHTML;
      this.refRange("healthAdviceRef", index);
    },
    // 体检结论输入回调
    conclusionFun(index, e) {
      this.zjjlData[index].conclusion = e.target.innerHTML;
      this.refRange("healthAdviceRef", index);
    },
    // 健康建议输入回调
    healthAdviceFun(index, e) {
      this.zjjlData[index].healthadvice = e.target.innerHTML;
      // 将光标定位至末尾
      this.refRange("healthAdviceRef", index);
    },
    refRange(Refname, index) {
      // 将光标定位至末尾
      let div = this.$refs[Refname][index];
    ...
    // 新增小结
    AddOpinionItems() {
      console.log("新增小结", this.userSignregId, this.zjjlData);
      this.zjjlData.push({
        combinecode: "",
        combinename: "",
        conclusion: "",
    ...
    // 职业检查结论回调
    checkTheConclusionTitleFun() {
      this.checkTheConclusionVisible = true;
    },
    // 签入签出
    userSign(type) {
      console.log(this.userSignregId);
      CheckIn({ enumCheck: type, regid: this.userSignregId }).then((res) => {
        console.log(res);
        if (res.code == 200 && res.result) {
          this.$message.success("操作成功");
          this.handleSearchTotalCheck();
        } else {
    ...
    // 增加组合关闭
    InsideFlagClose() {
      this.dialogInsideFlag = false;
    },
    // 点击职业危害显示下拉框
    HazardsBoxFun(index) {
      this.DataIndex = index;
      this.HazardsVisible = true;
      GetRegisterHarm({ regid: this.olderRegId }).then((res) => {
        console.log(res.result, "获取职业因素");
        this.HazardsSelectData = res.result;
      });
    },
    // 下拉框确定回调
    HazardsDialogFun() {
      this.zjjlData[this.DataIndex].harmcode = this.HazardsMsg.code;
      this.HazardsSelectData.forEach((k) => {
        k.code == this.HazardsMsg.code &&
          (this.zjjlData[this.DataIndex].harmname = k.name);
      });
      this.HazardsVisible = false;
      this.HazardsMsg.code = "";
      this.HazardsMsg.name = "";
      console.log(this.zjjlData[this.DataIndex], this.DataIndex);
    },
    // 点击 职业建议
    SuggestionBoxFun(index) {
      this.DataIndex = index;
      this.SuggestionVisible = true;
    },
    // 职业建议
    SuggestionDialogFun() {
      this.zjjlData[this.DataIndex].occupationadvice = this.SuggestionDialogMsg;
      this.SuggestionVisible = false;
      this.SuggestionDialogMsg = "";
      console.log(this.zjjlData[this.DataIndex]);
    },

    // 复检项目 ... 操作
    addTheProjectItem() {
      this.InnerTheProjectDialog = true;
      this.TheProjectVisible = false;
      let arr = JSON.parse(JSON.stringify(this.transferSelectData));
      this.transferSelectData = [];
      // console.log(this.transferSelectData, arr, "看看开始是什么");
      this.transferSelectData = arr.map((k) => k.code || k);
      // this.transferSelectData = arr;
      // console.log(this.transferSelectData, arr, "已选数据");
      this.getDicCombineFun("0");
    },
    // 获取组合字典数据
    getDicCombineFun(type) {
      GetDicCombineLoad({
        page: 1,
        limit: 50,
        key: "",
        ...
        if (type == 0) {
          ...
          this.commonlyUsedItem = res.data;
        }
    ...
    // 点击 复检项目及要求-- 外层组合项
    combinationItemClick(row) {
      console.log(row);
      this.transferSelectData = [row]; // 选中数据
      this.zjjlData[this.DataIndex].customerReviewItem.item = [row]; // 两个数组同时操作
      this.retestItemMsg = `${row.name}、`; //项目名
      this.textRequest = row.reviewRequirement; // 要求
    },
    // 复查项目及要求---> dialog 取消按钮回调
    TheProjectDialogColse() {
      this.TheProjectVisible = false;
      this.retestItemMsg = this.olderName; // 项目名字
      this.textRequest = this.olderRequire; // 要求
      this.zjjlData[this.DataIndex].reviewItem = [];
      this.transferSelectData.forEach((k) => {
        this.zjjlData[this.DataIndex].reviewItem.push({
          code: k.code ? k.code : k,
          name: this.transferData.filter((u) => u.code == k.code || k)[0].name,
          remark: this.textRequest,
        });
      });
      // 完成操作后把数组重新赋值给已选数据，方便下次进入时操作
      this.transferSelectData = this.zjjlData[this.DataIndex].reviewItem;
      this.zjjlData[this.DataIndex].customerReviewItem.item =
        this.transferSelectData;
      // console.log(this.olderRequire, this.olderName);
    },
    // 复查项目及要求---> dialog 确定按钮回调
    TheProjectDialogSubmit() {
      this.TheProjectVisible = false;
      if (!!this.retestItemMsg) {
        // console.log("进入判断中", this.retestItemMsg, this.transferSelectData);
        this.zjjlData[this.DataIndex].reviewItem = [];
        let nameArr = [];
        this.transferSelectData.forEach((k) => {
          nameArr = this.transferData.filter((u) => {
            if (!!k.code) {
              if (u.code == k.code) {
                return u;
              }
        ...
        console.log(this.zjjlData[this.DataIndex].reviewItem);
        // 完成操作后把数组重新赋值给已选数据，方便下次进入时操作
        this.transferSelectData = this.zjjlData[this.DataIndex].reviewItem;
        this.zjjlData[this.DataIndex].customerReviewItem.item =
          this.transferSelectData;
        ...
        //   "完成操作后把数组重新赋值给已选数据"
        // );
        this.zjjlData[this.DataIndex].reviewexplain = this.textRequest;
        this.zjjlData[this.DataIndex].customerReviewItem.showName =
          this.retestItemMsg;
        this.zjjlData[this.DataIndex].customerReviewItem.remark =
          this.textRequest;
      } else {
        console.log("没有选数据");
      }
    },
    // 复查项目及要求---> 内层dialog 选择回调
    transferSelectFun(row) {
      console.log(row, "去重");
      // 去重
      this.zjjlData[this.DataIndex].customerReviewItem.item = Array.from(
        new Set(row)
      );
      this.transferSelectData =
        this.zjjlData[this.DataIndex].customerReviewItem.item;
    },
    // 复查项目及要求---> 内层dialog 确定按钮回调
    InnerTheProjectDialogSubmit() {
      // console.log("// 内层dialog 确定按钮回调", this.transferSelectData);
      this.InnerTheProjectDialog = false;
      this.zjjlData[this.DataIndex].customerReviewItem.showName = "";
      // 查重后 匹配相同code 添加到数组中
      const resultArr = [];
      this.transferSelectData.forEach((k) => {
        resultArr.push(this.transferData.filter((u) => u.code == k)[0]);
      });
      // 拼接字符串组合名 -- 去除假值
      resultArr.forEach((k) => {
        // console.log("字符串拼接", k);
        this.zjjlData[
          this.DataIndex
        ].customerReviewItem.showName += `${k.name}、`;
      });
      this.retestItemMsg =
        this.zjjlData[this.DataIndex].customerReviewItem.showName;
      // 根据code找到相同数据 添加到原数组中,下次打开不会出错
      let transferArr = [];
      this.transferData.forEach((u) => {
        !!this.transferSelectData.filter((k) => k == u.code)[0] &&
          transferArr.push(
            this.transferSelectData.filter((k) => k == u.code)[0]
          );
      });
      this.zjjlData[this.DataIndex].customerReviewItem.item = transferArr;
      console.log(this.zjjlData[this.DataIndex].customerReviewItem.showName);
      this.$nextTick(() => {
        this.TheProjectVisible = true;
      });
    },
    // 点击 复查项目
    TheProjectBoxFun(index) {
      console.log(
        this.zjjlData[index],
        "当前点击的数据=>",
        this.zjjlData[index].customerReviewItem,
        "已选择的数据=>",
        this.transferSelectData
      );
      this.TheProjectVisible = true;
      this.DataIndex = index;
      this.transferSelectData = this.zjjlData[
        index
      ].customerReviewItem.item.map((k) => k); // 已选数组 code
      this.retestItemMsg = this.zjjlData[index].customerReviewItem.showName; // 组合名字
      this.textRequest = this.zjjlData[index].customerReviewItem.remark; // 要求
      this.olderName = this.zjjlData[index].customerReviewItem.showName; // 组合名字 -- dialog点击取消时 用此数据赋值
      this.olderRequire = this.zjjlData[index].customerReviewItem.remark; // 要求 -- dialog点击取消时 用此数据赋值
      // 打开dialog同时请求数据
      this.getDicCombineFun("1");
      this.getDicCombineFun("0");
    },
    // 复查项目下拉框
    TheProjectDialogFun() {
      // this.TheProjectSelectData  存放数据的
      this.zjjlData[this.DataIndex].reviewexplain = this.reviewTheProjectMsg;
      this.TheProjectVisible = false;
      this.reviewTheProjectMsg = "";
      console.log(this.zjjlData[this.DataIndex]);
    },
    // 组合下拉框回调
    TheCombinationSelect(index) {
      this.checkTheCombinationSelectData.forEach((item) => {
        item.code == this.zjjlData[index].combinecode &&
          (this.zjjlData[index].combinename = item.name);
      });
      console.log(this.zjjlData[index]);
    },

    //  目标疾病级联控件回调
    handleDiseaseClick(item, index) {
      //   console.log(index, this.DiseaseMsg,this.zjjlData[index], "目标疾病级联控件回调");
      this.zjjlData[index].wordcode = item.wordcode[1]; // 获取code
      this.DiseaseData.forEach((u) => {
        console.log(u);
        u.childs.forEach((k) => {
          console.log(k);
          k.code == item.wordcode && (item.wordname = k.name);
        });
      });
      // console.log(item.wordcode,item.wordname);
    },
    // 右侧总检结论 删除回调
    zjjlDelet(data, index) {
    ...
    handleProjectSubmit(data) {
      this.dialogInsideFlag = false;
      this.DialogProjectReviewData = data;
      console.log(this.DialogProjectReviewData, "新增项目提交按钮");
    },
    // 点击增加组合 按钮回调
    getProjectFun() {},
    // 项目复查 确定按钮回调  保存请求
    Setproject() {
      let arr = [];
      this.DialogProjectReviewData.forEach((item) => {
        item = {
      ...
      console.log(arr, "this.DialogProjectReviewData");
      SetReviewSet(arr).then((res) => {
        console.log(res);
        this.DialogProjectReviewData = [];
        if (res.code == 200) {
    ...
    // 点击项目复查按钮回调
    Review() {
      // this.DialogProjectReviewData = this.DialogSelectIntmentData
    },
    // 复查项目 点击删除回调
    ProjectReviewDel(row) {
      console.log(row, this.DialogProjectReviewData, "点击删除的数据");
      let result = this.DialogProjectReviewData.findIndex(
        (item) => item.combinecode == row.combinecode
      );
      this.DialogProjectReviewData.splice(result, 1);
      console.log(result, "找到删除数据");
    },
    // 复查项目选择项
    DialogSelectIntment(data) {
      this.DialogSelectIntmentData = data;
      console.log(this.DialogSelectIntmentData, "选择的数据");
    },
    // 修改疾病标志状态
    handleChangeIllnessstatus() {
      if (this.checkAdvise1.length == 0) {
        this.$message.warning("请先勾选操作数据!");
        return;
      }
      this.checkAdvise1.forEach((k) => {
        k.illnessstatus = 1;
      });
      this.$message({
        type: "success",
    ...
    // 添加到总检综述回调
    handleAddSummarize() {
      if (!this.chooseCommonWords) {
        this.$message.warning("请先选中需要操作的数据!");
        return;
      }
      if (this.chooseCommonWords && this.chooseCommonWords.dcdid == "") {
        this.$message.warning("请先保存当前选中数据,再进行操作!");
        return;
      }

      if (this.conclusiontext == "") {
        this.conclusiontext += this.chooseCommonWords.describe;
      } else {
        this.conclusiontext += `\n　${this.chooseCommonWords.describe}`;
      }
      this.handleCloseCommonWords();
    },
    // 添加到健康建议回调
    handleAddPropose() {
      if (!this.chooseCommonWords) {
      ...
      let tempArr = {
        ...
        combinename: "",
        wordname: "",
        healthadvice: this.chooseCommonWords.describe,
        specialistadvice: "",
        dietadvice: "",
        sportsadvice: "",
        healthknowledge: "",
      ...
      if (this.worditems.length == 0) {
        ...
        this.worditems.push(data);
      } else {
        this.worditems[0].wordname.push(tempArr);
      }
      this.handleCloseCommonWords();
    },
    // 个人模板列表弹框关闭
    handleCloseCommonWords() {
      this.setmealRadio = "";
      this.chooseCommonWords = undefined;
      this.commonWords = [];
    ...
    // 个人模板列表删除
    handleDelCommonWords(data) {
      if (data.dcdid == "") {
        this.commonWords.splice(data.$index, 1);
        this.$message.success("操作成功!");
      } else {
        commonWordsDelete([data.dcdid]).then((res) => {
          this.$message.success(res.message);
          this.handleCommonWordsLoad();
        });
      }
    },
    // 个人模板列表数据回调
    handleCommonWordsLoad(setmealRadio) {
      commonWordsLoad({
        page: 1,
        limit: 10000,
        whereitems: [],
        ...
        if (!res.data) {
          ...
          this.commonWords = res.data.map((k) => {
            return {
              ...k,
              isDisabled: true,
            };
          });
          console.log(setmealRadio, "setmealRadio");
          if (setmealRadio) {
            this.setmealRadio = setmealRadio;
            // let index = this.commonWords.findIndex(
            //   (k) => k.dcdid === setmealRadio
            // );
            // console.log(index, "index");
            // index != -1 && (this.chooseCommonWords = this.commonWords[index]);
            // this.$nextTick(() => {
            //   this.$refs.commonTable.$refs.easyTable.toggleRowSelection(
            //     this.commonWords[index],
            //     true
            //   );
            // });
          }
    ...
    handleAllCopy() {
      if (this.dialogActive == 0) {
        this.persData = this.waitData;
      } else if (this.dialogActive == 1) {
        this.persData = this.totalData;
      } else {
        this.persData = this.sureData;
      }
      this.showTotalCheck = false;
    },
    // 个人模板新增保存数据
    handleSaveCommonWords() {
      console.log(this.chooseCommonWords, "this.chooseCommonWords");
      if (!this.chooseCommonWords) {
        this.$message.warning("请先选中需要保存的数据!");
        return;
      }
      commonWordsCreateOrUpdate(this.chooseCommonWords).then(async (res) => {
        this.$message.success(res.message);
        // this.chooseCommonWords = undefined;
        console.log(res.result, "res.result");
        await this.handleCommonWordsLoad(res.result);
      });
    },
    // 个人模板选中数据
    handleCheckCommon(row) {
      if (!row) return;
      this.chooseCommonWords = row;
      this.setmealRadio = row.dcdid;
    },
    // 编辑选中个人模板
    handleEditCommonWords() {
      if (!this.chooseCommonWords) {
        this.$message.warning("请先选中需要编辑的数据!");
        return;
      }
      this.chooseCommonWords.isDisabled = false;
    },
    // 新增个人模板
    handleAddCommonWords() {
      if (
        this.commonWords.length > 0 &&
        this.commonWords[this.commonWords.length - 1].dcdid == ""
      ) {
        this.$message.warning("请先保存上一新增数据!");
        return;
      }
      this.commonWords.push({
        dcdid: "00000",
        type: "conclusion",
        dsscribe: "",
        isDisabled: false,
    ...
    // 调整弹框数据勾选
    handleSelectMoveDia(arr) {
      this.checkAdvise1 = arr;
    },
    // 打开移动健康建议弹框
      ...
      switch (domId) {
        case "btnReport": // 报告书
          this.$refs.popoverRefIdc.forEach((d) => {
            console.log(this.currentPeople.checktype, d);
            if (d.$children[0].$el.innerText == "报告书") {
              let indexP =
                d.$children[0].$attrs["data-remark"].split("、")[0] + "、";
              let repname = "";
              if (
                this.currentPeople.checktype == 1 ||
                this.currentPeople.checktype == 3 ||
                this.currentPeople.checktype == 8
              ) {
                repname = `${indexP}Fvip报告书`;
              } else if (
                this.currentPeople.checktype == 2 ||
                this.currentPeople.checktype == 7
              ) {
                repname = `${indexP}F报告书`;
              } else if (this.currentPeople.checktype == 6) {
                repname = `${indexP}F健康教师报告书`;
              } else if (this.currentPeople.checktype == 11) {
                repname = `${indexP}F职业体检表`;
              } else if (this.currentPeople.checktype == 12) {
                repname = `${indexP}F放射体检表`;
              } else {
    ...
    handleSetPrintOrderDetail() {
      setPrintOrderDetail({
        ...
        if (!res.result) return;
        // this.checkupWork = "";
        this.combData = res.result.filter((k) => {
          return k.dedflag != 1;
        });
      });
    },
    ...
    handleGetRegisterInfo() {
      ...
      // 6600008819
      // console.log(this.currentPeople.checkno, this.currentPeople.idcard);
      if (this.currentPeople.idcard) {
        data.idcard = this.currentPeople.idcard;
      } else {
        data.checkno = this.currentPeople.checkno;
      }
      // getRegisterInfo({checkno:''}).then((res) => {
      getRegisterInfo(data).then((res) => {
        if (!res.result) return;
        // this.isloading = false;
        // this.persData = res.result.reverse();
        this.DiseaseFlag = true; // 总检结论
        if (this.choosePers && this.choosePers.regid && this.isloading) {
          let flag = res.result.filter((k) => {
            return k.regid == this.choosePers.regid;
          })[0];
          ...
          if (flag && index > -1) {
            this.$nextTick(() => {
              // this.persData[index] = flag;
              this.persData.splice(index, 1, flag);
              this.choosePers = flag;
            });
          }
        } else {
          this.persData = res.result.reverse();
          if (this.currentPeople.regid) {
            this.choosePers = this.currentPeople;
          } else {
    ...
    handleChangeConclusionsList() {
      clearTimeout(this.timeCId);
      this.timeCId = setTimeout(() => {
    ...
    handleGetToday() {
      let today = new Date(+new Date() + 8 * 3600 * 1000)
        ...
        .substr(0, 10)
        .replace("T", " ");
      let lastTime = this.recentTime(-10, "yyyy-MM-dd", today);
      this.triageTime = [`${lastTime} 00:00:00`, `${today} 23:59:59`];
    },
    ...
    handleSetGenerateAgain() {
      ...
      setTotalCheckDelete({ regid: this.checkupWork }).then((res) => {
        this.$message.success(res.message);
        this.checkupWork = this.currentPeople.regid;
        this.handleGetTotalCheck();
      });
    ...
    handlegetgetPersonArchives() {
      console.log(this.currentPeople); // 获取当前档案详情数据
      console.log(
        this.currentPeople,
        "获取当前健康档案回调数据",
        "档案健康数据"
    ...
    // 体检总检审核接口回调
    async handleGetCheck() {
      if (!this.currentPeople.regid) {
        this.$message.warning("请先提供审核人员信息!");
        return false;
      }
      if (this.currentPeople.status >= 5 && this.currentPeople.status < 7) {
    ...
    handleWordMerge() {
      ...
      this.worditems[findex].wordname.push(temp);
    ...
    handleMoveBottomping() {
      ...
      this.worditems.forEach((k, i) => {
        for (let i = 0; i < k.wordname.length; i++) {
          if (this.checkAdvise1[0].uniqueid == k.wordname[i].uniqueid) {
            if (i == k.wordname.length - 1) {
              this.$message.warning("已移动到底部!");
              return false;
            } else {
              k.wordname.splice(i, 1);
              k.wordname.push(this.checkAdvise1[0]);
              break;
            }
          }
        }
    ...
    handleMoveTopping() {
      ...
      this.worditems.forEach((k, i) => {
        k.wordname.forEach((item, index) => {
          if (this.checkAdvise1[0].uniqueid == item.uniqueid) {
            if (index == 0) {
        ...
        k.wordname.forEach((item, index) => {
          if (this.checkAdvise1[0].uniqueid == item.uniqueid) {
            if (index == 0) {
              ...
              const key = index;
              const a1 = k.wordname[key];
              const a2 = k.wordname[key - 1];
              k.wordname.splice(key, 1, a2);
              k.wordname.splice(key - 1, 1, a1);
            }
          }
        });
      });
    ...
    handleSetTotalCheckSave() {
      // 暂时为空 健康建议111
      this.zjjlData.forEach((k) => {
        !k.evaluatecode && (k.evaluatecode = "0");
        !k.evaluatename && (k.evaluatename = "评价名称");
        !k.occupationadvice && (k.occupationadvice = "无");
        !k.healthadvice && (k.healthadvice = "无");
        !k.diagnosis && (k.diagnosis = "无");
        !k.combinecode && (k.combinecode = "0");
        !k.combinename && (k.combinename = "无");
        !k.conclusion && (k.conclusion = "无");
        !k.wordcode && (k.wordcode = "0");
        !k.wordname && (k.wordname = "无");
        k.regid = this.currentPeople.regid;
      });
        ...
        this.worditems.forEach((k) => {
          k.wordname.forEach((item) => {
            worditems.push({
              ...
              combinecode: k.combinecode,
              combinename: k.combinename,
              conclusiontoword: k.conclusiontoword,
              level: k.level,
              regid: k.regid,
              ...item,
            });
      ...
      let data = undefined;
      if (occupationitems.length == 0) {
        this.zjjlData.regid = this.currentPeople.regid;
        data = {
          regid: this.currentPeople.regid,
          doctorcode: this.currentPeople.doctorcode,
          doctorname: this.currentPeople.doctorname,
          ...
          conclusionOpinion: this.zjjlData,
        };
      } else {
        this.zjjlData.regid = this.currentPeople.regid;
        data = {
    ...
    handleSearchTotalCheck() {
      console.log(this.checkupWork, "this.checkupWork");

      //   获取组合字典
      GetCombination().then((res) => {
        // console.log(res, "获取组合字典");
        this.checkTheCombinationSelectData = res.data;
      });
      // 获取目标疾病数据
      GetdiseaseLoad({ page: 1, limit: 10000 }).then((res) => {
        // console.log(res, "获取目标疾病数据");
        if (res.code == 200) {
          this.DiseaseData = res.data;
        }
      });
      // 获取职业建议字典
      GetdictionaryAll({ type: "7" }).then((res) => {
        // console.log(res, "获取职业建议字典");
        if (res.code == 200) {
          this.SuggestionSelectData = res.data;
        }
      });
      //   获取复查建议字典
      GetTheRecommendations().then((res) => {
        // console.log(res, "复查项目");
        this.TheProjectSelectData = res.data;
      });

      if (!this.checkupWork) return;
      this.isloading = true;
      this.choosePers = undefined;
    ...
    handleGetTotalCheck() {
      console.log(this.checkupWork, "this.checkupWork");
      this.userSignregId = this.checkupWork;
      let whereitems = [
        {
          key: this.conditionCode,
          value: this.checkupWork.trim(),
      ...
      this.olderRegId = this.checkupWork.trim(); // 保存旧的regid
      if (this.currentPeople.regid) {
        whereitems.push({
          key: "oldregid",
      ...
      GetGeneralInspectionDetail({ page: 1, limit: 100000, whereitems }).then(
        (res) => {
          console.log(res.result, "获取体检总检");
          this.conclusiontext = res.result.conclusiontext; // 项目小结
          this.reviewTheProject = res.result.registerReview; // 复检记录
          let msg = "";
          //   在这里把健康建议拿到给每一个总检结论赋值, 后续可能会改
          res.result.worditems.forEach((k) => {
            k.wordname.forEach((u) => {
              msg += u.healthadvice;
            });
          });

          this.zjjlData = res.result.opinionItems; //总检结论
          this.zjjlData.forEach((k) => {
            !k.healthadvice && (k.healthadvice = msg);
          });
          // 判断meccode(院区标识）判断为1，报告书报表调用‘高干报告书’模版
          this.meccode = res.result.meccode;
          let data = res.result;
          ...
          // this.choosePers = data;
          this.disabledIsused = false;
          this.conclusiontext = data.conclusiontext;
          localStorage.setItem("ChangeUsedRegid", data.regid);
          if (data.status < 3) {
          ...
          this.checktypePrint = (checktype && checktype.code) || "";
          ...
          if (data.occupationitems) {
            ...
            index != -1 &&
              (this.checkRadio = this.occupationChecks[index].code);
          }
      ...
      this.condition = "体检号";
      this.conditionCode = "regid";
      this.checkActive = "beginning";
      this.checkActiveItem = "briefSummary";
    },
    // 结论词弹框确认按钮添加数据回调
    ...
    // 打开新增结论词弹框回调
    handleAddConclusionOpen() {
      this.$nextTick(() => {
        this.$refs.concSearchInput.focus();
      });
    },
    // 关闭结论词弹出层回调
    ...
    handleTabsItemChange() {
      // this.checkActiveItem    // 按钮存值
      console.log(this.checkActiveItem, "按钮存值");
      // briefSummary checkTheResults contrast
      // 项目小结   检查结果   历史对比
      if (this.checkActiveItem != "describe") {
      ...
      switch (this.checkActiveItem) {
        case "describe": // 综述
          this.checkText = "综述";
          break;
        case "briefSummary": // 项目小结
          this.checkText = "项目小结";
          // 调用项目小结相关接口
          break;
        case "briefcheckTheResults": // 检查结果
          this.checkText = "检查结果";
          // 调用检查结果相关接口
          break;
        case "contrast": // 历史对比
          this.checkText = "历史对比";
          this.contrastResult = ""; // 先清空 文本与描述
    ...
    // 左侧主体内容标签页切换回调
    handleTabsChange() {
      console.log(`第${this.checkActive + 1}次复查`);
      this.checkActiveItem = "briefSummary";
      // 判断是否职业病
      ...
      if (this.checkActive == "beginning") {
        this.getreview(this.olderRegId);
      }
      if (typeof this.checkActive === "number") {
        console.log(
          this.reviewTheProject[this.checkActive].regid,
          "拿到当前复检的regid来获取数据"
        );
        this.choosePers.regid = this.reviewTheProject[this.checkActive].regid; // 保留当前regid  获取detail用
        this.getreview(this.reviewTheProject[this.checkActive].regid);
      }
      // switch (this.checkActive) {
      //   case "describe": // 新增
      //     this.checkText = "综述";
    ...
    getreview(regID) {
      ...
      GetGeneralInspectionDetail({ page: 1, limit: 100000, whereitems }).then(
        (res) => {
          console.log(res, "复查数据");

          console.log(res.result, "获取次数体检总检");
          this.conclusiontext = res.result.conclusiontext; // 项目小结
          // this.reviewTheProject = res.result.registerReview; // 复检记录
          let msg = "";
          //   在这里把健康建议拿到给每一个总检结论赋值, 后续可能会改
    ...
    handlePrintReport() {
      this.$refs.popoverRefIdc.forEach((d) => {
        if (d.$children[0].$el.innerText == "打印、预览") {
          let indexP =
            d.$children[0].$attrs["data-remark"].split("、")[0] + "、";
          let newrepname = "";
          if (
            this.currentPeople.meccode == 0 &&
            this.currentPeople.companytype.toLowerCase().indexOf("vip") > -1
          ) {
            if (this.currentPeople.checktype == "1") {
              newrepname = `${indexP}报告书`;
            } else if (this.currentPeople.checktype == "2") {
              newrepname = `${indexP}入职报告书`;
            } else {
              newrepname = `${indexP}团体报告书`;
            }
          } else if (this._data.meccode == "1") {
            newrepname = `${indexP}高干报告书`;
          } else {
            if (this.currentPeople.personalflag == 1) {
              newrepname = `${indexP}团体报告书`;
            } else {
              if (this.checktypePrint == 2) {
                newrepname = `${indexP}入职报告书`;
              } else {
                newrepname = d.$children[0].$attrs["data-remark"];
              }
            }
          }
          let data = {
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnReport": // 导出报告书
          ...
          if (this.currentPeople.status >= 6) {
  ...
  beforeRouteEnter(to, form, next) {
    next((vm) => {
      ...
      if (to.query.regid) {
        ...
        vm.choosePers = {};
        vm.choosePers.regid = to.query.regid;
        vm.isloading = true;
...
@import "./css/generalinspection.scss";
