<template>
  <div class="company_register">
    <div v-if="!showAddOrEdit" class="filter-box">
      <div class="filter-container">
        <div class="filter-container-in">
          预约日期：　<el-date-picker v-model="queryDate" size="mini" type="daterange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd" class="daterange">
          </el-date-picker>
          <el-input @keyup.enter.native="handleFilter" size="mini" style="width: 300px; margin-right: 20px" class="filter-item" :placeholder="'名称'" v-model="searchWord"> </el-input>
      ...
      <div class="app-container">
        <div class="filter-container-btn">
          <permission-btn moduleName="company_register" ref="companyregister" size="mini" type="primary" plain v-on:btn-event="onBtnClicked"></permission-btn>
        </div>
        <div class="bg-white" style="height: calc(100vh - 210px)">
          <el-table ref="mainTable" min-width="500" height="100%" stripe :row-style="{ height: '32px' }" :cell-style="{ padding: '0px' }" :header-cell-style="{
              ...
              color: '#18253a',
              height: '40px'
            }" :key="tableKey" :data="list" v-loading="listLoading" fit highlight-current-row style="width: 100%" @row-click="rowClick" @selection-change="handleSelectionChange">
            <el-table-column type="selection" align="center" width="55"></el-table-column>
            <el-table-column prop="companycode" label="单位编码" width="100px" show-overflow-tooltip></el-table-column>
            <el-table-column width="200px" align="left" prop="companyname" label="单位名称" show-overflow-tooltip>
            ...
            <el-table-column prop="total" label="体检人数" show-overflow-tooltip></el-table-column>
            <el-table-column prop="realcount" label="登记人数" show-overflow-tooltip></el-table-column>
            <el-table-column width="180" prop="startdate" label="开始时间" show-overflow-tooltip></el-table-column>
            <el-table-column width="180" prop="enddate" label="结束时间" show-overflow-tooltip></el-table-column>
            <el-table-column prop="registername" label="创建人" show-overflow-tooltip></el-table-column>
            <el-table-column prop="receptionname" label="接待人" show-overflow-tooltip></el-table-column>
            ...
            <el-table-column prop="status" label="结算状态" show-overflow-tooltip>
              <template slot-scope="scope">
                ...
                <span v-if="scope.row.status == 1">体检中</span>
                <span v-if="scope.row.status == 2">已完成</span>
                <span v-if="scope.row.status == 3">完成结算</span>
                <span v-if="scope.row.status == 4">取消结算</span>
              </template>
            </el-table-column>
            <el-table-column prop="action" label="操作" show-overflow-tooltip>
              <template slot-scope="scope">
                <span class="link-type" @click="handleView(scope.row)">预约视图</span>
              </template>
            </el-table-column>
          </el-table>
          <pagination v-if="total > 0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="handleCurrentChange" />
        </div>
        <el-dialog v-el-drag-dialog class="dialog-mini" max-width="1500px" top="50px" :close-on-click-modal="false" :title="textMap[dialogStatus]" :visible.sync="adddialogFormVisible" @close="close">
          <el-form ref="dataForm" :rules="rules" :model="addtemp" label-position="right" label-width="130px">
            <sticky :className="'sub-navbar'">
              <el-button size="mini" type="primary" @click="chilopen">添加分组</el-button>
              <el-button size="mini" type="warning" @click="chilupd">修改分组</el-button>
              <el-button size="mini" type="danger" @click="childel">删除分组</el-button>
            </sticky>
            <el-tag type="info" style="margin-left: 70px" effect="dark"> 基本信息 </el-tag>
            <br />
            <br />
            <el-row>
              <el-col :span="6">
                <el-form-item size="small" :label="'单位名称'" align="left" prop="Companycode">
                  <el-select v-model="addtemp.Companycode" @change="companycurrentSel($event)">
                    <el-option v-for="(item, index) in result.companyItems" :key="index" :value="item.id" :label="item.companyname"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'批次'" prop="Batch">
                  <el-input v-model="addtemp.Batch" disabled="disabled"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'体检时间'" prop="Startdate">
                  <el-date-picker v-model="addtemp.Startdate" type="date" placeholder="选择日期"> </el-date-picker>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'至'" prop="Enddate">
                  <el-date-picker v-model="addtemp.Enddate" type="date" placeholder="选择日期"> </el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item size="small" :label="'单位联系人'" prop="Contacts">
              ...
              <el-col :span="6">
                <el-form-item size="small" :label="'体检地点'" prop="Meccode">
                  <el-select v-model="addtemp.Meccode">
                    <el-option v-for="(item, index) in result.centers" :key="index" :value="item.id" :label="item.name"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'接待人员'" prop="Receptioncode">
                  <el-select v-model="addtemp.Receptioncode" @change="receptionnamecurrentSel($event)">
                    <el-option v-for="(item, index) in result.users" :key="index" :value="item.account" :label="item.name"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'预约时间'" prop="Registerdate">
                  <el-date-picker v-model="addtemp.Registerdate" type="date" placeholder="选择日期"> </el-date-picker>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'注册机构'" prop="Hospitalcode">
                  <el-select v-model="addtemp.Hospitalcode">
                    <el-option v-for="(item, index) in centerarr" :key="index" :value="item.code" :label="item.name"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="6">
                <el-form-item size="small" :label="'合同编号'" prop="ContractNo">
                  <el-input v-model="addtemp.ContractNo"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'合同名称'" prop="Contractname">
                  <el-input v-model="addtemp.Contractname"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'合同附件'" prop="Telephone">
                  <el-upload class="upload-demo" ref="upload" name="files" :action="baseURL + '/Files/Upload'" :on-success="upFile" :on-preview="handlePreview" :on-remove="handleRemove" :before-remove="beforeRemove" multiple :limit="1" :on-exceed="handleExceed" :file-list="fileList">
                    <el-button size="small" type="primary">点击上传</el-button>
                  </el-upload>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item size="small" :label="'预约人'" prop="Registername">
                  <el-input v-model="addtemp.Registername" disabled="disabled"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
            ...
            <el-row>
              <el-tag type="info" style="margin-left: 70px" effect="dark"> 单位预约分组 </el-tag>
              <br />
              <br />
              <el-col :span="4" style="margin-left: 70px">
                <el-card style="margin-top: 10px" v-for="(item, index) in addtemp.Company_register_grouplist" :key="index">
                  <a @click="combinechange(item)">
                    <div style="padding: 14px">
                      <span>姓名：<span>{{ item.Groupname }}</span></span>
                      <div class="bottom clearfix">
                        <span>性别：<span>{{ item.Sex == 1 ? '男' : item.Sex == 2 ? '女' : item.Sex == 0 ? '未知' : '' }}</span></span>
              ...
              <el-col :span="17" style="margin-left: 50px">
                <div class="bg-white">
                  <el-table ref="mainTable" :key="tableKey" :data="company_register_combinelistcopy.CombineItems" v-loading="listLoading" border fit highlight-current-row style="width: 100%; margin-top: 10px; margin-left: 20px" @row-click="rowClick">
                    <el-table-column prop="setmealname" label="体检套餐" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="combinename" label="组合名称" show-overflow-tooltip></el-table-column>
                    <!-- <el-table-column  prop="deptcode" label="检查科室" show-overflow-tooltip></el-table-column> -->
                    <el-table-column prop="originalprice" label="原价" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="discount" label="折扣" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="discountprice" label="折后价格 " show-overflow-tooltip></el-table-column>
                  </el-table>
          ...
          <el-dialog v-el-drag-dialog class="dialog-mini" width="1500px" top="50px" :close-on-click-modal="false" :title="textMap[childialogStatus]" :visible.sync="chiladddialogFormVisible" @close="chilclose">
            <el-form :rules="rules" ref="groupdataForm" :model="addCompany_register_grouptemp" label-position="right" label-width="130px">
              <el-row>
                <el-col :span="6">
                  <el-form-item size="small" :label="'分组名称'" prop="Groupname">
                    <el-input v-model="addCompany_register_grouptemp.Groupname"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'优先级'" prop="Priority">
                    <el-input v-model="addCompany_register_grouptemp.Priority"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'年龄上限'" prop="Maxage">
                    <el-input v-model="addCompany_register_grouptemp.Maxage"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'年龄下限'" prop="Minage">
                    <el-input v-model="addCompany_register_grouptemp.Minage"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                ...
                <el-col :span="6">
                  <el-form-item size="small" :label="'分组标识'" prop="Groupmark">
                    <el-input v-model="addCompany_register_grouptemp.Groupmark"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'分组人数'" prop="Grouptotal">
                    <el-input v-model="addCompany_register_grouptemp.Grouptotal"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="6">
                  <el-form-item size="small" :label="'所属部门'" prop="Department">
                    <el-input v-model="addCompany_register_grouptemp.Department"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'原价'" prop="Originalprice">
                    <el-input v-model="addCompany_register_grouptemp.Originalprice"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'折扣'" prop="Discount">
                    <el-input v-model="addCompany_register_grouptemp.Discount"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'增项折扣'" prop="AddDiscount">
                    <el-input v-model="addCompany_register_grouptemp.AddDiscount"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="6">
                  <el-form-item size="small" :label="'折后价格'" prop="Discountprice">
                    <el-input v-model="addCompany_register_grouptemp.Discountprice"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="small" :label="'结算方式'" prop="Settlementtype">
                    <el-select v-model="addCompany_register_grouptemp.Settlementtype">
                ...
                <el-col :span="6">
                  <el-form-item size="small" :label="'增项结算方式'" prop="Addsettlementtype">
                    <el-select v-model="addCompany_register_grouptemp.Addsettlementtype">
              ...
              <el-row>
                <el-col :span="18">
                  <el-form-item size="small" :label="'分组说明'" prop="Remark">
                    <el-input v-model="addCompany_register_grouptemp.Remark"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="12">
                  <el-form-item size="small" :label="'体检套餐字典'" prop="Dic_setmeal">
                    <div class="bg-white">
                      <el-table ref="setmealTable" :key="tableKey" :data="result.setmeal" v-loading="listLoading" border fit highlight-current-row style="width: 100%; margin-top: 10px; margin-left: 20px" @row-click="rowClick" @selection-change="handleDic_setmealSelectionChange">
                        <el-table-column type="selection" align="center" width="55"></el-table-column>
                        ...
                        <el-table-column prop="companyprice" label="单位价格" show-overflow-tooltip></el-table-column>
                        <el-table-column prop="sex" label="性别" show-overflow-tooltip>
                          <template slot-scope="scope">
                            <span>{{ scope.row.sex == '1' ? '男' : scope.row.sex == '2' ? '女' : scope.row.sex == '0' ? '未知' : '' }}</span>
                          </template>
                        </el-table-column>
                      </el-table>
                      <!-- <pagination v-show="total > 0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="handleCurrentChange" /> -->
                ...
                <el-col :span="12">
                  <el-form-item size="small" :label="'体检组合字典'" prop="Dic_combine">
                    <div class="bg-white">
                      <el-table ref="combineTable" :key="tableKey" :data="result.combines" v-loading="listLoading" border fit highlight-current-row style="width: 100%; margin-top: 10px; margin-left: 20px" @row-click="rowClick" @selection-change="handleDic_combineSelectionChange">
                        <el-table-column type="selection" align="center" width="55"></el-table-column>
                        <el-table-column align="center" prop="name" label="组合名称" show-overflow-tooltip>
                          <template slot-scope="scope">
                            <span class="link-type" @click="handleUpdate(scope.row)">{{ scope.row.name }}</span>
                          </template>
                        </el-table-column>
                        <el-table-column align="center" prop="deptcode" label="科室编码" show-overflow-tooltip></el-table-column>
                        <el-table-column align="center" prop="price" label="组合价格" show-overflow-tooltip></el-table-column>
                        <el-table-column align="center" prop="sex" label="性别控制" show-overflow-tooltip>
                          <template slot-scope="scope">
                            <span>{{ scope.row.sex == '1' ? '男' : scope.row.sex == '2' ? '女' : scope.row.sex == '0' ? '未知' : '' }}</span>
                          </template>
                        </el-table-column>
            ...
            <div slot="footer">
              <el-button size="mini" @click="(chiladddialogFormVisible = false), resetaddCompany_register_grouptemp()">取消</el-button>
              <el-button size="mini" v-if="childialogStatus == 'chilcreate'" type="primary" @click="Company_register_grouptempcreateData">确认</el-button>
              <el-button size="mini" v-else type="primary" @click="Company_register_grouptempupdateData">确认</el-button>
            </div>
          </el-dialog>
          <div slot="footer">
            <el-button size="mini" @click="(adddialogFormVisible = false), addresetTemp()">取消</el-button>
            <el-button size="mini" v-if="dialogStatus == 'create'" type="primary" @click="createData">确认</el-button>
            <el-button size="mini" v-else type="primary" @click="updateData">确认</el-button>
          </div>
        </el-dialog>
        <!-- 体检预约视图 -->
        <el-dialog :title="'    体检预约视图'" style="z-index: 100 !important" :close-on-click-modal="false" width="800px" @close="viewShowDialog = false" :visible.sync="viewShowDialog">
          <div class="template">
            <div class="totalinfo" style="margin-bottom: 20px">
              <el-row :gutter="20">
                <el-col :span="8">
                  <div>
                    <el-statistic group-separator="," :precision="2" title="预约总数">
                      <template slot="formatter"> {{ totalNum }} </template>
                    </el-statistic>
                  </div>
                </el-col>
                <el-col :span="8">
                  <div>
                    <el-statistic title="已预约总数">
                      <template slot="formatter"> {{ useTotalNum }} </template>
                    </el-statistic>
                  </div>
                </el-col>
                <el-col :span="8">
                  <div>
                    <el-statistic title="可预约总数">
                      <template slot="formatter"> {{ noUseTotalNum }} </template>
                    </el-statistic>
                  </div>
                </el-col>
              </el-row>
            ...
            <div class="template_body" style="margin-top: 0">
              <vxe-table height="450px" border="inner" highlight-hover-row :data="viewlist" :sort-config="{ trigger: 'cell' }" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" row-id="id" keep-source>
                <vxe-table-column field="date" title="预约日期" width="100px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
                <vxe-table-column field="totalcount" title="预约总数" width="100px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
                <vxe-table-column field="companyusedcount" title="已预约数" width="100px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
                <vxe-table-column field="companycount" title="可预约数" width="100px" show-header-overflow show-overflow="title" show-footer-overflow></vxe-table-column>
              </vxe-table>
            </div>
          </div>
        </el-dialog>
    ...
    <keep-alive v-else :include="'AddOrEdit_gyey'">
      <AddOrEdit key="AddOrEdit_gyey" @submit="handleSubmitSuccess" :editObj="editObj" :addOrEditP="addOrEdit" @close="showAddOrEdit = false" />
    </keep-alive>
  </div>
</template>
...
// 添加编辑子组件
import AddOrEdit from "./components/AddOrEdit_gdgy.vue";
...
export default {
  ...
  //         onClose={this.showAddOrEdit = false}
  //       />
  //     </transition>
  //   </keep-alive>
  //   )
  ...
  methods: {
    ...
    handleView(row) {
      ...
      AppointmentCompanyLoad({
        ...
        this.viewlist = []; //res.result.nomarldateitems
        this.totalNum = 0;
        this.useTotalNum = 0;
        this.noUseTotalNum = 0;
        res.result.nomarldateitems.forEach((item) => {
          let nouserCount = item.totalcount - item.companyusedcount;
          item.companynousedcount = nouserCount < 0 ? 0 : nouserCount;
          this.viewlist.push(item);
          this.totalNum += item.totalcount;
          this.useTotalNum += item.companyusedcount;
          this.noUseTotalNum += item.companycount;
        });
      });
    },
    // 完成结算取消回调
    handleSettlementCancel() {
      if (this.multipleSelection.length == 0) {
        this.$message.warning("请先勾选需要操作单位!");
        return;
      }
      let data = this.multipleSelection.map((k) => {
        return {
          companycode: k.companycode,
          batch: k.batch,
        };
      });
      SetFinishNo(data).then((res) => {
    ...
    handleGetSysConfigInfo() {
      getSysConfigInfo({ cc: "Sys_Company_register_DateRange" }).then((res) => {
        ...
        const end = new Date();
        end.setTime(end.getTime() + 3600 * 1000 * 24 * +res.result);
    ...
    onBtnClicked: function (domId) {
      ...
      switch (domId) {
        case "btnSettlement":
          ...
          break;
        case "btnSettlementCancel":
          this.handleSettlementCancel();
          break;
        case "btnAdd":
          // this.handleCreate();
