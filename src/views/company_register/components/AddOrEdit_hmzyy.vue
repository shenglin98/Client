      ...
      <el-card class="box-card combineitems">
        ...
        <el-table
          ...
          :data="
            ...
            chooseGroup.choosecombineitems.length > 0
              ? [...chooseGroup.combineitems, ...chooseGroup.choosecombineitems]
              : chooseGroup.combineitems
          "
          ref="mainTable"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-checkbox
                v-model="scope.row.addflag"
                :true-label="1"
                :false-label="0"
                :disabled="!scope.row.isOptional"
              ></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  ...
  computed: {
    ...
    choosecombineitemsCount() {
      let templen1 =
        this.chooseGroup.combineitems &&
        this.chooseGroup.combineitems.filter((k) => k.isOptional);
  ...
  mounted() {
    ...
    if (this.copyregid && this.addOrEdit != "edit") {
      console.log("123456");
      this.handleCompanyRegisterCopy();
    }
    //   页面加载获取单位预约 新增/详情数据
  ...
  methods: {
    ...
    handleEditCompanyDetail() {
      getCompanyDetail({ regid: this.regid }).then((res) => {
        ...
        for (let i = 0; i < data.groupitems.length; i++) {
          for (let j = 0; j < data.groupitems[i].combineitems.length; j++) {
            data.groupitems[i].combineitems = data.groupitems[
              i
            ].combineitems.sort(function (a, b) {
              return a.viewgroup - b.viewgroup;
            });
          }
        }
        for (let i = 0; i < data.groupitems.length; i++) {
          data.groupitems[i].combineitems = data.groupitems[i].combineitems.map(
            (k) => {
              return {
                ...k,
                isOptional: false,
          ...
          data.groupitems[i].choosecombineitems = data.groupitems[
            i
          ].choosecombineitems.map((k) => {
            return {
              ...k,
              isOptional: true,
            };
    ...
    // 复制批次 详情数据回调
    handleCompanyRegisterCopy() {
      CompanyRegisterCopy({ regid: this.copyregid }).then((res) => {
        let data = res.result;
        this.bodyForm = data.mainitem;
        let select = data.mainitem.companyitems.filter((k) => k.selected)[0];
        let recept = data.mainitem.receptitems.filter((k) => k.selected)[0];
    ...
    handleSubmitDialogClose() {
      ...
      for (let i = 0; i < this.groupList.length; i++) {
        let ctemp = this.groupList[i].choosecombineitems;
        let tempArr =
          ctemp && ctemp.length > 0
            ? [...this.groupList[i].combineitems, ...ctemp]
            : this.groupList[i].combineitems;
        this.groupList[i].choosecombineitems = tempArr.filter(
          (k) => k.isOptional
        );
        this.groupList[i].combineitems = tempArr.filter((k) => !k.isOptional);
      }
      if (this.addOrEdit == "edit") {
        setGroupUpdate({
    ...
    handleSubmitMedicalItems(data) {
      this.chooseGroup.combineitems = data.sort(function (a, b) {
        return a.viewgroup - b.viewgroup;
      });
      let index = this.groupList.findIndex(
        (k) =>
          k.groupid === this.chooseGroup.groupid &&
          k.name === this.chooseGroup.name
      );
      this.groupList[index] = this.chooseGroup;
      // this.groupList[index] = JSON.parse(JSON.stringify(this.chooseGroup));
    ...
    handleChooseGroup(item) {
      ...
      // this.chooseGroup.combineitems = [
      //   ...this.chooseGroup.combineitems,
      //   ...this.chooseGroup.choosecombineitems,
      // ];
