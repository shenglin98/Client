<template>
  <div class="add_edit">
    <el-card class="box-card message_box">
      <div class="head_box">
        ...
        <div class="btn_group" style="margin-right: 10px">
          <el-button plain type="primary" size="small" @click="handleClickPrint"
            >打 印</el-button
          >
          <el-button
            plain
            ...
            <el-form-item size="small" label="体检时间" prop="rangeTime">
              <div class="date_box">
                <el-date-picker
                  ...
                  @focus="handleFocusRangeTime"
                  @change="handleChangeRangeTime"
                >
                </el-date-picker>
                <!-- <el-button type="primary" @click="handleShowAppointmentTime"
                  >预约时间段</el-button
                > -->
              </div>
            </el-form-item>
        ...
        <el-row style="margin-bottom: 5px">
          ...
          <el-col :span="6">
            <el-form-item size="small" label="分组详情">
              <el-select
                ...
                clearable
                style="width: 100%"
                v-model="bodyForm.showgroupdetailpage"
              >
          ...
          <el-col :span="6">
            <el-form-item size="small" label="取消时间">
              <el-input
                v-model.number="bodyForm.canclehours"
                @input="handleNumberInputCanclehours"
                clearable
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item size="small" label="可约时间">
              <el-input
                v-model.number="bodyForm.registerhours"
                @input="handleNumberInputRegisterhours"
                clearable
              ></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="6">
            <el-form-item size="small" label="单位折扣">
              <el-input
                ...
                @keyup.enter.native="handleChangeMainDiscount"
              ></el-input>
            </el-form-item>
          </el-col>
      ...
      <el-card class="box-card clearance_right">
        ...
        <div class="unit_grouping">
          <div
            ...
            :key="index"
            class="unit_item"
            @click="handleChooseGroup(item, index)"
            :class="[
              {
          ...
          >
              ...
              <el-row>
                ...
                <el-col :span="6">
                  <div class="grid-content bg-purple df">
                    ...
                    <input
                      type="text"
                      class="discount_input title_input"
                      :max="1"
                      :min="0"
                      v-model="item.discount"
                      @keyup.enter="handleChangeItemDiscount(item)"
                    />
                  </div>
                </el-col>
              </el-row>
                ...
                <el-col :span="6">
                  <div class="grid-content bg-purple df">
                    ...
                    <input
                      ...
                      :max="1"
                      :min="0"
                      v-model.lazy="item.realtotal"
                      @change="handleChangeItemRealtotal(item)"
                    />
                  </div>
                </el-col>
              </el-row>
            ...
            <div class="link_btn">
              <el-link
                ...
                :underline="false"
                type="primary"
                @click="handleCustomSettings(item)"
                >自选设置</el-link
              >
              <el-link
                class="copy_group"
      ...
      <el-card class="box-card combineitems">
        <div class="head_box" style="height: 38px">
          ...
          <div class="price_box" v-if="chooseGroup">
            ...
            <span style="margin-left: 15px" v-if="chooseGroup"
              >分组折后总价{{ discountpriceCount }}元</span
            >
        ...
        >
          <el-table-column label="操作" show-overflow-tooltip>
            <template slot-scope="scope">
              <i
                class="el-icon-delete delDelCom"
                @click="handleDelCom(scope.row)"
              ></i>
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column prop="discount" label="折扣" show-overflow-tooltip>
            <template slot-scope="scope">
              <input
                type="text"
                :class="`discount_input discount_${scope.$index}`"
                :max="1"
                :min="0"
                :disabled="scope.row.discountflag === 1"
                v-model.lazy="scope.row.discount"
                @focus="handleDiscountFocus"
                @keyup.enter="handleDiscountLower(scope.row, scope.$index)"
                @keyup.down="handleDiscountLower(scope.row, scope.$index)"
                @keyup.up="handleDiscountUpper(scope.row, scope.$index)"
                @change="handleChangeDiscount(scope.row)"
              />
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              ...
              <input
                ...
                :max="1"
                :min="0"
                v-model.lazy="scope.row.discountprice"
                @focus="handleFocusDiscountprice(scope.row)"
                @change="handleChangeDiscountprice(scope.row)"
              />
            </template>
          </el-table-column>
        </el-table>
    ...
    >
      <div class="top_s">
        ...
        <el-input
          ...
          v-model="searchWord"
          clearable
          @clear="handlegetQueryCopyList"
          @keyup.enter.native="handlegetQueryCopyList"
        >
        </el-input>
      </div>
      <div class="table_box">
        ...
        <div class="table_r">
          <div class="table_r_el">
            <el-table
              ...
              :data="queryCopyGroupList"
              @selection-change="handleChooseQueryCopyGroup"
            >
          ...
          <div class="table_b_btn">
            <el-button size="mini" type="primary" @click="handleSubmitCopyGroup"
              >复 制</el-button
            >
            <el-button size="mini" @click="showQueryCopy = false"
              >关 闭</el-button
            >
          </div>
        </div>
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
      title="复制分组"
      :visible.sync="showCopyGroup"
      @close="handleCloseCopyGroup"
    >
      <div class="discount_box">
        <el-input
          size="mini"
      ...
      <span slot="footer" class="dialog-footer history_table_footer">
        <el-button size="mini" type="primary" @click="handleConfirmCopyGroup"
          >确 认</el-button
        >
    ...
    >
            ...
            <el-col :span="8">
              <el-form-item size="mini" :label="'单位类型'">
                <el-select v-model="addtemp.typecode" clearable>
                  <el-option
                    v-for="(item, index) in typeitemsDia"
                    :key="index"
                    :value="item.code"
                    :label="item.name"
                  ></el-option>
            ...
            <el-col :span="8">
              <el-form-item size="mini" :label="'开户银行'" prop="openingbank">
                <el-select v-model="addtemp.openingbank" clearable>
                  <el-option
                    v-for="(item, index) in bankitemsDia"
                    :key="index"
                    :value="item.code"
                    :label="item.name"
                  ></el-option>
      ...
      <br />
      <div slot="footer">
        <el-button size="mini" @click="adddialogFormVisible = false"
          >取消</el-button
        >
    ...
    <el-dialog
      title="预约时间段列表"
      class="apptime_dia"
      :visible.sync="showAppointmentTime"
      width="85%"
      top="4%"
      :show-close="false"
      :close-on-click-modal="false"
    >
        ...
        <div class="acdate_box">
          <div class="title_select">
            ...
            <el-select size="small" filterable clearable v-model="category">
              <el-option
                :value="'CompanyCheckDate_Yes'"
                label="可预约"
              ></el-option>
              <el-option
                :value="'CompanyCheckDate_No'"
                label="不可预约"
              ></el-option>
            </el-select>
          </div>
          <el-checkbox-group class="datecheck_box" v-model="propertyitems">
            <el-checkbox
              class="datecheck_item"
              v-for="(item, index) in dayitems"
              :key="index"
              :label="item"
              >{{ item.propertyvalue }}({{ item.description }})</el-checkbox
            >
          </el-checkbox-group>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="mini" type="success" @click="handleAddAppTime"
          >新 增</el-button
        >
        <el-button size="mini" type="primary" @click="handleCloseAppTime"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    <el-dialog
      title="自选设置"
      :visible.sync="showCustomSettings"
      width="60%"
      :show-close="false"
      :close-on-click-modal="false"
      class="cust_dialog"
    >
        ...
        <div class="cdctop">
          <div class="cdctinput" v-if="chooseGroup">
            ...
            <el-input
              size="mini"
              placeholder="请输入内容"
              v-model="chooseGroup.chooseruleexpress"
              clearable
            >
            </el-input>
          </div>
          <div class="cdctinput" v-if="chooseGroup">
            ...
            <el-input
              size="mini"
              placeholder="请输入内容"
              v-model="chooseGroup.chooseruledes"
              clearable
            >
            </el-input>
          </div>
          <el-button
            ...
            plain
            icon="el-icon-plus"
            @click="handleAddProjectCS"
            >添加项目</el-button
          >
        </div>
        <div
          class="cdctable"
          v-if="chooseGroup && chooseGroup.choosecombineitems"
        >
          <el-table
            class="combineitems_table"
            :data="chooseGroup.choosecombineitems"
            ref="mainTable"
            height="calc(100% - 20px)"
            min-width="500"
            stripe
          ...
          >
            <el-table-column label="操作" show-overflow-tooltip>
              <template slot-scope="scope">
                <i
                  class="el-icon-delete delDelCom"
                  @click="handleDelComCS(scope.row)"
                ></i>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <input
                  ...
                  :min="0"
                  :disabled="scope.row.discountflag === 1"
                  v-model.lazy="scope.row.groupcode"
                  @focus="handleDiscountFocus"
                />
              </template>
            </el-table-column>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="showCustomSettings = false"
          >取 消</el-button
        >
        <el-button
          size="mini"
          type="primary"
          @click="showCustomSettings = false"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  mounted() {
    ...
    if (this.addOrEdit == "edit") {
      ...
      let time = new Date(+new Date() + 8 * 3600 * 1000)
        ...
        .substr(0, 10)
        .replace("T", " ");
      this.rangeTime = [time, this.getNextMonthDate(time)];
  ...
  methods: {
    ...
    handleSubmitDialog() {
      ...
      if (!/^\d+(\.\d+)?$/.test(this.bodyForm.total)) {
        this.$message.warning("体检人数输入异常!");
        return;
      }
