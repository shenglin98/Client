  ...
  <div class="add_edit">
    <el-card class="box-card message_box">
      ...
      >
        ...
        <el-row style="margin-bottom: 5px">
          ...
          <el-col :span="6">
            <el-form-item size="small" label="体检地点" prop="address">
    ...
    >
      ...
      <el-card class="box-card combineitems">
        ...
        >
          <el-table-column width="45" show-overflow-tooltip>
            <template slot-scope="scope">
              <i
                class="el-icon-delete del_com"
          ...
          >
            <template slot-scope="scope">
              {{ scope.row.discountprice.toFixed(2) || 0 }}
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  components: { SetMealDialog, SetMealOnly, CmbineOnly },
  ...
  data() {
    ...
    return {
      ...
      mainruls: {
        ...
        address: [
          { required: true, message: "请选中体检地点", trigger: "blur" },
        ],
      },
      groupRules: {
        groupname: [
  ...
  methods: {
    ...
    handleConfirmDiscount() {
      ...
      //   }
      // }
      if (!this.isNumber(+this.discountValue)) {
    ...
    handleDelCom(row) {
      ...
      this.chooseGroup.combineitems.forEach((k, i) => {
        if (k.combinecode === row.combinecode) {
          this.chooseGroup.combineitems.splice(i, 1);
        } else {
          this.chooseGroup.originalprice += k.price;
        }
      });
      this.chooseGroup.originalprice =
        +this.chooseGroup.originalprice.toFixed(2);
    },
    ...
    handleChangeAddress(value) {
      if (value) {
        let flag = this.hospitalnameArr.filter((k) => {
          return k.name === value;
        })[0];
        this.bodyForm.meccode = flag.id;
      } else {
        this.bodyForm.meccode = "";
      }
    },
    //   获取单位预约 新增/详情数据回调
    ...
    "groupForm.discount": {
      handler() {
        this.groupForm.discountprice = (
          +this.groupForm.originalprice *
          (+this.groupForm.discount / 100)
        ).toFixed(2);
      },
      deep: true,
    },
