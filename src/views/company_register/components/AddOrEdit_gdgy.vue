  ...
  <div class="add_edit">
        ...
        <el-row>
          <el-col :span="6">
            <el-form-item size="small" :label="'领取方式'">
              <el-select v-model="bodyForm.showReporttake" clearable style="width: 100%">
    ...
    <div class="bottom_group" :class="[addOrEdit == 'edit' ? 'edit_h' : 'add_h']">
      ...
      <el-card class="box-card clearance_right">
        <div class="head_box" style="margin-bottom: 10px">
          ...
          <el-input style="width: 200px;" size="mini" placeholder="输入关键字检索" v-model="searchGroupList" clearable>
          </el-input>
          <div class="btn_group" style="margin-right: 10px">
        ...
        <div class="unit_grouping">
          <div v-for="(item, index) in groupListSS" :key="index" class="unit_item" @click="handleChooseGroup(item, index)" :class="[
              {
      ...
      <el-card class="box-card combineitems">
        <div class="head_box newtjxmbox">
          <div class="newtjxmbox1">
            <div class="tj_box">
            ...
            <span v-else>暂无分组</span> -->
              ...
              <span style="margin-left: 15px" v-if="chooseGroup">折后总价{{ discountpriceCount }}元</span>
              <span style="margin-left: 15px" v-else>折后总价0元</span>
            </div>
          </div>
          <div class="newtjxmbox2">
            <div class="reverse_box" style="display: flex;align-items: center;">
              <div class="rbtt">调整金额：</div>
              <el-input placeholder="请输入金额" style="width:150px" size="mini" v-model="reversePrice" @keyup.enter.native="handleCalcCombinePriceReverse" clearable>
              </el-input>
            </div>
            <div class="btn_group" style="margin-right: 10px">
              <el-button type="primary" size="mini" plain icon="el-icon-plus" @click="handleSetDiscount">折扣</el-button>
        ...
        <div class="combineitems_table_box" style="height:calc(100% - 80px)" v-if="chooseGroup && chooseGroup.combineitems">
          <el-table class="combineitems_table" :data="chooseGroup.combineitems" ref="mainTable" height="100%" min-width="500" stripe fit highlight-current-row style="width: 100%" :row-style="{ height: '32px' }" :header-cell-style="{
            textAlign: 'center',
            background: '#b6cfee',
            fontWeight: 600,
            color: '#18253a',
    ...
    <el-dialog class="dialog-mini group_form" width="58%" top="10%" :close-on-click-modal="false" title="添加分组" :visible.sync="chiladddialogFormVisible" @open="handleOpenChiladd" @close="handleChilclose">
      <el-form ref="groupdataForm" :model="groupForm" :rules="groupRules" label-position="right" label-width="100px">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item size="small" :label="'问卷类型'">
              <el-select v-model="groupForm.quecode" clearable style="width: 100%">
                <el-option v-for="(item, index) in quecodeItems" :key="index" :value="item.code" :label="item.name"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item size="small" :label="'问卷必填'">
              <el-select v-model="groupForm.quemustinput" clearable style="width: 100%">
          ...
          <el-col :span="8">
            <el-form-item size="small" :label="'启用状态'">
              <el-select v-model="groupForm.stopstatus" clearable style="width: 100%">
    ...
    <el-dialog title="预约时间段列表" class="apptime_dia" :visible.sync="showAppointmentTime" width="85%" top="4%" :show-close="false" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="mini" type="primary" @click="handleShowTimetemp('apptimeData')">保存模板</el-button>
        <el-button size="mini" type="primary" @click="handleTimeSelect('apptimeData')">选择模板</el-button>
      </span>
    </el-dialog>
    <el-dialog title="补约时间段列表" class="apptime_dia" :visible.sync="showAppointSupple" width="85%" top="4%" :show-close="false" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="mini" type="primary" @click="handleShowTimetemp('apptimeDataSupple')">保存模板</el-button>
        <el-button size="mini" type="primary" @click="handleTimeSelect('apptimeDataSupple')">选择模板</el-button>
      </span>
    </el-dialog>
    <!-- 模板名称 -->
    <el-dialog class="dialog-mini discount_dialog data_temp_box" width="40%" top="20%" :close-on-click-modal="false" title="模板名称" :visible.sync="showTimeTemp" @close="handleCloseTimeTemp">
      <div class="discount_box">
        <el-input size="mini" placeholder="请输入模板名称" v-model="dataTemplateName" clearable>
        </el-input>
        <!-- <div>%</div> -->
      </div>
      <span slot="footer" class="dialog-footer history_table_footer">
        <el-button size="mini" type="primary" @click="handleTimeSave">确 认</el-button>
        <el-button size="mini" @click="handleCloseTimeTemp">取 消</el-button>
      </span>
    </el-dialog>
    <!-- 选择模板 -->
    <el-dialog class="dialog-mini data_temp_box select_temp_dia" width="70%" top="10%" :close-on-click-modal="false" title="选择模板" :visible.sync="showSelectTimeTemp" @close="handleCloseTimeTemp">
      <div class="select_temp_dia_content">
        <el-row>
          <el-col :span="7" class="select_temp_l">
            <div class="stl_title">模板名称:</div>
            <div class="stl_box">
              <el-checkbox-group @change="handleChangeSelectTemp" v-model="checkListstl">
                <el-checkbox v-for="item,index in selectTimeData" :key="index" :label="item">{{item.tempname}}</el-checkbox>
              </el-checkbox-group>
            </div>
          </el-col>
          <el-col :span="17" class="select_temp_r">
            <EasyTable height="100%" class="select_temp_table" :data="selectTimeDataSon" :tableOptions="selectTimeDataOptions" :radio-config="{ rowClick: true }" :row-style="rowstyle" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }" />
          </el-col>
        </el-row>
      </div>
      <span slot="footer" class="dialog-footer history_table_footer">
        <el-button size="mini" type="primary" @click="handleSelectTimeSave">确 认</el-button>
        <el-button size="mini" @click="handleCloseSelectTimeTemp">取 消</el-button>
        <el-button size="mini" type="danger" @click="handleSelectTimeDel">删除模板</el-button>
      </span>
    </el-dialog>
      ...
      <div class="cdcontent">
        ...
        <div class="cdctable" v-if="chooseGroup && chooseGroup.choosecombineitems">
          <el-table class="combineitems_table" :data="chooseGroup.choosecombineitems" ref="mainTable" height="calc(100% - 20px)" min-width="500" stripe fit highlight-current-row style="width: 100%" :row-style="{ height: '32px' }" :header-cell-style="{
            ...
            <el-table-column label="操作" show-overflow-tooltip>
              <template slot-scope="scope">
                <i class="el-icon-delete delDelCom" @click="handleDelComCS(scope.row,scope.$index)"></i>
              </template>
            </el-table-column>
            <el-table-column width="85" prop="combinecode" label="组合编码" show-overflow-tooltip></el-table-column>
            <el-table-column width="230" prop="combinename" label="组合名称" show-overflow-tooltip></el-table-column>
            ...
            <el-table-column prop="groupcode" label="分组" show-overflow-tooltip>
              <template slot-scope="scope">
                <input type="text" :class="`discount_input discount_${scope.$index}`" v-model.lazy="scope.row.groupcode" />
              </template>
            </el-table-column>
            <el-table-column prop="groupcode" width="180" label="替换项目">
              <template slot-scope="scope">
                <el-select style="width:170px" multiple collapse-tags filterable size="mini" v-model="scope.row.rejectCombineCodeList">
    ...
    <!-- 体检项目 -->
    <SetMealOnly v-if="showSetMealDialog" :sex="chooseGroup.sex" :groupTableData="chooseGroup.combineitems" @submit="handleSubmitMedicalItemsnew" @close="showSetMealDialog = false" />
    <!-- 选择组合分组弹框 -->
    <CmbineOnly v-if="showCmbine" :sex="chooseGroup.sex" :groupTableData="chooseGroup.combineitems" @submit="handleSubmitMedicalItemsnew" @close="showCmbine = false" />
    <SetMealDialog v-if="showProjectDialog" :sex="chooseGroup.sex" :flag="addOrEdit" :groupTableData="[]" :groupName="chooseGroup ? chooseGroup.groupname : ''" :filterData="chooseGroup.combineitems" @submit="handleSubmitMedicalItems" @close="showProjectDialog = false" />
    <!-- chooseGroup.choosecombineitems -->
  </div>
</template>
<script>
import {
...
import { getDicQuestionnaireList } from "@/api/questionnaire.js";
...
export default {
  ...
  components: { SetMealDialog, SetMealOnly, CmbineOnly, CreatedForm },

  computed: {
    // 分组检索回调
    groupListSS() {
      const search = this.searchGroupList;
      if (search) {
        return this.groupList.filter((data) => {
          return Object.keys(data).some((key) => {
            let containArr = ["groupname"];
  ...
  async mounted() {
    ...
    getDicQuestionnaireList({
      ...
      if (!res.data) return;
      // 过滤问卷 不允许为子项并且必须是启用状态
      this.quecodeItems = res.data.filter((k) => {
        return k.ischild == 0 && k.status == 0;
      });
    });
  ...
  methods: {
    handleSelectTimeSave() {
      if (this.checkListstl.length == 0) {
        this.$message.warning("请勾选需要选择的模板!");
        return;
      }
      let tempArr = [];
      for (let i = 0; i < this.checkListstl.length; i++) {
        tempArr.push(...JSON.parse(this.checkListstl[i].tempvalue));
      }
      if (this.isDataTempType == "apptimeData") {
        this.apptimeData.push(...tempArr);
      } else {
        this.apptimeDataSupple.push(...tempArr);
      }
      this.handleCloseSelectTimeTemp();
    },
    handleSelectTimeDel() {
      if (this.checkListstl.length == 0) {
        this.$message.warning("请勾选需要选择的模板!");
      ...
      let data = this.checkListstl.map((k) => k.id);
      SysDataTemplateDelete(data).then((res) => {
        this.$message.success(res.message || res.msg);
        this.handleCloseSelectTimeTemp();
      });
    },
    handleCloseSelectTimeTemp() {
      this.showSelectTimeTemp = false;
      this.selectTimeData = [];
      this.selectTimeDataSon = [];
      this.checkListstl = [];
    },
    handleChangeSelectTemp(arr) {
      if (arr.length > 0) {
        this.selectTimeDataSon = JSON.parse(arr[arr.length - 1].tempvalue);
      } else {
        this.selectTimeDataSon = [];
      }
    },
    handleTimeSelect(type) {
      SysDataTemplateLoad({ category: "Company_register_time" }).then((res) => {
        if (!res.data) {
          this.$message.warning("暂无模板数据!");
          this.isDataTempType = "";
          return;
        }
        this.isDataTempType = type || "";
        this.selectTimeData = res.data;
        this.showSelectTimeTemp = true;
      });
    },
    handleShowTimetemp(type) {
      this.showTimeTemp = true;
      this.isDataTempType = type || "";
    },
    handleCloseTimeTemp() {
      this.showTimeTemp = false;
      this.dataTemplateName = "";
    },
    handleTimeSave() {
      if (!this.apptimeData || this.apptimeData.length == 0) {
        this.$message.warning("请先新增数据在进行保存模板!");
        return;
      }
      if (!this.dataTemplateName.trim()) {
        this.$message.warning("模板名称不允许为空!");
        return;
      }
      let tempvalue = null;
      if (this.isDataTempType == "apptimeData") {
        tempvalue = this.apptimeData.map((k) => {
          return { ...k, timeid: "" };
        });
      } else {
        tempvalue = this.apptimeDataSupple.map((k) => {
          return { ...k, timeid: "" };
        });
      }
      SysDataTemplateSave({
        tempcategory: "Company_register_time",
        tempname: this.dataTemplateName.trim(),
        tempvalue: JSON.stringify(tempvalue),
      }).then((res) => {
        this.$message.success(res.message || res.msg);
        this.handleCloseTimeTemp();
      });
    },
    handleSubmitMedicalItemsnew(obj) {
      let data = obj[0];
      let dialogFlag = obj[1];
      if (dialogFlag === "combine") {
        if (this.chooseGroup.combineitems.length > 0) {
          data = [...this.chooseGroup.combineitems, ...data];
        }
      }
    ...
    // 计算当前项目金额信息等 - 总金额反推
    handleCalcCombinePriceReverse() {
      if (!this.chooseGroup) {
        this.$message.error("请先勾选至少一个分组!");
        return false;
      }
      if (!this.reversePrice) {
        this.$message.error("调整的金额不允许为空!");
        return false;
      }
      let data = this.chooseGroup.combineitems.map((k, index) => {
        let data = {
        ...
        if (index == 0) {
          data.sysDecimals = 2;
          data.settotal = this.reversePrice;
        }
        return data;
      });
    ...
    handleDelComCS(item, flagindex) {
      if (!item) return;
      this.chooseGroup.choosecombineitems.splice(flagindex, 1);
    ...
    handleFreeSubmit() {
      let tips = [];
      for (let i = 0; i < this.chooseGroup.choosecombineitems.length; i++) {
        if (this.chooseGroup.choosecombineitems[i].status != 0) {
          tips.push(this.chooseGroup.choosecombineitems[i].combinename);
        }
      }
      if (tips.length > 0) {
        ...
        this.chooseGroup.choosecombineitems.push(
          ...data.map((k) => {
            return {
              ...k,
              settlementtype: 0,
              rejectCombineCodeList: k.rejectCombineCodeList || [],
            };
          })
        ); // 赋值对象中
