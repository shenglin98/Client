    ...
    <div class="createPost-container">
      <el-form class="form-container" :model="postObj" :rules="rules" ref="postObj">
        ...
        <el-form-item label="选择下一个节点执行用户" v-if="postObj.NodeDesignateType === 'RUNTIME_SPECIAL_USER'">
          <select-users placeholder="*选择下一个节点执行用户" :userNames.sync="postObj.NodeDesignateTxts" :users="postObj.NodeDesignates" v-on:users-change="usersChange">
          </select-users>
        </el-form-item>

        <el-form-item label="选择下一个节点执行角色" v-if="postObj.NodeDesignateType === 'RUNTIME_SPECIAL_ROLE'">
          <select-roles placeholder="*选择下一个节点执行角色" :userNames.sync="postObj.NodeDesignateTxts" :roles="postObj.NodeDesignates" v-on:roles-change="rolesChange">
          </select-roles>
        </el-form-item>
        <!--运行时选定用户 end-->
          ...
          <el-collapse-item name="1">
            ...
            <template v-else-if="postObj.frmType === 2">
              <div style="padding-right: 25px;">
                <CreatedForm insite="false" ref="createdForm" :data="createdFormData" :value="defaultValue">
                </CreatedForm>
              </div>
            </template>
