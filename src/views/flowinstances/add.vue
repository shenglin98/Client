...
export default {
  ...
  methods: {
    ...
    onSchemeChange(schemeId) {
      ...
      //判定开始节点可编辑的表单项内容
      var startNode = schemeContent.nodes.find(
        (i) => i.id.indexOf("start round mix") > -1
      );
      this.canWriteFormItemIds = startNode.setInfo.CanWriteFormItemIds;

      //需要判定开始节点的下一个节点是否为执行时选定执行人
      var startline = schemeContent.lines.find(
        (i) => i.from.indexOf("start round mix") > -1
      );
            ...
            _this.contentData &&
              ...
              _this.contentData.list.forEach((item) => {
                item.options.disabled =
                  _this.canWriteFormItemIds.indexOf(item.model) == -1;
              });
