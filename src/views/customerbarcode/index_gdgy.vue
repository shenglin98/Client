  ...
  <div class="customer_barcode">
    ...
    <sticky class="btn_group" :className="'sub-navbar'">
      <div class="filter-container">
        <permission-btn ref="permissionBtn" v-loading="btnloading" moduleName="customerResult" size="mini" v-on:btn-event="hadnleOnBtnClicked"> </permission-btn>
      </div>
    </sticky>
    <!-- 主体内容 -->
    <div class="subject">
        ...
        <div class="head_body">
          ...
          <div class="checkno_box mt1">
            <div class="checkno">
              ...
              <el-input v-model="regid" ref="checkupWorkInput" @keyup.native.enter="handleInputWork" size="mini" clearable> </el-input>
              <div class="select_box">
                <div class="form_label">团检名称</div>
                <el-select v-model="companycode" clearable @change="handleChangeCompany" size="mini">
            ...
            <el-col class="depart" :span="5">
              ...
              <el-form ref="form" class="depart_form mt1" label-width="80px">
                ...
                <el-form-item class="checkdate" size="mini" label="检查时间">
                  <el-date-picker v-model="collectdate" type="datetime" placeholder="选择日期时间" value-format="yyyy-MM-dd hh-mm-ss"> </el-date-picker>
            ...
            <el-col class="message" :span="18">
              ...
              <el-form ref="form" class="message_form mt1" :model="messageForm" disabled label-width="80px">
                ...
                <el-form-item size="mini" label="体检类型">
                  <el-input v-model="messageForm.checktypename"></el-input>
                </el-form-item>
      ...
      <div class="table_box">
        <el-row>
          <el-col class="t_left" :span="12">
            ...
            <el-pagination style="text-align: center" :pager-count="5" :small="true" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[100, 300, 500, 1000]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="count">
            </el-pagination>
          ...
          <el-col class="t_right" :span="12">
            <div class="title">
              体检组合
              <div class="record" style="margin-left: 40px; font-size: 14px">记录数：{{ combinData.length || 0 }}</div>
            </div>
            <EasyTable height="100%" :data="combinData" class="combin_table" :tableOptions="combinOptions" :checkbox-config="{ fetch: true, rowClick: false }" @selected-change="handleSelectCombin" :border="false" row-key="detid" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="handleTableRowColor" :cell-style="{ padding: '0' }" />
            <!-- <div class="record mt1">记录数：{{ combinData.length || 0 }}</div> -->
            <div class="title" style="margin-top: 20px">
              条码打印
              <div class="record" style="margin-left: 40px; font-size: 14px">记录数：{{ barCodeData.length || 0 }}</div>
            </div>
            <EasyTable height="100%" ref="barCode_table" :data="barCodeData" class="barCode_table mt1" :tableOptions="barCodeOptions" :checkbox-config="{ fetch: true, rowClick: true }" @selected-change="handleSelectBarCode" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="{ height: '32px' }" :cell-style="{ padding: '0' }" />
    ...
    <div class="wait_box" v-if="showSmallGuidance" @dblclick="handleAgainShow">
      <div class="wait_box_no" v-if="waitLength == 0">无人等候</div>
      <div class="wait_box_yes" v-else>等候人数：{{ waitLength }}</div>
    </div>
    <!-- 客户历史体检记录弹框 -->
...
export default {
  ...
  components: { Sticky, permissionBtn, SearchDialog, Guidance },
  ...
  methods: {
    ...
    handleChangeCombine() {
      ...
      let temparr = this.barCodeData.filter((k) => k.status == 0);
      if (temparr.length > 0) {
        this.selectbarCodes = temparr;
    ...
    handleGetCustomerBarcodeList() {
      let whereitems = [];
      if ((this.isEnter && this.regid) || this.messageForm.regid) {
        whereitems.push({
          key: "regid",
          value: this.regid || this.messageForm.regid,
        });
      }
      if (this.companycode) {
        whereitems.push({
    ...
    handleGenerateBarCode() {
      if (this.chooseCustom.length == 0) {
        this.$message.warning("请勾选需要生成条码人员!");
        return;
      }
      let regidList = this.chooseCustom.map((k) => {
        return k.regid;
    ...
    handleCreateBarcodeForLis() {
      ...
      CreateBarcodeForLis([this.regid])
        .then((res) => {
          loading.close();
          if (!res.result) return;
          this.handleGetCustomerBarcodeList();
    ...
    handleInputWork() {
      ...
      if (this.conditionCode === "regid") {
        this.handleCreateBarcodeForLis(); // 生成条码
        // this.handleGetCustomerBarcodeList();
      } else {
          ...
          whereitems: [
            {
              ...
              value: this.regid, // 此时下拉选中不为regid 为其他条件
              // value: this.messageForm.regid,
            },
            {
              key: "pageflag",
    ...
    handleBatchDel() {
      ...
      getCodeDelete({ regiditems }).then((res) => {
    ...
    // 条码更新回调
    handleGetUpdatePrint(regid, flag) {
    ...
    // 获取条码详情回调
    handleGetCodeDetail() {
      this.messageForm.regid && (this.regid = this.messageForm.regid);
      if (!this.regid.trim()) {
        this.$message.warning("请先填写体检号!");
        return false;
      }
      getCodeDetail({ regid: this.regid }).then((res) => {
    ...
    handleSelectCustom(row) {
      ...
      if (temparr.length > 0) {
        for (let i = 0; i < temparr.length; i++) {
          this.$nextTick(() => {
            this.$refs.barCode_table.$refs.easyTable.toggleRowSelection(
              temparr[i],
              true
            );
          });
    ...
    handleSelectBarCode(rows) {
      ...
      console.log(this.selectbarCodes, "selectbarCodes");
      this.codeArr = this.selectbarCodes.map((i) => {
        return i.barcode;
      });
      console.log(this.codeArr, "codeArr");
    },
    // id获取文件回调
    handleGetFiles() {
    ...
    handlePrintBarcode() {
      ...
      let tempList = this.combinData.filter(
        (item) => item.barcode === "" && item.viewgroup == "2"
      );
      if (tempList.length != 0) {
        let msgStr = "";
        tempList.forEach((item) => {
          msgStr += item.combinename + "，";
        });
        this.$confirm(
          "准备打印条码，下列项目未生成条码：" + msgStr + "是否继续打印？",
          "提示",
          {
          ...
          .then(() => {
            console.log("--------确定打印-------------");
            const barCodeloading = this.$loading({
              lock: true,
              text: "条码打印中,请稍候",
              spinner: "el-icon-loading",
              background: "rgba(255, 255, 255, 0.5)",
            ...
            this.$refs.permissionBtn.$children.forEach(async (k, i) => {
              if (k.$el.innerText === "条码打印") {
                this.selectbarCodes = this.selectbarCodes.sort(function (a, b) {
                  return a.seqno - b.seqno;
                });
                ...
                let flag = false;
                let tips = this.selectbarCodes.filter((k) => {
                  return k.status >= 1;
                });
                if (tips.length > 0) {
                  let str = "该条码已打印过，是否继续打印?";
                  for (let i = 0; i < tips.length; i++) {
                    str += `<br/>条码号：${tips[i].barcode}   打印时间：${tips[i].printdate}`;
                  }
                  this.$confirm(str, "提示", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    ...
                    .then(async () => {
                      for (let i = 0; i < this.selectbarCodes.length; i++) {
                        for (
                          let j = 0;
                          j < this.selectbarCodes[i].barcount;
                          j++
                        ) {
                          await this.handlePrintBarcodeApi(
                            this.selectbarCodes[i].barcode,
                            k.$attrs["data-remark"]
                          );
                          i == this.selectbarCodes.length - 1 &&
                            j == this.selectbarCodes[i].barcount - 1 &&
                            (flag = true);
                          await this.handleGetUpdatePrint(
                            this.selectbarCodes[i].barcode,
                            flag
                          );
                        }
                      }
                      await barCodeloading.close();
                    })
                    .catch(() => {
                      barCodeloading.close();
                      this.$message({
                        type: "info",
                        message: "已取消打印",
                      });
                  ...
                  for (let i = 0; i < this.selectbarCodes.length; i++) {
                    for (let j = 0; j < this.selectbarCodes[i].barcount; j++) {
                      await this.handlePrintBarcodeApi(
                        this.selectbarCodes[i].barcode,
                        k.$attrs["data-remark"]
                      );
      ...
      //     //     k.$attrs["data-remark"]
      //     //   );
      //     //   i == this.codeArr.length - 1 && (flag = true);
      //     //   await this.handleGetUpdatePrint(this.codeArr[i], flag);
      //     // }
      //   }
      // });
    },
