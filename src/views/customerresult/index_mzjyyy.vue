    ...
    <div class="pageReg" v-if="!showQuestionnaire">
      ...
      >
        <EasyTable
          ...
          style="height: 380px"
          :data="selectResult"
          :tableOptions="selectResultOptions"
          :radio-config="{ rowClick: true }"
          :row-style="rowstyle"
          :border="false"
          :header-cell-style="handleReturnStyle"
          :row-class-name="handleTableRowClassName"
          :cell-style="{ padding: '0' }"
          @row-click="handleChooseSelectResult"
        />
        <span slot="footer" class="dialog-footer" style="text">
          <el-button type="primary" size="mini" @click="handleCloseSelectResult"
            >关 闭</el-button
          >
          <el-button
            type="primary"
            size="mini"
            @click="handleConfirmSelectResult"
            >确 定</el-button
          >
        </span>
      </el-dialog>
...
export default {
      ...
      return [
        {
          ...
          omit: 3,
          render: (h, data) => {
            return <span>{data.items[0].describe}</span>;
          },
        },
        {
          title: "小结",
          omit: 1,
          render: (h, data) => {
            return <span>{data.conclusion}</span>;
          },
        },
      ];
    },
  ...
  methods: {
    ...
    handleConfirmSelectResult() {
      if (!this.chooseSelectResult) {
        this.$message.warning("请先选中需替换结果!");
        return;
      }
      this.conclusion = this.chooseSelectResult.conclusion;
      this.chooseDepart.items[0].result =
        this.chooseSelectResult.items[0].result;
      this.chooseDepart.items[0].describe =
        this.chooseSelectResult.items[0].result;
      if (this.chooseProjectArr.length > 0) {
        this.chooseProjectArr[0].result =
          this.chooseSelectResult.items[0].result;
        this.chooseProjectArr[0].describe =
          this.chooseSelectResult.items[0].result;
      } else {
        this.chooseProjectArr[0] = this.chooseSelectResult.items[0];
        this.chooseProjectArr[0].describe = this.chooseProjectArr[0].result;
      }
      // 获取小结
      this.handleGetConclusionDetail();
      this.handleCloseSelectResult();
    },
    handleChooseSelectResult(row) {
      this.chooseSelectResult = row;
    },
    handleCloseSelectResult() {
      this.selectResult = [];
    ...
    handleResultSelection() {
      if (!this.chooseDepart) {
        this.$message.warning("请先选中项目组合!");
        return;
      }
      if (this.chooseDepart.viewgroup != 3) {
        this.$message.warning("当前项目不支持结果选择!");
        return;
      }
      getCombinePacsDetail({ regid: this.personalForm.regid }).then((res) => {
        if (!res.result) return;
        this.selectResult = res.result;
    ...
    handleSubmitPage() {
      ...
      let data = {
        ...
        doctorcode: (this.summaryForm && this.summaryForm.doctorcode) || "",
        doctorname: (this.summaryForm && this.summaryForm.doctorname) || "",
        inputdate: this.summaryForm.inputdate,
        items: this.chooseProjectArr,
        worditems: this.entryData,
        imageitems,
      };
    ...
    handleNodeClick(value) {
      if (value && value.combinecode) {
        ...
        getCombineDetail({
          ...
          setTimeout(() => {
            let tempArr =
              this.chooseDepart.items &&
              this.chooseDepart.items.filter((k) => {
