  ...
  <div class="customer_result">
    ...
    <div class="triage_ejectbox" v-drag v-if="showTriageDia">
      ...
      <div class="top_btn">
        <el-button
          size="small"
          type="primary"
          :disabled="triagData.roomstatus == 1"
          @click="handleTriagCall"
          >叫号</el-button
        >
        <el-button
          size="small"
        ...
        <el-button
          ...
          type="primary"
          :disabled="triagData.roomstatus == 1"
          @click="handleTriagSkip"
          >过号</el-button
        >
        <el-button
          size="small"
          type="primary"
          v-if="triagData.roomstatus == 0"
          @click="handleTriagClose"
          >停诊</el-button
        >
        <el-button
          size="small"
          type="primary"
          v-if="triagData.roomstatus == 1"
          @click="handleTriagOpen"
          >开诊</el-button
        >
        <el-button size="small" type="primary" @click="showTriageDia = false"
          >关闭</el-button
        >
      </div>
      <div class="current_message">
        <el-card class="box-card">
          <div class="cmtitle">当前叫号：{{ currentTriag.checkitem }}</div>
          <div class="cmcbox">
            <div class="cmcbox_item">{{ currentTriag.regid }}</div>
      ...
      <div class="eject_table_box">
        ...
        <div class="ejecttable_box">
          <vxe-table
            ...
            highlight-hover-row
            :data="ejectStatus == '待检' ? queuePaiDuiList : completeList"
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-link type="primary" @click="handleTriagCall(scope.row)"
                  >叫号</el-link
                >
              </template>
            </vxe-table-column>
      ...
      <span>
        <div class="tetop">
          ...
          <el-input
            placeholder="请输入内容"
            size="mini"
            v-model="tetopRetrieval"
            @keyup.enter.native="handleLoadRoomList"
            clearable
          >
          </el-input>
        </div>
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-link
                  type="primary"
                  @click="handleChooseTediaTableData(scope.row)"
    ...
    >
      <span>
        ...
        <div class="dtmessage">
          <div class="dtm_item"><div class="sgb">导诊信息</div></div>
          <div class="dtm_item">
            <div>检查科室：</div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="handleNextShowFinish"
          >呼叫下一位</el-button
        >
    ...
    <div class="pageReg" v-if="!showQuestionnaire">
            ...
            <div class="checkup_l">
              <div class="btn_checkup_box">
                <!-- <el-popover
                  ...
                  >
                    ...
                    <el-button
                      type="primary"
                      plain
                      style="padding: 7px 4px; margin-right: 5px"
                      size="mini"
                      slot="reference"
                      >{{ condition }}</el-button
                    >
                ...
                <el-button
                  type="primary"
                  plain
                  style="padding: 7px 4px; margin-left: 5px"
                  size="mini"
                  @click="handleTriageOpen"
                  >分诊</el-button
                >
              </div>
              <el-input
          ...
          <div class="content_r-in">
            <el-form ref="form" :model="personalForm" label-width="70px">
              <el-row>
                ...
                <el-col :span="6">
                  <el-form-item size="mini" label="收费状态">
                    <el-input
                      disabled
                      v-model="personalForm.chargeflagcn"
                    ></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                ...
                <el-col :span="4">
                  <el-form-item
                    class="group_inspection"
                    size="mini"
                    label="费用"
                    label-width="80px"
                  >
                    <el-input disabled v-model="personalForm.price"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item size="mini" label="总检状态">
                    <el-input
                      v-model="personalForm.statuscn"
                      disabled
                    ></el-input>
                  </el-form-item>
                </el-col>
              ...
              <el-row>
                ...
                <el-col :span="6">
                  ...
                  >
                    <el-input
                      v-if="chooseDepart"
                      v-model="chooseDepart.applynomedical"
                    ></el-input>
                    <el-input
                      v-else
                      disabled
                      v-model="personalForm.applynomedical"
                    ></el-input>
                    <el-button
                      :disabled="!personalForm.regid"
                      type="primary"
                      size="mini"
                      @click="handleChangeApplyno"
                      >修改</el-button
                    >
                  </el-form-item>
                </el-col>
      ...
      <div class="subject">
        ...
        <div class="subject_left">
          <div class="department content_right">
            <el-empty
              class="department_empty"
              v-if="departitems.length == 0"
            ></el-empty>
            <div v-if="departitems.length > 0">
              ...
              >
                ...
                >
                  ...
                  />
                  <img
                    v-if="data.isconclusion && data.conclusionstatus == 0"
                    :src="substatus0"
                    :draggable="false"
                    class="rightstreeimg"
                  />
                  <img
                    v-if="data.isconclusion && data.conclusionstatus == 1"
                    :src="substatus1"
                    :draggable="false"
                    class="rightstreeimg"
                  />
        ...
        <div class="subject_right">
            ...
            <div class="project">
              ...
              <div class="project_box">
                ...
                <div v-if="chooseDepart" class="chooseDepart">
                  ...
                  <div
                    class="project_dom1"
                    v-if="chooseDepart.viewgroup == '1'"
                  >
                    <EasyTable
                      :loading="loading"
                      height="100%"
                      class="table_dom1"
                      :data="chooseDepart.items"
                      :tableOptions="checkoutdom1Options"
                      :radio-config="{ rowClick: true }"
                      :row-style="rowstyle"
                      @row-dblclick="handleCheckEcharts"
                      :border="false"
                      :header-cell-style="handleReturnStyle"
                      :row-class-name="handleTableRowClassName"
                  ...
                  <div
                    v-if="chooseDepart.viewgroup == '3'"
                    class="project_dom3"
                  >
                    <!-- dom 3 -->
                    <EasyTable
                      :loading="loading"
                      class="project_dom3_table"
                      height="100%"
                      :data="chooseDepart.items"
                      :tableOptions="doMainOptions"
                      :radio-config="{ rowClick: true }"
                      :border="false"
                      :header-cell-style="handleReturnStyle"
                      :row-class-name="handleTableRowClassName"
                  ...
                  <div class="fruit_btn">
                    ...
                    <el-button-group class="nowrap">
                      <el-button
                        ...
                        plain
                        type="primary"
                        @click="handleGetDefaultConclusion"
                        >默认小结</el-button
                      >
                      <el-button
                        size="mini"
                        plain
                        type="primary"
                        @click="handleGetGenerateConclusion"
                        >生成小结</el-button
                      >
                    </el-button-group>
                    <!-- <el-button
                ...
                <div class="summary-box" v-else>
                  ...
                  >
                    ...
                    <div
                      slot="file"
                      slot-scope="{ file }"
                      @mouseenter="showremoveid = file.url"
                      @mouseleave="showremoveid = ''"
                    >
                      <el-image
                  ...
                  <el-row>
                    <el-col :span="8">
                      <el-form-item size="mini" label="检查医生">
                        <el-select
                          :disabled="summaryForm.conclusionstatus == 1"
                          v-model="summaryForm.doctorcode"
                    ...
                    <el-col :span="8">
                      <el-form-item size="mini" label="录入者">
                        <el-input
                          v-model="summaryForm.operatorname"
                          disabled
                        ></el-input>
                      </el-form-item>
                    </el-col>
                  ...
                  <el-row>
                    ...
                    <el-col :span="8">
                      <el-form-item size="mini" label="状态">
                        <el-input
                          v-model="summaryForm.statusName"
                          disabled
                        ></el-input>
          ...
          <div class="common_result">
            ...
            <el-radio-group
              size="mini"
              :disabled="showTableDisabled"
              v-model="showTableFlag"
            ...
            <div class="table_box">
              <!-- 常见结果 -->
              <EasyTable
                v-if="showTableFlag == 'commonResult'"
                :loading="loading"
                class="common_table"
                height="100%"
                table-layout="auto"
                ...
                :radio-config="{ rowClick: true }"
                @row-dblclick="handleCommonDbclick"
                :border="false"
                :header-cell-style="handleReturnStyle"
                :row-class-name="handleTableRowClassName"
                :row-style="{ height: '32px' }"
              ...
              <!-- 所有检查 -->
              <EasyTable
                v-if="showTableFlag == 'allInspections'"
                :loading="loading"
                class="allInspect_table"
                height="100%"
                :data="allInspectData"
                :tableOptions="allInspectOptions"
                :radio-config="{ rowClick: true }"
                @row-dblclick="handleAllInspectDbclick"
                :border="false"
                :header-cell-style="handleReturnStyle"
                :row-class-name="handleTableRowClassName"
                :row-style="{ height: '32px' }"
              ...
              <!-- 所有检查详情弹框 -->
              <el-dialog
                :visible.sync="showDbSpect"
                width="30%"
                class="dbSpect_dialog"
                @close="handleCloseDbSpect"
              >
                <EasyTable
                  class="dbSpect_table"
                  height="200px"
                  :data="dbSpectData"
                  :tableOptions="dbSpectOptions"
                  :radio-config="{ rowClick: true }"
                  :border="false"
                  :header-cell-style="handleReturnStyle"
                  :row-class-name="handleTableRowClassName"
                ...
                />
                <span slot="footer" class="dialog-footer">
                  <el-button size="mini" @click="handleCloseDbSpect"
                    >取 消</el-button
                  >
                </span>
              </el-dialog>
              <!-- 健康档案 -->
              <EasyTable
                v-if="showTableFlag == 'healthRecords'"
                :loading="loading"
                class="healthRecords_table"
                height="100%"
                :data="healthRecordsData"
                :tableOptions="healthRecordsOptions"
                :radio-config="{ rowClick: true }"
                @row-click="handleClickHealthRecords"
                :border="false"
                :header-cell-style="handleReturnStyle"
                :row-class-name="handleTableRowClassName"
                :row-style="{ height: '32px' }"
              ...
              <EasyTable
                v-if="showTableFlag == 'healthRecords'"
                :loading="loading"
                class="healthDt_table"
                height="100%"
                :data="healthDtData"
                :tableOptions="healthDtOptions"
                :radio-config="{ rowClick: true }"
                @row-dblclick="handleAllInspectDbclick"
                :border="false"
                :header-cell-style="handleReturnStyle"
          ...
          <el-col :span="9" class="tree_box">
            ...
            <el-scrollbar style="height: calc(100% - 38px)">
              <el-tree
                ...
                :default-checked-keys="clusionCheck"
      ...
      <el-dialog
        ...
        :close-on-click-modal="false"
        class="specialis"
        :visible.sync="showSpecialis"
        width="40%"
        @close="handleSpecialisCancel"
      >
        <!-- 按钮组 -->
        <div class="btn_group">
          <el-button size="mini" type="primary" @click="handlePlusSpecialis"
            >新增</el-button
          >
          <el-button size="mini" type="danger" @click="handleDelSpecialis"
            >删除</el-button
          >
          <el-button size="mini" type="warning" @click="handleEditSpecialis"
            >编辑</el-button
          >
          <el-button size="mini" type="success" @click="handleConfirmSpecialis"
            >确认</el-button
          >
          <!-- <el-button size="mini" type="primary" @click="handleSubmitSpecialis">保存</el-button> -->
          <el-button size="mini" type="danger" @click="handleSpecialisCancel"
            >关闭</el-button
          >
        </div>
        <EasyTable
          class="specialis_table"
          height="100%"
          :data="specialisData"
          :tableOptions="specialisOptions"
          :radio-config="{ rowClick: true }"
          @row-click="handleChooseSpecialis"
        />
        <!-- 建议输入框文本域 -->
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button size="mini" type="primary" @click="handleSubmitAddOrEdit"
            >保 存</el-button
          >
        </span>
      </el-dialog>
      ...
      <el-dialog
        ...
        width="50%"
        @close="handleRecommendCancel"
      >
        <!-- 按钮组 -->
        <div class="btn_group">
          <div class="title">智能推荐</div>
          <div>
            <el-button size="mini" type="primary" @click="handleAddSummary"
              >添加小结</el-button
            >
            <el-button size="mini" type="success" @click="handleAddProposal"
              >添加建议</el-button
            >
            <el-button size="mini" type="danger" @click="handleRecommendCancel"
              >关闭</el-button
            >
          </div>
        </div>
        <EasyTable
          class="recommend_table"
          height="100%"
          :data="recommendData"
          :tableOptions="recommendOptions"
          :checkbox-config="{ rowClick: true }"
          @selected-change="handleChooseRecommend"
          :border="false"
          :header-cell-style="handleReturnStyle"
          :row-class-name="handleTableRowClassName"
          :row-style="{ height: '32px' }"
      ...
      <el-dialog
        :close-on-click-modal="false"
        class="echarts_dialog"
        :visible.sync="showEcharts"
        width="50%"
        @opened="handleShowEchartsOpened"
        @close="handleEchartsCancel"
      >
      ...
      <el-dialog
        :close-on-click-modal="false"
        class="triage_dialog"
        :visible.sync="showTriage"
        width="50%"
        @close="handleTriageCancel"
      >
        <div class="personnel">
          <div class="personnel_btn">
            <div class="time_box">
          ...
          <div class="personnel_Table">
            ...
            <vxe-table
              ...
              :header-cell-style="handleReturnStyle"
              :row-class-name="handleTableRowClassName"
              @cell-dblclick="handlePersonnelCheck"
      ...
      >
        <div class="table_box">
          <EasyTable
            ...
            height="100%"
            row-id="groupid"
            :data="historySelectData"
            :tableOptions="historySelectOptions"
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          <el-button size="mini" @click="handleCancelToExamine"
            >取 消</el-button
          >
      ...
      >
        ...
        <span slot="footer" class="dialog-footer" style="text">
          <el-button type="primary" size="mini" @click="handleConfirmConclusion"
            >确 定</el-button
          >
        </span>
      </el-dialog>
      ...
      >
        <EasyTable
          ...
          style="height: 380px"
          :data="criticalList"
          :tableOptions="criticalListOptions"
          :radio-config="{ rowClick: true }"
          :row-style="rowstyle"
          :border="false"
          :header-cell-style="handleReturnStyle"
      ...
      <!-- <el-dialog
          class="triage_dia"
          v-el-drag-dialog
          :visible.sync="showTriageDia"
          width="450px"
          :modal="false"
          :show-close="false"
          :close-on-click-modal="false"
        >
          <span class="triage_content">分诊弹框</span>
        </el-dialog> -->
    </div>
    <Questionnaire
...
export default {
            ...
            return (
              <el-checkbox
                ...
                vOn:change={(e) => {
                  ...
                  if (data.resflag == 0) {
                    let delIndex = this.critiitems.findIndex(
                      (k) => k.itemcode === data.itemcode
                    );
                    delIndex != -1 && this.critiitems.splice(delIndex, 1);
                  }
                }}
              ></el-checkbox>
            );
              ...
              return (
                <div>
                  <el-select
                    ...
                    vOn:focus={(e) => {
                      let index = this.chooseProjectArr.findIndex(
                        (k) => data.itemcode == k.itemcode
                      );
                      index == -1 && this.chooseProjectArr.push(data);
                    }}
                    vOn:change={(e) => {
                      let index = this.chooseProjectArr.findIndex(
                        (k) => data.itemcode == k.itemcode
                      ...
                      index > -1 &&
                        (this.chooseProjectArr[index].tips = data.tips);
                      data.result && this.handleGetConclusionDetail();
                    }}
                  >
        ...
        },
        {
          ...
          render: (h, data) => {
            if (data.result) {
              ...
              return <span style="color:red">{data.describe}</span>;
            }
          },
        },
        {
              ...
              <div style="display: flex;">
                ...
                <el-input
                  ...
                  vOn:input={() => {
                    ...
                    this.chooseProjectArr[0].describe = data.result;
                  }}
                  v-model={data.result}
                ></el-input>
              </div>
  ...
  methods: {
    /* 修改申请号 */
    handleChangeApplyno() {
      this.$confirm("此操作将修改当前申请号, 是否继续?", "提示", {
        ...
        .then(() => {
          ApplynoSave({
            ...
            applyno: this.chooseDepart.applynomedical,
          }).then((res) => {
      ...
      that.$global.ws.onmessage = function (msg) {
        ...
        if (msg.data != "连接成功" && JSON.parse(msg.data)) {
          ...
          if (data.businessType == "Customer_triag_queueRoom") {
            ...
            console.log(that.ejectStatus, "ejectStatus");
            console.log(that.queuePaiDuiList, "queuePaiDuiList");
            console.log(that.completeList, "completeList");
          }
        }
      };
    },
    ...
    handleSetSwitch() {
      if (!this.chooseDepart) {
        this.$message.warning("请先选中需要切换的组合/项目!");
        return;
      }
      // defaultvalue
      // defaultvalue1
      /* 设置默认值标识 1-defaultvalue  2-defaultvalue1 */
      let defaultFlag = JSON.parse(localStorage.getItem("defaultFlag")) || 0;
      this.chooseDepart.items.forEach((k) => {
        if (defaultFlag == 2) {
          k.result = k.defaultvalue;
        } else {
          k.result = k.defaultvalue1;
        }
      });
      ...
      if (defaultFlag == 2) {
        localStorage.setItem("defaultFlag", 1);
      } else {
        localStorage.setItem("defaultFlag", 2);
      }
    },
    // 设置录入状态
    handleSetDepartCheckInputCheck() {
      if (!this.personalForm.regid) {
        this.$message.warning("请先查询人员信息!");
        return false;
      }
      setDepartCheckInputCheck({ regid: this.personalForm.regid }).then(
        (res) => {
    ...
    handleChangeDoctor() {
      ...
      index && (this.summaryForm.doctorname = index.name);
    },
    // 结论词确认提交回调
    ...
    handleGetConclusionDetail() {
      ...
      getConclusionDetail(data).then((res) => {
        ...
        // this.chooseDepart.items = this.chooseDepartItems;
        ...
        .then((res) => {
          ...
          if (!res.result) {
            ...
            if (this.chooseDepart) {
              ...
              {
                /* if (index != -1) {
                  nextClick = nextClick.filter((k, i) => {
                    return i > index;
                  });
                } */
              }
              ...
              if (nextClick.length > 0) {
                ...
                this.$nextTick(() => {
                  if (
                    data.departitems.length > 0 &&
                    data.departitems[0].combineitems.length > 0
                  ) {
            ...
            this.$message.success("操作成功!");
            this.$nextTick(() => {
              console.log(this.personalForm.fileid, "this.personalForm.fileid");
    ...
    handleNodeClick(value) {
      if (value && value.combinecode) {
        ...
        getCombineDetail({
          ...
          this.chooseDepartItems = JSON.parse(JSON.stringify(data.items));
    ...
    hadnleOnBtnClicked(domId) {
      switch (domId) {
        ...
        case "btnSearch": // 检索
          ...
          break;
        case "btnSwitch": // 设置默认值回调
          this.handleSetSwitch();
          break;
        case "btnTriag": // 导诊弹框
        ...
        case "btnToExamine": // 审核页面项目
          ...
          break;
        case "btnCheckEntry": // 设置录入状态
          this.handleSetDepartCheckInputCheck();
          break;
