  ...
  <div class="total_check">
    ...
    <div class="filter-container">
      <div v-for="(btn, index) of buttons" :key="index">
        ...
        <el-button type="primary" :ref="btn.domId" plain v-else :size="size" v-bind:key="btn.Id" v-bind:data-remark="btn.remark" class="filter-item" @click="hadnleOnBtnClicked(btn.domId)"><i :class="`iconfont icon-${btn.icon}`"></i>{{ btn.name }}</el-button>
      </div>
    </div>
    <!-- 主体内容区域 -->
    <div class="subject">
      ...
      <div class="search_box">
        ...
        <el-row class="search_body">
          ...
          <el-col :span="19" class="form_box">
            <el-form ref="form" disabled :model="currentPeople" label-width="80px">
              <el-row>
                <el-col :span="5">
              ...
              <el-row>
                ...
                <el-col :span="5">
                  <el-form-item size="mini" label="身份证">
                    <el-input v-model="currentPeople.idcard"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="5">
                  <el-form-item size="mini" label="体检类型">
                    <el-input v-model="currentPeople.checktypeCN"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="5">
                ...
                <el-col :span="4">
                  <el-form-item size="mini" label="费用">
                    <el-input v-model="currentPeople.price"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="5">
                  <el-form-item size="mini" label="标准金额">
                    <el-input v-model="currentPeople.quota"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="5">
                  <el-form-item size="mini" label="收费状态">
                    <el-input v-model="currentPeople.chargeflagcn"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="5">
                  <el-form-item size="mini" label="打印状态">
                    <el-input v-model="currentPeople.reportprintcn"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="5">
                  <el-form-item size="mini" label="打印时间">
                    <el-input v-model="currentPeople.reportprintdate"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
      ...
      <el-row class="content_box">
        <el-col :span="5" class="personnel_box">
          <div class="pers_table_box">
            ...
            <EasyTable v-if="showPersData" :loading="loading" class="pers_table" height="100%" :data="persData" @row-click="handleChoosePers" :tableOptions="persOptions" :radio-config="{
                rowClick: true,
        ...
        <el-col :span="19">
          <div class="body_box">
            ...
            <div class="check_input">
              <el-input class="virtualconclusion" type="textarea" resize="none" :autosize="true" v-model="virtualconclusion"></el-input>
              <el-button type="primary" size="mini" @click="handleAddConclusionDia">结论选择</el-button>
            </div>
            <!-- 底部表单 -->
    ...
    <!-- 新增结论弹框 -->
    <el-dialog title="结论选择" :visible.sync="showAddConclusion">
      <EasyTable height="100%" :data="describeType" @row-dblclick="handleConfirmAddConc" :tableOptions="addConcOptions" :radio-config="{
          rowClick: true,
        }" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="{ height: '32px' }" :cell-style="{ padding: '0' }" />
      <!-- <el-form :model="addConcForm">
        <el-form-item label="类型" size="mini" label-width="80px">
          <el-select
            style="width: 100%"
            v-model="addConcForm.typecode"
            placeholder="请选择类型"
            clearable
            filterable
          ...
          >
            <el-option
              v-for="(item, index) in describeType"
              :key="index"
              :value="item.typecode"
              :label="item.typecode"
            ></el-option>
        ...
        <el-form-item label="内容" label-width="80px">
          <el-input
            ...
            v-model="addConcForm.name"
          ></el-input>
        </el-form-item>
      </el-form> -->
      <div slot="footer" class="dialog-footer" style="align-items: center">
        <el-button size="mini" @click="handleCloseAddConc">关 闭</el-button>
        <!-- <el-button size="mini" type="primary" @click="handleConfirmAddConc"
          >确 定</el-button
        > -->
      </div>
    </el-dialog>
    ...
    <el-dialog title="批量总检" :visible.sync="showBatchOperate" class="batchOperate" width="65%" :close-on-click-modal="false" :show-close="false">
      <div class="b_content">
              ...
              <el-col :span="4">
                <el-form-item size="mini" label="状态">
                  <el-select v-model="searchForm.status" clearable placeholder="请选择状态" @change="handleGetPrintOrderList">
                    ...
                    <el-option label="已检" value="3,4"></el-option>
                    <el-option label="可总检" value="5"></el-option>
        ...
        <div class="total">数量:{{peopleData.length}}</div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="handleBatchCancel">取消批量总检</el-button>
...
export default {
  ...
  methods: {
    ...
    handleBatchOperateCancel() {
      this.$refs.batable.clearCheckboxReserve();
      this.searchForm = {
        companycode: "",
        batch: "",
        groupcode: "",
    ...
    handleConfirmAddConc(value) {
      if (!value) return;
      if (
        this.virtualconclusion &&
        this.virtualconclusion.includes(value.name)
      ) {
        this.$message.warning("不可重复添加同一结论!");
        return;
      }
      console.log(this.virtualconclusion, "this.virtualconclusion");
      if (this.virtualconclusion == "") {
        this.virtualconclusion += value.name;
      } else {
        this.virtualconclusion += `\n${value.name}`;
      }
      // this.handleCloseAddConc();
    },
    handleCloseAddConc() {
      this.addConcForm = {
        typecode: "",
      ...
      this.showAddConclusion = false;
    },
    // 结论选择改变类型回调
    handleChangeTypeCode(value) {
      if (value) {
        let flag = this.describeType.filter((k) => {
          return k.typecode === value;
        })[0];
        if (!flag) return;
        this.addConcForm.name = flag.name;
      } else {
        this.addConcForm.name = "";
      }
    },
    // 点击展示新增结论选择弹框
    handleAddConclusionDia() {
      dictionary.getList({ tableName: "Dic_describe" }).then((response) => {
        this.describeType = response.data;
        this.showAddConclusion = true;
      });
    },
    // 历史项目表格选中数据回调
        ...
        case "btnReport": // 报告书
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "报告书") {
              let index = d.$children[0].$attrs["data-remark"].split("、")[0];
              let repname = "";
              if (this.currentPeople.checktype == 2) {
                repname = `${index}、公共场所报告书`;
              } else if (this.currentPeople.checktype == 3) {
                repname = `${index}、食品从业报告书`;
              } else if (this.currentPeople.checktype == 5) {
                repname = `${index}、普通员工报告书`;
              } else if (this.currentPeople.checktype == 6) {
                repname = `${index}、自选项目报告书`;
              } else if (this.currentPeople.checktype == 7) {
                repname = `${index}、驾驶证报告书`;
              } else if (this.currentPeople.checktype == 8) {
                repname = `${index}、特种设备报告书`;
              } else {
          ...
          break;
        case "btnEntryReport": // 入职报告书
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "入职报告书") {
              let data = {
                fingercode: localStorage.getItem("md5code"),
                codes: [this.currentPeople.regid],
                repname: d.$children[0].$attrs["data-remark"],
    ...
    handleSetTotalCheckSave() {
      ...
      if (occupationitems.length == 0) {
        data = {
          ...
          virtualconclusion: this.virtualconclusion,
          worditems,
    ...
    handleGetTotalCheck() {
      ...
      getTotalCheckQyzyy({ whereitems }).then((res) => {
        ...
        this.virtualconclusion = data.virtualconclusion;
        if (data.status < 3) {
        ...
        if (data.worditems.length > 0) {
          data.worditems[0].wordname = data.worditems[0].wordname.reverse();
