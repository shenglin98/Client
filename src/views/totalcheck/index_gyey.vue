<template>
  <div class="total_check">
              ...
              <div class="check_right">
                ...
                <div :class="[
                  ...
                  <div class="handle_btn_group">
                    <!-- <div class="handle_btn" @click="handleCopyProposal">
                      <el-tooltip
                        class="item"
                        effect="dark"
                        content="复制建议"
                    ...
                    <div class="handle_btn" @click="handleFollowUpReferral">
                                      ...
                                      <el-col :span="7" class="departmentClas checkFlex">
                                        <div>
                                          ...
                                          <el-select size="mini" v-model="val.transferdeptcode" style="width: 100%" @change="handleTrasnferdept(val)" clearable>
                                            <el-option v-for="(
                                                item, index
                                              ) in DepartmentList" :key="index" :label="item.name" :value="item.id"></el-option>
                                          </el-select>
                                        </div>
    ...
    <!-- 总检弹框 -->
    <el-dialog class="total_dialog" :visible.sync="showTotalCheck" width="50%" @close="handleTotalCheckClose">
      <div class="head_box">
        <div class="personalflag_box">
          <div class="time_title" style="margin-right: 5px">健康证:</div>
          <el-checkbox @change="handleGetDepartCheckList" :true-label="1" :false-label="0" v-model="healthcardflag"></el-checkbox>
        </div>
        <div class="personalflag_box">
          <div class="time_title" style="margin-right: 5px">类型:</div>
        ...
        <div class="time_box">
          ...
          <el-select style="width: 120px" v-model="timeFlag" size="mini" placeholder="请选择" @change="handleGetDepartCheckList">
            <el-option label="登记时间" value="登记时间"> </el-option>
            <el-option label="体检时间" value="体检时间"> </el-option>
            <el-option label="总检时间" value="总检时间"> </el-option>
          </el-select>
    ...
    <el-dialog title="" class="summaryDia" top="1vh" :visible.sync="showMoveConclusion" width="1000px" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" @open="handleOpenMoveConclusion">
      ...
      <el-tree v-if="worditems[0] && showTree" :data="worditems[0].wordname" class="droptree" :props="dragProps" show-checkbox node-key="uniqueid" default-expand-all draggable ref="droptree" :allow-drop="allowDrop" :expand-on-click-node="false" @check-change="handleSelectMoveTree">
        ...
        <span class="custom-tree-node" slot-scope="{ node, data }">
          <!-- <span class="dtitle">{{ data.conclusiontoword }}</span> -->
    ...
    <!-- 个人常用语弹框 -->
    <el-dialog title="个人模板" class="summaryDia commonWordsDia" top="5vh" :visible.sync="showCommonWords" width="800px" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <div class="commonTable_box">
        <div class="tcbInp">
          <el-input placeholder="请输入检索关键字" v-model="searchCommonWords" size="mini" clearable>
          </el-input>
        </div>
        <div class="tcbBox">
          <EasyTable :loading="loading" height="100%" ref="commonTable" class="commonTable table_100 move_conclusion_t" :data="commonWordsSS" :tableOptions="commonWordsOptions" @checked-change="handleCheckCommon" :radio-config="{ fetch: true, selectedVal: setmealRadio }" :row-style="rowstyle" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }" row-id="dcdid" />
        </div>
      </div>
      <span slot="footer" class="dialog-footer" style="text">
    ...
    <!-- 职业病检索组件 -->
    <SearchDialogzyb v-if="OccupationashowSearch" :showSearch="OccupationashowSearch" :pageFlag="pageFlag" @close="handleCloseSearchFp" @submit="handleSearchSubmit" />
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    // 个人模板检索
    commonWordsSS() {
      const search = this.searchCommonWords;
      if (search) {
        return this.commonWords.filter((data) => {
  ...
  methods: {
    ...
    // 打开收藏弹框
    hadnleShowCollect() {
      CustomerCollectPersonDetail({ regid: this.currentPeople.regid })
        .then((res) => {
          console.log(res);
          if (!res.result) {
    ...
    handleCopyProposal() {
      ...
      for (let i = 0; i < this.worditems[0].wordname.length; i++) {
        tempStr == ""
          ? (tempStr += this.worditems[0].wordname[i].healthadvice)
          : (tempStr += `\n\r${this.worditems[0].wordname[i].healthadvice}`);
      }
      let input = document.createElement("input"); // 创建input对象
      input.value = tempStr; // 设置复制内容
        ...
        case "btnReport": // 报告书
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "打印、预览") {
              ...
              } else if (this.currentPeople.meccode == 3) {
                newrepname = `${indexP}番禺报告书`;
              } else {
        ...
        whereitems.push({
          key:
            this.timeFlag == "登记时间"
              ? "startdate"
              : this.timeFlag == "体检时间"
              ? "startregistercheckdate"
              : "startconclusioninputdate",
        ...
        whereitems.push({
          key:
            this.timeFlag == "登记时间"
              ? "enddate"
              : this.timeFlag == "体检时间"
              ? "endregistercheckdate"
              : "endconclusioninputdate",
    ...
    handleWordMerge() {
      ...
      let temp = {
        ...
        conclusiontoword: this.checkAdvise1[0].conclusiontoword,
        illnessstatus: 0,
      };
    ...
    handleMoveTopping() {
      ...
      temp.length > 0 && this.worditems[0].wordname.unshift(...temp);
      this.temp = [];
      this.checkAdvise1 = [];
      this.showMoveConclusion && this.handleUpdateTree();
    },
          ...
          k.wordname.forEach((item, index) => {
            if (
              ...
              if (index == 0) {
                ...
                k.wordname.splice(index, 1);
                k.wordname.splice(index - 1, 0, this.checkAdvise1[i]);
              }
            }
          });
        });
    ...
    handleMoveBottom() {
      ...
      this.checkAdvise1 = this.checkAdvise1.reverse();
      for (let i = 0; i < this.checkAdvise1.length; i++) {
        let index = this.worditems[0].wordname.findIndex(
          (k) =>
            this.checkAdvise1[i].uniqueid == k.uniqueid &&
            this.checkAdvise1[i].wordname == k.wordname
        );
        if (index == this.worditems[0].wordname.length - 1) {
          this.$message.warning("已移动到底部!");
          return false;
        } else {
          this.worditems[0].wordname.splice(index, 1);
          this.worditems[0].wordname.splice(index + 1, 0, this.checkAdvise1[i]);
        }
      }
      this.checkAdvise1 = [];
      this.showMoveConclusion && this.handleUpdateTree();
      ...
      if (!this.chooseConclusion) {
        ...
        let tempArr = [this.chooseConclusion].map((item) => {
          return {
            ...
            conclusiontoword: item.conclusiontoword || "",
            illnessstatus: 0,
          };
        });
        if (this.worditems.length == 0) {
      ...
      this.$refs.popoverRefIdc.forEach((d) => {
        if (d.$children[0].$el.innerText == "打印、预览") {
          let indexP =
            d.$children[0].$attrs["data-remark"].split("、")[0] + "、";
          let newrepname = "";
          console.log(this.currentPeople.meccode, "this.currentPeople.meccode");
          console.log(this.currentPeople.meccode == 3);
          if (
            this.currentPeople.meccode == 0 &&
            this.currentPeople.companytype.toLowerCase().indexOf("vip") > -1
          ) {
          ...
          } else if (this.currentPeople.meccode == "1") {
            newrepname = `${indexP}高干报告书`;
          } else if (this.currentPeople.meccode == 3) {
            newrepname = `${indexP}番禺报告书`;
          } else {
          ...
          console.log(newrepname, "newrepname");
          let data = {
            fingercode: localStorage.getItem("md5code"),
            codes: [this.currentPeople.regid],
            // repname: d.$children[0].$attrs["data-remark"],
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnPhotograph": // 拍照
          this.handlephotographOpenDialog(); // dialog开关
          break;
        default:
          break;
      }
