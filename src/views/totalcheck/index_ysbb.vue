<template>
  <div class="total_check" v-loading="loadPage" element-loading-text="正在生成中..." element-loading-custom-class="my-custom-loading">
    <!-- 头部操作控件 -->
              ...
              <el-row>
                ...
                <el-col :span="4">
                  <el-form-item size="mini" label="体检日期">
                    <el-input v-model="currentPeople.checkdate"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
            ...
            <div class="check_body">
              <div class="check_left">
                <div class="radio_box">
                  ...
                  <el-button type="primary" class="overview" size="mini" plain @click="handleResetSeqno">重新排序</el-button>
                  <el-button type="primary" class="overview" size="mini" plain @click="handleResetOverView">生成综述</el-button>
                  <el-button type="primary" class="overview" size="mini" plain @click="handleDSTotalCheck">AI总检</el-button>
                </div>
                <!-- 当前页操作控件 -->
                ...
                <el-scrollbar class="check_b">
                  ...
                  <div v-for="(item, index) in worditems" :key="index">
                    ...
                    <el-checkbox-group v-if="
                      ...
                      <draggable v-model="item.wordname" chosenClass="chosen" :disabled="false" forceFallback="true" handle=".el-icon-thumb" :group="index" animation="400" @start="handleDragOnStart" @end="handleDragOnEnd">
            ...
            <div class="check_form">
              <el-form ref="form" :disabled="currentPeople.updatestatus == 0" :model="currentPeople" label-width="80px">
                <el-form-item size="mini" label="总检医生">
                  <el-select disabled v-model="currentPeople.doctorcode" clearable>
                ...
                <el-form-item size="mini" label="审核医生">
                  ...
                  <el-select v-model="currentPeople.checkcode" clearable disabled @change="handleChangeCheckCode">
                    <el-option v-for="(item, index) in currentPeople.doctoritems" :key="index" :label="item.name" :value="item.code"></el-option>
                  </el-select>
                </el-form-item>
    ...
    <el-dialog class="dialog-mini history_pson" width="55%" height="100%" top="10%" :close-on-click-modal="false" title="历史体检信息" :visible.sync="historyShow" @close="handleCloseHistory">
      <div class="table_box" style="height: 600px">
    ...
    <!-- AI总检内容展示 -->
    <el-dialog title="" class="DSTotal_dia" :visible.sync="showDSTotal" width="60%" :show-close="false" :close-on-click-modal="false">
      <div class="DSTotal_dia_content">
        <div class="dsdc_logo">
          <img src="./images/ds.png" draggable="none" />
          <div class="ccc">以下内容由:deepseek生成</div>
        </div>
        <div class="dsdc_item" v-for="item,index in dSTotalItems" :key="index">
          <div class="dsdc_item_title">{{index+1}}、{{item.检查项目}}：{{item.诊断}}</div>
          <div class="dsdc_item_content">
            <div class="dsdcc_title">结果分析：</div>
            <el-input class="dsdcc_text" type="textarea" resize="none" :autosize="true" v-model="item.结果分析"></el-input>
            <div class="dsdcc_title">健康建议：</div>
            <el-input class="dsdcc_text" type="textarea" resize="none" :autosize="true" v-model="item.建议"></el-input>
          </div>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" type="primary" plain>导入</el-button>
        <el-button size="mini" type="primary" @click="showDSTotal = false">关 闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  GenerateAgainConclusionTextSortIndex,
  DSTotalCheck,
...
export default {
  ...
  methods: {
    handleDSTotalCheck() {
      ...
      DSTotalCheck([
        ...
        .then((res) => {
          ...
          this.dSTotalItems = JSON.parse(res.result);
          this.showDSTotal = true;
        })
        .finally(() => {
          this.loadPage = false;
        });
      ...
      this.$axios
        ...
        .then(async (res) => {
          ...
          this.dSTotalItems = JSON.parse(res.data.result);
          this.showDSTotal = true;
        })
        .finally(() => {
          this.loadPage = false;
    ...
    handleResetSeqno() {
      ...
      GenerateAgainConclusionTextSortIndex({ key: this.conclusiontext }).then(
        (res) => {
          this.$message.success(res.message);
          if (!res.result) return;
          this.conclusiontext = res.result;
        }
      );
    },
    // 生成综述回调
    ...
    handleMoveBottom() {
      ...
      console.log(this.checkAdvise1.length, "几次");
      for (let i = 0; i < this.checkAdvise1.length; i++) {
    ...
    async hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnKeep": // 保存
          if (
            this.currentPeople.conclusionstatus != -1 &&
            this.currentUser != this.currentPeople.operatorname &&
            this.currentUser != this.currentPeople.checkname &&
            this.userInfo.usertype != "主审医生"
          ) {
            this.$message.warning("当前人员暂无权限修改数据!");
            return false;
          }
        ...
        case "btnToExamine": // 审核
          if (
            ...
            this.$message.warning("当前人员暂无审核权限!");
            return false;
          }
          this.handleGetCheck();
          break;
        case "btnToExamineCancel": // 取消审核
          ...
          let authStrategyFlag1 = this.loginUserInfo.authStrategy.toLowerCase();
          let userTypeFlag1 = this.loginUserInfo.userType.split(",");
          if (
            this.currentPeople.conclusionstatus == 1 &&
            !userTypeFlag1.includes("主审医生") &&
            authStrategyFlag1 != "system"
          ) {
            this.$message.warning("当前人员暂无取消审核权限!");
            return false;
          }
          ...
          break;
        case "btnBatchOperate": // 检索
          let authStrategyFlag = this.loginUserInfo.authStrategy.toLowerCase();
          let userTypeFlag = this.loginUserInfo.userType.split(",");
          if (authStrategyFlag == "normal" && userTypeFlag != "主审医生") {
            this.$message.warning("当前人员暂无权限修改数据!");
            return false;
          }
        ...
        case "btnReport": // 导出报告书
          ...
          if (this.currentPeople && this.currentPeople.status < 7) {
            this.$message({
              showClose: true,
              message: "当前人员未审核,请先审核再进行打印!",
              type: "error",
            });
            return false;
          }
          ...
          if (resObj.result && resObj.result.length > 0) {
            this.$confirm(
              `该报告书在${resObj.result[0].processdate}已打印，操作人${resObj.result[0].operatorname}是否确认再次打印 `,
              "提示",
              {
                confirmButtonText: "确定",
                cancelButtonText: "否",
...
/* 使用 ::v-deep */
::v-deep .my-custom-loading .el-loading-text {
  font-size: 40px;
  color: #409eff;
}
/* 也可以调整图标大小 */
::v-deep .my-custom-loading .el-loading-spinner i {
  font-size: 30px;
}
</style>