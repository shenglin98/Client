...
export default {
          ...
          render: (h, data) => {
            return (
              <div>
                ...
                <span style="color: red" v-show={data.status == 8}>
                  二审
                </span>
              </div>
            );
  ...
  methods: {
    handleIsLastProcess() {
      LastProcess({ regid: this.currentPeople.regid, pt: "PrintReport" }).then(
        (res) => {
          console.log(res);
          if (!res.result) {
            setCancelCheck({
    ...
    async hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnBatchOperate": // 检索
          ...
          if (
            authStrategyFlag == "normal" &&
            !userTypeFlag.includes("主审医生")
          ) {
