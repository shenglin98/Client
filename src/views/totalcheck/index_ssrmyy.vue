<template>
  <div class="total_check">
      ...
      <div class="search_box">
        ...
        <el-row class="search_body" :class="showTopAnm ? 'search_body_y' : 'search_body_n'" v-show="showTopAnm">
          <el-col :span="4" class="code_box">
            ...
            <div class="code_box_bottom">
              <!-- 单位条件 -->
              <el-select filterable clearable size="mini" @change="handleSelectComany" v-model="companycode" placeholder="请选择单位">
                <el-option v-for="item in companyList" :key="item.code" :label="`${item.code} ${item.name}`" :value="item.code"></el-option>
              </el-select>
            </div>
          </el-col>
        ...
        <div class="personnel_box" :class="showLeftAnm ? '' : 'personnel_box_n'">
          <div class="show_box" v-show="showLeftAnm">
            <div class="pers_table_box">
              ...
              <EasyTable :loading="loading" ref="persTable" class="pers_table" height="100%" :data="persData" @row-click="handleChoosePers" :tableOptions="persOptions" :radio-config="{
                  rowClick: true,
              ...
              <el-pagination :pager-count="5" :small="true" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[20, 50, 100, 200, 300]" :page-size="pageSize" layout="total, prev, pager, next" :total="persDataCount">
              </el-pagination>
            </div>
            ...
            <div class="check_body">
              <div class="check_left">
                <div class="btn_top_box">
                  ...
                  <el-button type="primary" class="overview" size="mini" plain @click="handleResetOverView">生成综述</el-button>
                ...
                <el-scrollbar class="check_b">
                  ...
                  <div v-if="checkActive == 'describe'" class="dom1">
                    <el-input class="combinename_text" ref="combinenameText" type="textarea" resize="none" :autosize="true" v-model="conclusiontext"></el-input>
                    <!-- <div
              ...
              <div class="check_right">
                ...
                <div class="check_b jkjyBom" ref="checkL">
                          ...
                          <el-checkbox class="dom1_item" :label="val">
                            ...
                            <div class="dra_b">
                              ...
                              <div class="dra_cont">
                                ...
                                <div class="item_top item_zhon dom1_item_top" v-if="
                  ...
                  <!-- 科室检查 dom3 depart -->
                  <div v-if="checkActive == 'depart'" class="dom3">
                    <EasyTable class="departDt_table" height="100%" :data="departDtData" :tableOptions="
                        departDtFlag === '3'
                          ? departDtOptions1
                          : departDtOptions
    ...
    <!-- 批量总检 -->
    <el-dialog title="批量总检" :visible.sync="showBatchOperate" class="batchOperate" width="65%" :close-on-click-modal="false" :show-close="false">
      <div class="b_content">
        <div class="top">
        ...
        <div class="batable">
          <vxe-table height="auto" border="inner" :loading="bcloading" highlight-hover-row :data="peopleData" :checkbox-config="{ reserve: true }" :sort-config="{ trigger: 'cell' }" :scroll-y="{ enabled: false }" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" @checkbox-change="handleSelectPeopleData" @checkbox-all="handleSelectPeopleAll" row-id="regid">
            <vxe-table-column type="checkbox" width="45" :reserve-selection="true"></vxe-table-column>
            <vxe-table-column field="regid" width="200" title="体检号"></vxe-table-column>
            <vxe-table-column field="name" title="姓名" width="200"></vxe-table-column>
            <vxe-table-column field="sex" title="性别" width="170">
            ...
            <vxe-table-column field="age" title="年龄" width="170"></vxe-table-column>
            <vxe-table-column field="status" title="状态" min-width="90px" show-header-overflow show-overflow="title" show-footer-overflow>
              <template slot-scope="scope">
                <div>
                  ...
                  <span v-show="scope.row.status == 7"> 已审核 </span>
                </div>
              </template>
            </vxe-table-column>
          </vxe-table>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="handleBatchOperateCancel">取 消</el-button>
        <el-button size="mini" type="primary" @click="handleBatchOperateConfirm">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { getPersonalDetail, getPageList } from "@/api/searchDialog.js";
export default {
  name: "totalcheck",
  ...
  activated() {
    ...
    this.$nextTick(() => {
      setTimeout(() => {
        // 设置滚动条的位置
        this.$refs.persTable.$refs.easyTable.bodyWrapper.scrollTop =
          this.scrollTop;
      }, 100); // 需要设置延迟，否则无效
    });
  },
  ...
  methods: {
    ...
    handleGetPrintOrderList() {
      ...
      getPrintOrderList({
        ...
        .then((res) => {
          this.peopleData = res.data.customeritems;
    ...
    handleChangeBatch(data) {
      if (data) {
        ...
        this.peopleData = [];
        let flag = this.batchitems.filter((k) => data === k.code)[0];
    ...
    // 批量总检确认
    handleBatchOperateConfirm() {
      console.log(this.selectPeopleData);
      if (this.selectPeopleData.length == 0) {
        this.$message.warning("请先勾选人员数据!");
        return;
      }
      let data = this.selectPeopleData.map((k) => {
        return k.regid;
      });
      batchSave(data).then((res) => {
        this.$message.success(res.message);
        this.handleBatchOperateCancel();
      });
    },
    // 批量总检取消
    handleBatchOperateCancel() {
      this.showBatchOperate = false;
      this.searchForm = {
        companycode: "",
        batch: "",
        groupcode: "",
    ...
    // 批量总检弹框
    handleSetBatchOperateSave() {
      this.handleGetCompanyList();
      this.showBatchOperate = true;
    },
    // 批量总检弹框选中数据回调
    handleSelectPeopleData({ records }) {
      this.selectPeopleData = records;
    },
    // 批量总检弹框选中数据回调
    ...
    handleGetCompanList() {
      ...
      getPageList({
        ...
        if (res.data) {
          let { customeritems } = res.data;
          this.persDataCount = res.count;
          this.persData = customeritems;
        } else {
          this.persDataCount = 0;
          this.persData = [];
        }
    ...
    handleGetToday() {
      let today = new Date(+new Date() + 8 * 3600 * 1000)
        ...
        .substr(0, 10)
        .replace("T", " ");
      let lastTime = this.recentTime(-30, "yyyy-MM-dd", today);
      ...
      if (this.currentPeople.status >= 5 && this.currentPeople.status < 7) {
        ...
        setTotalCheckSave(data).then((res) => {
          getCheck({
            ...
            this.checkupWork = this.currentPeople.regid;
            this.isloading = true;
            this.showTopAnm = true;
            this.showLeftAnm = true;
            this.handleGetTotalCheck();
    ...
    handleGetTotalCheck() {
      ...
      this.$nextTick(() => {
        let eleA = this.$refs.combinenameText;
        let eleB = this.$refs.checkL;
        eleA && (eleA.$refs.textarea.scrollTop = 0);
        eleB && (eleB.scrollTop = 0);
        this.handleGetHeight();
      });
    },
    handleGetHeight() {
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnBatchOperate": // 批量总检
          this.handleSetBatchOperateSave();
          break;
