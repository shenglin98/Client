        ...
        <el-col :span="8">
          <div class="content_left">
            ...
            <div class="checkup_code">
              ...
              <input type="text" class="read_card" v-model.lazy="checkupWork" />
            </div>
            <!-- //单位名称 -->
            <div class="checkup_code">
              <el-button plain style="width: 80px" size="mini" slot="reference"
                >单位名称</el-button
              >
              <el-select
                v-model="companycode"
            ...
            <div class="checkup_code">
              <el-button plain style="width: 80px" size="mini" slot="reference"
                >批次</el-button
              >
              <el-select v-model="batch" clearable>
                <el-option
            ...
            <div class="checkup_btn">
              <el-button plain type="primary" size="mini" @click="handleChangeStatus('')"
                >全部</el-button
              >
              <el-button plain type="primary" size="mini" @click="handleChangeStatus(0)"
                >待签到</el-button
              >
              <el-button plain type="primary" size="mini" @click="handleGetSign(0)"
                >已签到</el-button
              >
              <el-button plain type="primary" size="mini" @click="handleChangeStatus(2)"
                >待交表</el-button
              >
              <el-button plain type="primary" size="mini" @click="handleGetSign(1)"
                >已交表</el-button
              >
            </div>
            <EasyTable
              ...
              :radio-config="{ selectedVal: setmealRadio, rowClick: true }"
          ...
          <div class="content_right">
            ...
            <div class="checkup_project">
              ...
              <EasyTable
                ...
                @selected-change="handleSelectCombines"
                :border="false"
                :header-cell-style="handleReturnStyle"
                :row-class-name="handleTableRowClassName"
                :row-style="{ height: '32px' }"
  ...
  methods: {
    ...
    handleGetBatch() {
      getBatch({ companycode: this.companycode }).then((res) => {
        ...
        if (selectIndex > -1) {
          ...
          let group = batchitems[selectIndex].groupitems.filter((k) => k.selected)[0];
          group && (this.groupcode = group.code);
        }
      });
    },
    ...
    handleGetPersonalRegisterList() {
      ...
      getPersonalRegisterList({
        ...
        .then((res) => {
          ...
          //   item.groupname = this.groupitems.find((x) => x.code == item.groupcode);
          //   resData.push(item);
          // });
          ...
          this.checkupWork = "";
          if (
            (this.conditionCode === "regid" || this.conditionCode === "hiscode") &&
            res.data.length === 1
          ) {
    ...
    handleGetPersonalDetail(regid) {
      ...
      getPersonalDetail({
        ...
        this.personalForm.sex = data.sex == 0 ? "未知" : data.sex == 1 ? "男" : "女";
          ...
          if (
            ...
            if (this.choosePeople) {
              ...
              if (this.autoSignin && !this.autoCrossCousin) {
                ...
                if (!this.choosePeople.reporttake) {
                  this.$message.warning("请选择报告书领取方式!");
                  return;
                }

                this.$confirm(
                  `确认报告书领取方式为（${this.choosePeople.reporttake}）`,
                  "提示",
                  {
                    confirmButtonText: "是",
                    cancelButtonText: "否",
