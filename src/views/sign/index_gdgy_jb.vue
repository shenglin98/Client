      ...
      <el-row class="bg_white">
        ...
        <el-col :span="8">
          <div class="content_left">
            ...
            <!-- //单位名称 -->
            <div class="checkup_code">
              <el-button plain style="width: 80px" size="mini" slot="reference">单位名称</el-button>
              <el-select v-model="companycode" clearable filterable @change="handleChangeCompany">
                <el-option v-for="(item, index) in companyitems" :key="index" :label="`${item.code}　${item.name}`" :value="item.code"></el-option>
              </el-select>
            </div>
            <!-- //批次 -->
            <div class="checkup_code">
              <el-button plain style="width: 80px" size="mini" slot="reference">批次</el-button>
            ...
            <!-- //单位分组 -->
            <div class="checkup_code">
              <el-button plain style="width: 80px" size="mini" slot="reference">单位分组</el-button>
              <el-select v-model="groupcode" clearable filterable @change="handleChangeGroup">
                    ...
                    <el-row>
                      <el-col :span="7">
                        <el-form-item size="mini" label="领取方式">
                          <el-select :disabled="!personalForm.regid" style="width: 100%;" v-model="personalForm.reporttake" size="mini" clearable placeholder="修改领取方式">
...
export default {
  ...
  methods: {
    ...
    handleSaveReporttake() {
      if (!this.personalForm.regid) {
        this.$message.warning("请先选中需要修改的人员!");
        return;
      }
      if (!this.personalForm.reporttake) {
        this.$message.warning("请先选中领取方式!");
        return;
      }
      let data = [
        {
        ...
        {
          key: "reporttake",
          value: this.personalForm.reporttake,
        },
      ];
    ...
    handleInputWorkScanCode(code) {
      ...
      getPersonalRegisterList({
        ...
        .then((res) => {
          ...
          if (index == -1) {
            this.conditionData.unshift(...res.data);
            this.count += +res.count;
          } else {
            this.conditionData = res.data;
            this.count = res.count;
                ...
                setSignIn([this.choosePeople.regid]).then(async (res) => {
                  ...
                  if (!this.choosePeople.reporttake) {
                    this.$confirm(
                      ...
                      .then(() => {
                        setSignOut([this.choosePeople.regid]).then(
                          async (res) => {
                            if (res.code == 500) {
                              this.handleGetSign(1);
                              return;
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnGiveUpCancel": // 取消弃检
          ...
          break;
        case "btnSaveReporttake": // 保存修改领取方式
          this.handleSaveReporttake();
          break;
        case "btnDelay": // 延检
