      ...
      <div class="s_right">
        ...
        <div class="mode_box">
          ...
          <el-radio-group :disabled="pageFlag == 'edit'" @change="handleChangeSettleMode" v-model="settleMode">
            ...
            <el-radio :label="4">已检结算</el-radio>
          </el-radio-group>
        </div>
        <!-- 结算列表 -->
        ...
        <!-- 操作按钮 -->
        <div class="page_btn">
          <el-button size="mini" type="primary" @click="handleShowUnitDia" :disabled="chargeflag == 1 || pageFlag == 'edit'">结算</el-button>
        ...
        <!-- 单位名称弹框 -->
        <el-dialog class="dialog-mini discount_dialog" width="50%" top="20%" :close-on-click-modal="false" title="" :visible.sync="showUnitDia" @close="handleCloseUnitDia">
          <div class="discount_box">
            <div class="dbox_input">
              <div class="dbi_title">开票名称：</div>
              <el-input style="width: 80%" placeholder="请输入开票名称" size="mini" max="100" min="1" v-model="unitname" clearable>
              </el-input>
            </div>
            <div class="dbox_input">
              <div class="dbi_title">开票税号：</div>
              <el-input style="width: 80%" placeholder="请输入开票税号" size="mini" max="100" min="1" v-model="certno" clearable>
              </el-input>
            </div>
            <div class="dbox_input">
              <div class="dbi_title">开票金额：</div>
              <el-input style="width: 80%" placeholder="请输入开票金额" size="mini" max="100" min="1" v-model="setactualtotal" clearable>
              </el-input>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleConfirmUnitname() {
      ...
      this.handleSetSettlementCreate(); // 调用结算接口
    },
    // 新增结算回调
    handleSetSettlementCreate() {
      ...
      let settlementitem = {
        ...
        companynamesettlement: this.unitname || "",
    ...
    handleCompanySettlementPrint(flag) {
      ...
      if (this.setid === "") {
        this.$message.warning("请先结算完成再进行打印!");
        return;
      }
      let whereitems = [
        {
    ...
    handleGetExportPdf(flag) {
      ...
      if (this.setid === "") {
        this.$message.warning("请先结算完成再进行导出!");
        return;
      }
      let whereitems = [
        {
    ...
    handleSetSettlDetail() {
      setSettlDetail({ setid: this.setid }).then((res) => {
        ...
        this.certno = settlementitem.certno;
