        ...
        <div class="btn_group">
          ...
          >
            <el-button
              ...
              slot="reference"
              plain
              v-if="hospitalsFlag != 'index_hsrmyy'"
              >单位结算表</el-button
            >
            <div
              class="print_item"
              @click="handleCompanySettlementPrint('unit')"
            >
              打印
            </div>
          ...
          >
            ...
            <div
              class="print_item"
              @click="handleCompanySettlementPrint('item')"
            >
              打印
            </div>
    ...
    <div class="subject">
      ...
      <div class="s_left">
        ...
        <EasyTable
          ...
          :data="staffDataSS"
          class="staff_table"
          :tableOptions="pageFlag != 'edit' ? staffOptions : staffOptionsEdit"
        ...
        />
        <div class="total_box">总人数：{{ staffDataSS.length || 0 }}</div>
      </div>
      <!-- 主体右侧操作盒子 -->
      <div class="s_right">
          ...
          <div class="message_itme">
            ...
            <div class="m_value">
              <el-input
                ...
                clearable
                :disabled="pageFlag == 'edit'"
                @keyup.native.enter="handleCalcCombinePrice('', setactualtotal)"
              ></el-input>
            </div>
          </div>
          <div class="message_itme">
        ...
        <div class="mode_explain">
          ...
          <el-input
            class="explain"
            :disabled="chargeflag == 1 || pageFlag == 'edit'"
            type="textarea"
            resize="none"
            v-model="explain"
          ></el-input>
        ...
        <div class="page_btn">
          ...
          <!-- <el-button
            size="mini"
            type="primary"
            :disabled="admisstimes > 0"
            @click="handleSetSettlementSend()"
            >发送</el-button
          > -->
          <!-- :disabled="chargeflag == 1 || pageFlag == 'add'" -->
          <el-button size="mini" type="primary" @click="handleCancelSettlement"
            >取消结算</el-button
          >
          <!-- <el-button
            :disabled="chargeflag == 1"
            size="mini"
            type="primary"
            @click="handleSettlementSplit"
            >结算拆分</el-button
          > -->
        </div>
        <!-- 费用拆分弹框 -->
        ...
        >
          <div class="split_content">
            <!-- 头部按钮 -->
            <div class="top_btn">
              <el-button size="mini" type="primary" @click="handleSplitClose"
                >关 闭</el-button
              >
              <el-button size="mini" type="primary" @click="handleSplitAdd"
                >增 加</el-button
              >
            </div>
            <!-- 主体 -->
                ...
                <el-card
                  :class="
                    chooseItem && chooseItem.customId == item.customId
                      ...
                      : ''
                  "
                  class="box-card split_item"
                  v-for="(item, index) in splitItemData"
                  :key="index"
                >
                  <!-- 遮罩层 -->
                  <div
                    class="item_mask"
                    v-if="!(chooseItem && chooseItem.customId == item.customId)"
                  ></div>
                  <!-- 头部信息 -->
                  <div class="item_top">
                    <div class="item_title">开票信息【{{ index + 1 }}】</div>
                    <div class="item_btn">
                      <el-link type="primary" @click="handleSplitSave"
                        >保存</el-link
                      >
                      <el-link type="primary" @click="chooseItem = item"
                        >编辑</el-link
                      >
                      <el-link type="primary">删除</el-link>
                      <el-link type="primary" @click="handleSetSettlementSend()"
                        >发送</el-link
                      >
                    </div>
                  </div>
                  ...
                  >
                    <el-form-item label="企业名称">
                      <el-select
                        ...
                        @change="handleChangeInvoice(item.invoicecode, item)"
                      >
                        <el-option
                          v-for="(item, index) in companyitems"
                          :key="index"
                    ...
                    <el-form-item label="电话">
                      <el-input
                        size="mini"
                        v-model="item.invoicephone"
                      ></el-input>
                    </el-form-item>
                    <el-form-item label="开户行">
                      <el-input
                        size="mini"
                        v-model="item.invoicebank"
                      ></el-input>
                    </el-form-item>
                    <el-form-item label="账号">
                      <el-input
                        size="mini"
                        v-model="item.invoiceaccountnumber"
                      ></el-input>
                    </el-form-item>
...
import {
  ...
  getUnSettlement,
  setSettlementCreate,
  setSettlementCreateNoSend,
  setSettlementSend,
  setSettlDetail,
  setSettlCancel,
  getCalcPayMoney,
  getCalcCustomerPayMoney,
  CompanySettlementDetail,
  CompanySettlementSave,
} from "@/api/companysettlement.js";
import { CalcCombinePrice } from "@/api/company_register.js";
...
export default {
                    ...
                    case 13: // 键盘回车
                      {
                        ...
                        /* this.handleCalcCombinePrice(data.combinecode); */
                      }
                      break;
                    default:
                      break;
  ...
  methods: {
    ...
    // 计算当前人员标准金额信息等
    handleCalcPriceTotal() {
      this.quotatotal = 0;
      for (let i = 0; i < this.settleModeData.length; i++) {
        this.quotatotal += +this.settleModeData[i].price;
      }
      this.quotatotal = Math.floor(+this.quotatotal * 100) / 100;
    },
    // 计算当前人员金额信息等
    handleCalcPrice() {
      this.settotal = 0;
      for (let i = 0; i < this.settleModeData.length; i++) {
        this.settotal += +this.settleModeData[i].discountprice;
      }
      this.setdiscounttotal = +this.settotal * +this.discount;
      this.setactualtotal =
        Math.floor(+this.settotal * +this.discount * 100) / 100;
    },
    // 计算当前项目金额信息等
      ...
      CalcCombinePrice(data).then((res) => {
        ...
        for (let i = 0; i < res.result.combineitems.length; i++) {
          let index = this.settleModeData.findIndex(
            (k) => k.combinecode === res.result.combineitems[i].combinecode
          );
          if (index > -1) {
    ...
    handleGetCalcPayMoney() {
      ...
      getCalcCustomerPayMoney(regiditems).then((res) => {
        ...
        this.quotatotal = res.result.settlementitem.quotatotal;
        this.setdiscounttotal = +this.settotal * +this.discount;
        {
    ...
    handleSetSettlDetail() {
      setSettlDetail({ setid: this.setid }).then((res) => {
        ...
        this.quotatotal = settlementitem.quotatotal;
