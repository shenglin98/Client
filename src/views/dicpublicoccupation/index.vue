<template>
  <div class="dicpubdictionary-wrap">
    <div class="dicpubdictionary-box">
      <div class="dicpubdictionary-container flex-row">
        <div style="border-right: 1px solid #ccc">
          <div class="buttons-box filter-container">
            <el-button :icon="`iconfont icon-${btn.icon}`" :type="btn.class" size="mini" v-for="btn of dicpubdictionaryBtns" v-bind:key="btn.Id" class="filter-item" @click="onBtnClicked(btn.domId)">{{ btn.name }}</el-button>
          ...
          <el-card shadow="never" class="body-small dicpubdictionary-menu-card">
            <el-tree :current-node-key="listQuery.TypeId" ref="pageTree" node-key="code" :highlight-current="true" @node-click="handleNodeClick" :data="dicpubdictionaryTypes" :default-checked-keys="defaultChecked" :expand-on-click-node="false" default-expand-all :props="dicpubdictionaryTypeProps">
              <span class="custom-tree-node" :ref="`treeItem_${data.code}`" slot-scope="{ node, data }">
        ...
        <el-main class="dicpubdictionary-content flex-item">
          ...
          <el-table v-if="detaillist2.length != 0" height="calc(100vh - 149px)" ref="mainTable" :key="tableKey" :data="detaillist2" v-loading="listLoading" fit highlight-current-row style="width: 100%" @row-click="rowClick" @selection-change="handleSelectionChange" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :cell-style="{ padding: '0' }">
            <el-table-column type="selection" align="center" width="55"></el-table-column>
            <el-table-column width="85" prop="code" label="编号" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="name" label="名称" show-overflow-tooltip>
            ...
            <el-table-column prop="checkType" label="体检类型" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{ scope.row.checkType == '1' ? '职业病' : '常规' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="seqno" label="排序" show-overflow-tooltip>
            </el-table-column>
      ...
      <el-dialog v-if="detaillist2.length != 0" :destroy-on-close="true" :close-on-click-modal="false" class="dialog-mini custom-dialog user-dialog" width="400px" title="公共字典" @close="close" :visible.sync="addTypesDialog">
        <el-form ref="dicpubdictionaryTypeForm" :model="dicpubdictionaryTypesInfo" :rules="dicpubdictionaryRules" el="dicpubdictionary-tayps-form" label-width="80px">
          <el-form-item prop="id" label="编号">
            <el-input size="small" v-model="dicpubdictionaryTypesInfo.code" :disabled="true"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="seqno" label="排序">
            <el-input size="small" v-model="dicpubdictionaryTypesInfo.seqno" oninput="value=value.replace(/[^\d]/g,'')"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="checkType" label="体检类型">
            <el-select v-model="dicpubdictionaryTypesInfo.checkType" placeholder="请选择">
      ...
      <el-dialog v-if="detaillist2.length != 0" v-el-drag-dialog class="dialog-mini" :close-on-click-modal="false" width="500px" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" @close="close">
        <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="100px">
          ...
          <el-form-item prop="checkType" label="体检类型">
            <el-select v-model="temp.checkType" placeholder="请选择">
              <el-option :label="'常规'" :value="0"></el-option>
              <el-option :label="'职业病'" :value="1"></el-option>
            </el-select>
          </el-form-item>
        ...
        <div slot="footer">
          ...
          <el-button size="mini" v-if="dialogStatus == 'create'" :loading="saveLoading" type="primary" @click="createData">确认</el-button>
          <el-button size="mini" v-else type="primary" :loading="saveLoading" @click="updateData">确认</el-button>
        </div>
      </el-dialog>

      <el-dialog v-if="detaillist2.length == 0" :destroy-on-close="true" :close-on-click-modal="false" class="dialog-mini custom-dialog user-dialog" width="400px" title="类型" @close="close" :visible.sync="addTypesDialog">
        <el-form ref="dicpubdictionaryTypeForm" :model="dicpubdictionaryTypesInfo" :rules="dicpubdictionaryRules" el="dicpubdictionary-tayps-form" label-width="80px">
          <el-form-item prop="id" label="编号">
            <el-input size="small" v-model="dicpubdictionaryTypesInfo.code" :disabled="true"></el-input>
          </el-form-item>
...
import * as dicpublicoccupation from "@/api/dicpublicoccupation.js";
...
export default {
  ...
  methods: {
    handleGetList() {
      dicpublicoccupation.getList(this.listQuery).then((res) => {
        if (!res.data) return;
    ...
    onBtnClicked: function (domId) {
      switch (domId) {
        ...
        case "btnUpdatedicpubdictionary":
          if (
            this.dicpubdictionaryTypesInfo.code != undefined &&
            this.dicpubdictionaryTypesInfo.code != null &&
            this.dicpubdictionaryTypesInfo.code != ""
          ) {
            this.addTypesDialog = true;
          } else {
            this.$message({
    ...
    handleCreate() {
      ...
      if (
        this.parentdata.code == undefined ||
        this.parentdata.code == null ||
    ...
    createData() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          this.temp.diccode = this.parentdata.code;
          console.log(this.temp);
          dicpublicoccupation.add(this.temp).then(() => {
            this.dialogFormVisible = false;
            this.listQuery.tableName = "Dic_pub_dictionary";
            dicpublicoccupation.getList(this.listQuery).then((response) => {
      ...
      this.$confirm("是否确认删除?", "提示", {
        ...
        .then(() => {
          if (
            ...
            // 多行删除
            dicpublicoccupation.deleteList(rows.map((u) => u.code)).then(() => {
              this.listQuery.tableName = "Dic_pub_dictionary";
              dicpublicoccupation.getList(this.listQuery).then((response) => {
                this.detaillist = response.data;
              });
      ...
      this.$refs["dicpubdictionaryTypeForm"].validate((valid) => {
        if (valid) {
          ...
          if (
            ...
            dicpublicoccupation
              .addType(this.dicpubdictionaryTypesInfo)
              .then(() => {
                this.$notify({
                  title: "成功",
                  message: "添加成功",
      ...
      this.$confirm("是否确认删除?", "提示", {
        ...
        .then(() => {
          if (
            ...
            dicpublicoccupation
              .deleteType([this.dicpubdictionaryTypesInfo.code])
              .then(() => {
                this.resetdicpubdictionaryTypesInfoTemp();
                this.loaddicpubdictionaryTypes();
                this.$notify({
    ...
    loaddicpubdictionaryTypes() {
      ...
      dicpublicoccupation.getListType(this.listQuery).then((response) => {
        console.log(response);
        this.dicpubdictionaryTypes = response.data;
        if (this.dicpubdictionaryTypes[0].code == "10002") {
          this.detaillist2 = this.dicpubdictionaryTypes[0].detailItems;
        } else {
          this.detaillist = this.dicpubdictionaryTypes[0].detailItems;
        }
        clearTimeout(this.timeId);
        this.timeId = setTimeout(() => {
          console.log(this.dicpubdictionaryTypes);
          if (this.dicpubdictionaryTypes.length == 0) return;
          this.defaultChecked.push(this.dicpubdictionaryTypes[0].code);
          let code = this.dicpubdictionaryTypes[0].code;
    ...
    handleNodeClick(val) {
      ...
      //   if (val.code == "10003") {
      //     this.detaillist2 = val.detailItems;
      //   } else {
      //     this.detaillist2 = [];
      //     this.detaillist = val.detailItems;
      //   }
