<template>
  <div class="personal_register">
    <div class="pageReg" v-if="!showQuestionnaire && !showHealthView">
            ...
            <div class="content_left" :class="[message != 'basic' ? 'height_h' : '']">
              ...
              <div class="basicInfo">
                <div class="message">
                  ...
                  <el-switch v-model="personalForm.visitflag" :disabled="!personalForm.regid" @change="handleChangeVisitflag" active-color="#13ce66" inactive-color="#ff4949" active-text="复检" :active-value="1" :inactive-value="0">
                  </el-switch>
                  <el-switch v-model="personalForm.specialflag" :disabled="!personalForm.regid" @change="handleChangeSpecialflag" active-color="#13ce66" inactive-color="#ff4949" active-text="专项检查" :active-value="1" :inactive-value="0">
                  </el-switch>
                  <div>上一次体检时间：{{ personalForm.lastregdate }}</div>
                  ...
                  <div class="card_operations" v-if="message == 'basic'">
                    <el-select style="width: 80px" :disabled="formFlag" v-model="healthcardcode" placeholder="请选择" size="mini" @change="handleSelectHealthcardCheck">
                ...
                <div v-if="message == 'basic'" class="personal_info">
                  ...
                  <div class="info_body">
                    <div class="form_box">
                      <el-form ref="form" :disabled="formFlag" :model="personalForm" :rules="rulePersonalForm" label-width="90px">
                        <el-row>
                          <el-col :span="6">
                        ...
                        <el-row>
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="婚姻">
                              <el-select style="width: 100%" @change="handleSelectChange" v-model="personalForm.marriage" placeholder="">
                        ...
                        <el-row>
                          <el-col :span="6">
                            <el-form-item size="mini" label="体检类型">
                              <el-select style="width: 100%" @change="handleSelectChange" v-model="personalForm.checktype" clearable placeholder="">
                        ...
                        <el-row>
                          ...
                          <el-col :span="5">
                            <el-form-item size="mini" label="标准金额">
                              <el-input v-model="personalForm.quota"></el-input>
                            </el-form-item>
                          </el-col>
                          <el-col :span="5">
                          ...
                          <el-col :span="4">
                            <el-form-item size="mini" label="纸质报告">
                              <el-select v-model="personalForm.ispaperreport" placeholder="是/否" clearable>
                  ...
                  <div class="checkup_body">
                    <div class="state_group">
                      ...
                      <div class="state_item">
                        ...
                        <div class="state2">
                          {{ personalForm.price || groupPriceTotal.toFixed(2) }}
                        </div>
                      </div>
                      <div class="state_item">
                      ...
                      <div class="state_item" v-if="
                          ...
                          personalForm.regid &&
                          personalForm.price - personalForm.quota > 0 &&
                          personalForm.quota > 0
                        ">
                        <div class="title2">超额</div>
                        <div class="title2">
                          {{
                            (
                              +personalForm.price - +personalForm.quota
                            ).toFixed(2)
                          }}
                        </div>
                      </div>
      ...
      <!-- 替换组合弹框 -->
      <ReplaceCombine v-if="showReplaceCmbine" :sex="personalForm.sex" :flag="pageState" :replaceCombine="chooseReplaceCombine" :groupTableData="groupTableData" @submit="handleReplaceSubmit" @close="showReplaceCmbine = false" />
      <!-- 选择套餐分组弹框 -->
      <!-- <SetMealOnly
          ...
          <el-row class="content_box">
            <el-col class="content_l" :span="17">
              <div class="cl_message">
                ...
                <div class="clm_item">
                  体检类型：<span>{{ pageData.checktype }}</span>
                </div>
                <div class="clm_item">
                  团检名称：<span>{{ pageData.companyname }}</span>
                </div>
              </div>
              <el-radio-group v-model="tableActive" size="mini" @change="handlePaymentInfoFindPay">
              ...
              <div class="table_box">
                <el-table :data="pageData.combineitems" class="pagetable" height="100%" border style="width: 100%" :header-cell-style="handleReturnStyle">
                  ...
                  <el-table-column prop="setmealname" label="体检套餐" width="150">
                  </el-table-column>
                  <el-table-column prop="combinename" label="体检组合" width="150">
                  </el-table-column>
                  ...
                  <el-table-column :prop="
                      tableActive == 1 ? 'paymenttypeyes' : 'paymenttypeno'
                    " :label="tableActive == 1 ? '支付方式' : '退费方式'" width="110" v-if="tableActive != 0">
                  </el-table-column>
                  <el-table-column :prop="tableActive == 1 ? 'paytimeyes' : 'paytimeno'" :label="tableActive == 1 ? '支付时间' : '退费时间'" width="180" v-if="tableActive != 0">
                  </el-table-column>
                  <el-table-column :prop="tableActive == 1 ? 'paynameyes' : 'paynameno'" :label="tableActive == 1 ? '收费人' : '退费人'" width="110" v-if="tableActive != 0">
                  </el-table-column>

                  <el-table-column prop="operatorname" label="开单医生" width="110">
                  </el-table-column>
                  <el-table-column prop="operatordate" label="开单时间" width="150">
                  </el-table-column>
                </el-table>
              </div>
              <div class="bottom1">
              ...
              <div class="bottom2">
                ...
                <div class="b2tag_box" v-for="(item, index) in invoicepriceitems" :key="index">
            ...
            <el-col class="content_r" :span="7">
              <div class="chargemess_item">
                总金额：{{ pageData.totalprice }}
              </div>
              <div class="chargemess_item">
                优惠：{{ pageData.youhuiprice }}
              </div>
            </el-col>
          </el-row>
        </div>
      ...
      <el-dialog title="打印知情同意书" :visible.sync="showInformed" :show-close="false" :close-on-click-modal="false" width="34%">
        <span>
          <el-radio v-model="informedName" label="胃镜检查知情同意书">胃镜检查知情同意书</el-radio>
          <el-radio v-model="informedName" label="肠镜检查知情同意书">肠镜检查知情同意书</el-radio>
        </span>
        <span slot="footer" class="dialog-footer">
          <el-button size="mini" @click="handlePrintInformed">打 印</el-button>
          <el-button size="mini" @click="informedName='',showInformed = false">取 消</el-button>
        </span>
      </el-dialog>
      <!-- 门诊档案弹框 -->
...
import {
  ...
  CustomerRegisterVisit,
  CustomerRegisterSpecial,
  RefundSymbol,
  PaperReport,
  SendCostToHis,
...
import ReplaceCombine from "@/components/SetMealDialog/replaceCombine.vue"; // 组合独立弹框
...
export default {
  ...
  data() {
    ...
    return {
      showReplaceCmbine: false,
      chooseReplaceCombine: null,
      discountProperty: [],
      lodop: "",
      companycode: "",
      companycodeItems: [],
    ...
    tableOptions() {
      return [
          ...
          render: (h, data) => {
            return (
              <el-button
                type="primary"
                size="mini"
                disabled={this.pageState !== "edit"}
                plain
                style="height:23px;padding: 0 10px;"
                icon="el-icon-sort"
                vOn:click_stop_prevent={() => {
                  this.hadnleReplaceCombination(data);
                }}
              ></el-button>
            );
          },
        ...
        },
        {
              ...
              <div style="display: flex;">
                <input
                  ...
                  onkeyup={(e) => {
                    ...
                    switch (e.keyCode) {
                      case 13: // 键盘回车
                        this.handleAloneDiscountPrice(data);
                        break;
                      default:
                        break;
                    }
                  ...
                  vOn:focus={(e) => {
                    e.stopPropagation();
                    e.currentTarget && e.currentTarget.select();
                    this.tempDiscountPrice = data.discountprice;
                  }}
          ...
          //   return this.personalForm.regid
          //     ? data.discountprice
          //     : (Math.floor(data.price * data.discount * 100) / 100).toFixed(2);
          // },
        },
        {
          title: "超出金额",
        ...
        },
        {
          ...
          omit: 1,
          returnVal: (val) => {
            return val == 1 ? "已发送" : val == 2 ? "已登记" : "已完成";
          },
        },
      ];
    },
  ...
  methods: {
    ...
    handleHisCharge() {
      ...
      SendCostToHis({ regid: this.personalForm.regid }).then((res) => {
    ...
    handlePaymentInfoFindPay() {
      PaymentInfoFindPay({
        OrderCode: this.personalForm.regid,
    ...
    handleRefundSymbol() {
      if (!this.choosePeople && !this.choosePeople.regid) {
        this.$message({
          showClose: true,
          message: "请选择人员再进行退费标记!",
          type: "error",
        });
        return false;
      }
      RefundSymbol({ regid: this.personalForm.regid }).then((res) => {
        this.$message.warning(res.message);
        this.handleGetPersonalDetail(this.personalForm.regid); // 更新详情人员数据
      });
    },
    handleChangeSpecialflag(value) {
      if (!this.personalForm.regid) return;
      CustomerRegisterSpecial({
        ids: [this.personalForm.regid],
        specialflag: this.personalForm.specialflag,
      }).then((res) => {
        this.$message.success(res.message);
        this.handleGetPersonalDetail(this.personalForm.regid); // 更新详情人员数据
      });
    },
    handleChangeVisitflag(value) {
      if (!this.personalForm.regid) return;
      CustomerRegisterVisit({
        ids: [this.personalForm.regid],
        visitflag: this.personalForm.visitflag,
      }).then((res) => {
        this.$message.success(res.message);
        this.handleGetPersonalDetail(this.personalForm.regid); // 更新详情人员数据
      });
    ...
    // 替换项目回调
    hadnleReplaceCombination(data) {
      if (!data) return;
      this.chooseReplaceCombine = data;
      this.showReplaceCmbine = true;
    },
    ...
    handleClearCombine() {
      ...
      clearCombine({ regid: this.choosePeople.regid }).then((res) => {
        this.$message.success(res.message);
        this.handleGetPersonalDetail(this.personalForm.regid); // 更新详情人员数据
      });
    },
    ...
    handleConfirmDiscount() {
      ...
      if (!this.personalForm.regid) {
        for (let i = 0; i < this.groupTableData.length; i++) {
          this.groupTableData[i].discount = this.discountValue;
          this.groupTableData[i].discountprice = (
            +this.discountValue * +this.groupTableData[i].price
          ).toFixed(2);
        }
    ...
    // 单个修改折后价确认回调
    handleAloneDiscountPrice(item) {
      if (!item) return;
      if (item.discountprice == this.tempDiscountPrice) return;
      if (this.discountProperty.length != 2) {
        this.$message.warning("当前用户暂无打折权限!");
        item.discountprice = this.tempDiscountPrice;
        return false;
      }
      if (item.discountprice > item.price || item.discountprice < 0) {
        this.$message.warning(
          `折后价不合法! 推荐输入范围 0-${item.price} 之间`
        );
        item.discountprice = this.tempDiscountPrice;
        return;
      }
      if (!this.personalForm.regid) {
        if (+(item.discountprice / item.price).toFixed(2) == 0) {
          item.discount = 0.01;
        } else {
          item.discount = (item.discountprice / item.price).toFixed(2);
        }
      } else {
        setDiscount({
          regid: this.personalForm.regid,
    ...
    hadnleSubmit() {
      let isDiscountNum = "";
      for (let i = 0; i < this.groupTableData.length; i++) {
        if (this.groupTableData[i].discount != 1) {
          isDiscountNum = this.groupTableData[i].discount;
          break;
        }
      }
      if (isDiscountNum) {
        ...
        if (this.discountProperty.length === 2) {
          let min = +this.discountProperty[0];
          let max = +this.discountProperty[1];
          if (+isDiscountNum < min || +isDiscountNum > max) {
            this.$message.warning("超出当前用户打折范围!请调整后重试");
            return false;
          }
        }
      }
      ...
      setSubmit(this.personalForm)
        .then((res) => {
          ...
          // 是否需要纸质报告
          if (this.ispaperreport == "1") {
            PaperReport({
              ids: [this.personalForm.regid],
              ispaperreport: this.personalForm.ispaperreport,
            }).then((res) => {});
          }
        })
        .catch((err) => {
    ...
    handleReplaceSubmit(regid) {
      this.showReplaceCmbine = false;
    ...
    handlePrintGuideSingle() {
      ...
      if (d.$el.innerText == "指引单打印") {
        ...
        } else if (this.choosePeople.meccode == 10003) {
          if (this.choosePeople.personalflag == 1) {
            repname = `${indexP}水电团体指引单`;
          } else {
            repname = `${indexP}水电指引单`;
          }
        } else if (this.choosePeople.meccode == 10004) {
          if (this.choosePeople.personalflag == 1) {
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnInformed": //  知情同意书
          ...
          break;
        case "btnRefundFlag": // 退费标记
          this.handleRefundSymbol();
          break;
        case "btnHisCharge": // his收费
          this.handleHisCharge();
          break;
        case "btnSearch": // 检索
