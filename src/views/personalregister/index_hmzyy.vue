    ...
    <div class="pageReg" v-if="!showQuestionnaire && !showHealthView">
            ...
            <div class="content_left" :class="[message != 'basic' ? 'height_h' : '']">
              ...
              <div class="basicInfo">
                  ...
                  <div class="info_body">
                    <div class="form_box">
                      <el-form ref="form" :disabled="formFlag" :model="personalForm" :rules="rulePersonalForm" label-width="80px">
                        ...
                        <el-row>
                          <el-col :span="6">
                            <el-form-item size="mini" label="体检类型" prop="checktype">
                              <el-select @change="handleSelectChange" v-model="personalForm.checktype" placeholder="" clearable>
                                <el-option v-for="item in checktypeitems" :key="item.code" :label="item.name" :value="item.code"></el-option>
                              </el-select>
                            </el-form-item>
                          </el-col>
                          <el-col :span="6">
                            <el-form-item size="mini" label="电话" prop="telephone">
                ...
                <div v-if="message == 'other'" class="other_info">
                  ...
                  <el-form ref="form" class="other_info_form" :disabled="formFlag" :model="personalForm" :rules="rulePersonalForm" label-width="90px">
                    <el-row>
                      <el-col :span="8">
                    ...
                    <el-row>
                      <el-col :span="8">
                        <el-form-item size="mini" label="现读学校">
                          <!-- :prop="personalForm.checktype == 2 ? 'email' : ''" -->
                          <el-input v-model="personalForm.email"></el-input>
                        </el-form-item>
                      </el-col>
                      <el-col :span="8">
                        <el-form-item size="mini" label="现读年级">
                          <!-- :prop="personalForm.checktype == 2 ? 'remark' : ''" -->
                    ...
                    <el-row>
                      ...
                      <el-col :span="8">
                        <el-form-item size="mini" label-width="55px" class="age" label="年龄">
      ...
      <el-dialog class="dialog-mini charge_details" width="58%" top="4%" :close-on-click-modal="false" title="收费列表" :visible.sync="showChargeDetails" @close="handleCloseChargeDetails">
        <div>
          ...
          <el-button size="mini" style="margin-left: 20px" type="primary" @click="handlePrintFeeDetails">打印收费明细</el-button>
        </div>
        <div class="table_box">
...
import SetMealDialog from "@/components/SetMealDialog/index_hmzyy.vue"; // 新项目弹框
...
export default {
  ...
  data() {
    ...
    return {
      ...
      buttons: [],
      size: "mini",
      domIdArr: ["btnPaySingle", "btnLabel", "btnApply", "btnGuideSingle"],
      dialogVisible: false,
      pdfUrl: "",
  ...
  methods: {
    ...
    handleGetUserPropertyValues() {
      ...
      users.getUserProperty({ id, ct }).then((res) => {
        ...
        if (resDis) {
          this.discountPropertySource = resDis.propertyvalue;
          this.discountProperty = resDis.propertyvalue.split("-");
        }
      });
    },
    ...
    handleConfirmDiscount() {
      ...
      if (this.discountProperty.length === 2) {
        ...
        if (+this.discountValue < min || +this.discountValue > max) {
          /* this.$message.warning("超出当前用户打折范围!"); discountPropertySource*/
          this.$confirm("超出当前用户打折范围, 是否联系管理员?", "提示", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning",
          })
            .then(() => {
              SysMessagesSendMsg({
                TypeId: "SYS_MSG_Discount",
                Content: `原折扣范围：${this.discountPropertySource}，修改折扣为：${this.discountValue}`,
              }).then((res) => {
                if (res.code == 200) {
                  this.discountValue = 1;
                  this.$message.success("已成功发送消息!");
    ...
    // 点击保存提交回调
    hadnleSubmit() {
      /* if(this.personalForm.checktype == 2 && (this.personalForm.email == "" || this.personalForm.remark == "")){
        this.$message.warning("现读学校/年级不能为空!");
        return false;
      } */
      ...
      if (!this.personalForm.checktype) {
        this.$message.warning("请选择体检类型!");
        return false;
      }
    ...
    async handleCardReading() {
      ...
      if (this.healthcardcode == "idcard") {
        ...
        } else if (this.cardReaderFlag == "中控") {
          this.handleReadCardZhongKon(); // 精伦读卡
        }
      } else {
        ...
        getPatientInfo({
          ...
          .then((res) => {
            ...
            if (res.result.length == 1) {
              if (
                (res.result[0] && res.result[0].idcard) ||
                res.result[0].hiscode
              ) {
    ...
    // 中控读卡
    handleReadCardZhongKon() {
      let url = `http://127.0.0.1:24010/ZKIDROnline/ScanReadIdCardInfo?OP-DEV=1&CMD-URL=4&REPEAT=1`;
      this.$axios
        .get(url)
        .then((res) => {
          let tempData = JSON.parse(res.data.replace(/\\/g, "/"));
          if (tempData.ret == 0) {
            let data = tempData.Certificate;
            let date = new Date();
            let year = date.getFullYear();
            this.personalForm.name = data.Name;
            this.personalForm.address = data.Address;
            this.personalForm.sex = data.Sex == "男" ? 1 : 2;
            this.personalForm.birthday = data.Birthday.replace(
              /^(\d{4})(\d{2})(\d{2})$/,
              "$1-$2-$3"
            );
            this.personalForm.idcard = data.IDNumber;
            this.healthCard = data.IDNumber;
            this.personalForm.age = year - data.Birthday.slice(0, 4);
            this.personalForm.filebase64 = data.Base64Photo;
            this.avatarImg = `data:image/jpeg;base64,${data.Base64Photo}`;
            this.handleHealthCard();
            // this.handleGetRegisterInfo();
          } else {
            this.$message.warning(`${res.data.retmsg}，请联系管理员!`);
    ...
    handleHealthCard() {
      ...
      getPatientInfo({
        ...
        .then((res) => {
          ...
          if (res.result.length == 1) {
            console.log(
              (res.result[0] && res.result[0].idcard) || res.result[0].hiscode,
              "调试"
            );
            if (
              (res.result[0] && res.result[0].idcard) ||
