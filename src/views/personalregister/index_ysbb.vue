                  ...
                  <div class="info_body">
                        ...
                        <el-row>
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="身份证" prop="idcard">
                              ...
                              ></el-input> -->
                              <input type="text" class="read_card" v-model.lazy="personalForm.idcard" @keyup.enter="handleChangeIdcard" />
                            </el-form-item>
                          </el-col>
                        </el-row>
                        <el-row>
                        ...
                        <el-row>
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="标准金额">
                              <el-input v-if="pageState == 'add'" disabled v-model="checkGroup.value"></el-input>
                              <el-input v-else disabled v-model="personalForm.quota"></el-input>
                            </el-form-item>
                          </el-col>
                          <el-col :span="6">
                    ...
                    <div class="photo_box">
                      ...
                      <el-dialog class="photo_box_dialog" title="拍照上传" :close-on-click-modal="false" :visible.sync="showCanvas" @close="handleOnCancel">
                        <div class="box">
                          <video id="videoCamera" class="canvas" style="border-radius: 50%; border:1px solid black" :width="videoWidth" :height="videoHeight" autoPlay></video>
                          ...
                          <el-row>
                            <el-col :span="10">
                              <el-form-item class="tuanjian" size="mini" label="团检名称">
                                <el-select filterable clearable @change="handleSelectComany" v-model="personalForm.companycode" placeholder="" :class="
...
import { userMedia } from "../../utils/faces";
  ...
  methods: {
    ...
    handleGetDicSetmealList() {
      getDicSetmealList({ tp: "", sex: 0, gp: "1" }).then((res) => {
        ...
        console.log(chooseSetmeal, "chooseSetmeal");
    ...
    handleOnComplete() {
      ...
      if (canvas.getContext) {
        ...
        // 调用 toDataURL 方法获取图像数据
        const imageDataURL = canvas.toDataURL("image/png");
        // 您现在可以处理 imageDataURL 了，例如将其发送到服务器
        console.log(imageDataURL.split(",")[1]);
        this.avatarImg = imageDataURL;
        this.personalForm.filebase64 = imageDataURL.split(",")[1];
        this.showCanvas = false;
        this.handleStopNavigator(); // 关闭摄像头
        this.handleClearCanvas("canvasCamera"); // 清空画布
      }
    ...
    openCamera() {
      ...
      this.$nextTick(() => {
        ...
        this.thisContext = canvas.getContext("2d");
        this.videoObj = document.getElementById("videoCamera");
        this.thisVideo = document.getElementById("videoCamera"); 
        // eslint-disable-next-line no-undef
        const tracker = new tracking.ObjectTracker("face"); // 检测人脸
        console.log(canvas, context, this.videoObj, tracker);
        tracker.setInitialScale(4);
        tracker.setStepSize(2);
        tracker.setEdgesDensity(0.1); 
        // // eslint-disable-next-line no-undef
        this.trackerTask = tracking.track("#videoCamera", tracker, {
          camera: true,
        });
        ...
        tracker.on("track", (event) => {
          event.data.forEach((rect) => {
            // 绘制到 canvas
            console.log(rect, this.videoObj, "rect");
            context.drawImage(this.videoObj, 0, 0, canvas.width, canvas.height);
            context.font = "16px Helvetica";
            context.strokeStyle = "#1890ff";
            context.strokeRect(rect.x, rect.y, rect.width, rect.height); // 识别出来的人像带框
          });
