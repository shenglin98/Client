                           ...
                           -->
                            ...
                            <el-input size="mini" :ref="val" v-model="obj[index]" :key="index">
                              <template slot="append">{{val}}</template>
                                    ...
                                    <el-checkbox v-if="[2, 4].indexOf(value.inputtype) !== -1" v-model="value.flag" :label="value.content" :true-label="1" :false-label="0">{{ value.content }}</el-checkbox>
...
export default {
  ...
  methods: {
            ...
            CheckPreSaveChooseRule(filterSubmit(this.personalForm)).then(
              (res) => {
                if (res.code == 200 && res.result) {
                  ...
                  this.$confirm("当前分组包含自选项目,是否继续选择?", "提示", {
                    confirmButtonText: "是",
                    cancelButtonText: "否",
                    showClose: false,
                    type: "warning",
    ...
    handleGetPersonalDetail(regid) {
      ...
      getPersonalDetail({
        ...
        if (this.personalForm.companycode && this.personalForm.batch) {
          console.log(this.personalForm.batch, "调用了");
    ...
    handlePrintGuideSingle() {
      ...
      if (d.$el.innerText == "指引单打印") {
        LastProcess({ regid: this.choosePeople.regid, pt: "PrintGuide" }).then(
          (res) => {
            if (!res.result) {
              this.handlePrintGuideSingleApi(d.$attrs["data-remark"]);
            } else {
              this.$confirm(
    ...
    handlePrintGuideSingleApi(btnrepname) {
      ...
      let repname = "";
      if (this.choosePeople.checktype == 3) {
