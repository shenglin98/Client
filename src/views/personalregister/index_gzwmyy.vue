...
export default {
  ...
  data() {
    ...
    return {
      ...
      buttons: [],
      size: "mini",
      domIdArr: ["btnPaySingle", "btnLabel", "btnApply", "btnPhysique"],
      dialogVisible: false,
      pdfUrl: "",
  ...
  methods: {
      ...
      setFilesUpload(data).then((res) => {
        ...
        if (this.personalForm.regid) {
          SaveHeadImage({
            ...
            fileid: this.personalForm.fileid,
          }).then((result) => {
            this.$message.success(result.message || result.msg);
          });
        }
      });
    },
    ...
    handlePreview(domId) {
      ...
      switch (domId) {
        ...
        case "btnLabel": // 标签
          ...
          break;
        case "btnPhysique": // 健康证
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "健康证打印") {
              let data = {
                fingercode: localStorage.getItem("md5code"),
                codes: [this.choosePeople.regid],
                repname: d.$children[0].$attrs["data-remark"],
    ...
    handlePrintGuideSingle() {
      ...
      if (d.$el.innerText == "指引单打印") {
        ...
        } else if (this.choosePeople.meccode == 10005) {
          if (this.choosePeople.personalflag == 1) {
            repname = `${indexP}第一健康团体指引单`;
          } else {
            repname = `${indexP}第一健康指引单`;
          }
        } else if (this.choosePeople.meccode == 10006) {
          if (this.choosePeople.personalflag == 1) {
            repname = `${indexP}广州第一健康团体指引单`;
          } else {
            repname = `${indexP}广州第一健康指引单`;
          }
        } else if (this.choosePeople.meccode == 10007) {
          if (this.choosePeople.personalflag == 1) {
