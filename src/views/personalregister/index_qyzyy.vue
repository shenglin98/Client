<template>
  <div class="personal_register">
    <div class="pageReg" v-if="!showQuestionnaire && !showHealthView">
      ...
      <div class="filter-container">
        <div v-for="(btn, index) of buttons" :key="index">
          <el-popover ref="popoverRefIdc" v-if="domIdArr.includes(btn.domId) && btn.domId != 'btnApply'" class="print_box" placement="bottom" trigger="click">
            ...
            <div class="print_item" v-if="btn.domId != 'btnPhysique'" @click="hadnleOnBtnClicked(btn.domId)">
              打印
            </div>
            <div class="print_item" v-if="btn.domId != 'btnPhysique'" @click="handlePreview(btn.domId)">
              预览
            </div>
            <div class="print_item" v-if="btn.domId == 'btnPhysique'" @click="handlePrintGuideSingleAdult('体格检查表')">
              体格检查表
            </div>
            <div class="print_item" v-if="btn.domId == 'btnPhysique'" @click="handlePrintGuideSingleAdult('儿童体格检查卡')">
              儿童体格检查卡
            </div>
            <div class="print_item" v-if="btn.domId == 'btnPhysique'" @click="handlePrintGuideSingleAdult('幼儿园入学体检表')">
              幼儿园入学体检表
            </div>
          </el-popover>
              ...
              <div class="basicInfo">
                <div class="message">
                  ...
                  <div class="card_operations" v-if="message == 'basic'">
                    <el-select :disabled="formFlag" v-model="healthcardcode" placeholder="请选择" size="mini" @change="handleSelectHealthcardCheck">
                    ...
                    <input type="text" class="read_card" v-model.lazy="healthCard" ref="read_idcard" v-if="healthcardcode == 'idcard'" @keyup.enter="handleEnterIdcard" :disabled="formFlag" />
                    <input type="text" class="read_card" v-model.lazy="healthCard" ref="read_healthcard" v-else :disabled="formFlag" @keyup.enter="handleCardReading" />
                    <el-button type="primary" size="mini" plain @click="handleCardReading" :disabled="formFlag" style="margin-left: 10px">读卡</el-button>
                    <el-button type="primary" size="mini" plain @click="handleCardSaveHeadImage" :disabled="formFlag" style="margin-left: 10px">保存头像</el-button>
                  </div>
                        ...
                        <el-row>
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="姓名" :rules="[{ required: !personalForm.name }]">
                              ...
                              ></el-input> -->
                              <input type="text" class="read_card" :disabled="formFlag" v-model="personalForm.name" @keyup.enter="handleGetRegisterInfo" />
                            </el-form-item>
                          </el-col>
                          <el-col :span="6">
                            <el-form-item size="mini" label="性别" :rules="[{ required: !personalForm.sex }]">
                              <el-select v-model="personalForm.sex" class="fromselect" placeholder="">
                        ...
                        <el-row>
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="婚姻">
                              <el-select @change="handleSelectChange" v-model="personalForm.marriage" class="fromselect" placeholder="">
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="证件类型">
                              <el-select @change="handleSelectChange" v-model="personalForm.idtype" class="fromselect" placeholder="">
                                <el-option v-for="item in idtypeItems" :key="item.code" :label="item.name" :value="item.code"></el-option>
                              </el-select>
                            </el-form-item>
                          </el-col>
                          <el-col :span="6">
                            <el-form-item size="mini" label="身份证" prop="idcard">
                              <!-- <el-input
                                v-model="personalForm.idcard"
                              ></el-input> -->
                              <input type="text" class="read_card" :disabled="formFlag" v-model.lazy="personalForm.idcard" @keyup.enter="handleChangeIdcard" />
                            </el-form-item>
                          </el-col>
                        </el-row>
                        <el-row>
                          <el-col :span="6">
                            <el-form-item size="mini" label="体检类型">
                              <el-select @change="handleSelectChange" v-model="personalForm.checktype" class="fromselect" placeholder="">
                                <el-option v-for="item in checktypeitems" :key="item.code" :label="item.name" :value="item.code"></el-option>
                              </el-select>
                            </el-form-item>
                          </el-col>
                          <el-col :span="6">
                            <el-form-item size="mini" label="电话" :rules="[{ required: !personalForm.telephone }]">
                        ...
                        <el-row>
                          ...
                          <el-col :span="6">
                            <el-form-item size="mini" label="标准金额">
                              <el-input v-if="pageState == 'add'" v-model="checkGroup.value"></el-input>
                              <el-input v-else v-model="personalForm.quota"></el-input>
                            </el-form-item>
                          </el-col>
                          <el-col :span="6">
                    ...
                    <el-row>
                      <el-col :span="6">
                        <el-form-item size="mini" label="危害因素">
                          <el-select v-model="personalForm.harmcode" @change="handleChangeHarmcode">
                      ...
                      <el-col :span="6">
                        <el-form-item size="mini" label="工种">
                          <el-select v-model="personalForm.worktypecode" @change="handleChangeWork">
                    ...
                    <el-row>
                      <el-col :span="6">
                        <el-form-item size="mini" label="行业类型">
                          <el-select v-model="personalForm.industrytypecode" @change="handleChangeIndustry">
                      ...
                      <el-col :span="6">
                        <el-form-item size="mini" label="入职日期">
                          <el-date-picker style="width: 100%" v-model="entryDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期">
                          </el-date-picker>
                        </el-form-item>
                      </el-col>
                    </el-row>
                          ...
                          <el-row>
                            <el-col :span="10">
                              <el-form-item class="tuanjian" size="mini" label="团检名称">
                                <!-- 名称变红 :class="personalForm.coststatus == 1 ||personalForm.balancecode? 'selectred': '' " -->
                            ...
                            <el-col :span="7">
                              <el-form-item size="mini" label="团检批次">
                                <el-select @change="handleSelectBatch" style="width: 100%" v-model="personalForm.batch" placeholder="" :class="
                                    personalForm.coststatus == 1
                                      ? 'selectred'
                                      : ''
                                  ">
                            ...
                            <el-col :span="7">
                              <el-form-item size="mini" label="团检分组">
                                <el-select @change="handleCheckGroup" style="width: 100%" v-model="personalForm.groupcode" placeholder="" :class="
                                    personalForm.coststatus == 1
                                      ? 'selectred'
                                      : ''
                                  ">
                  ...
                  <div class="checkup_body">
                    <div class="state_group">
                      ...
                      <div class="state_item">
                        ...
                        <div class="state2">
                          {{
                            personalForm.price || groupPriceTotalCom.toFixed(2)
                          }}
                        </div>
                      </div>
                      <div class="state_item">
                      ...
                      <div class="state_item" v-if="
                          personalForm.personalflag &&
                          personalForm.personalflag == 1
                        ">
                        <div class="title4">单位缴费</div>
                        <div class="state4">
                          {{ personalForm.companyActPrice || 0 }}
                        </div>
                      </div>
                      <div class="state_item" v-else>
                        <div class="title4">实际缴费</div>
                        <div class="state4">
                          {{
                            personalForm.actPrice || groupPriceEnd.toFixed(2)
                          }}
                        </div>
                      </div>
                      <div class="state_item" v-if="personalForm && personalForm.paytotalamount > 0">
                        <div class="title3">个人缴费</div>
                        <div class="state3">
                          {{ personalForm.paytotalamount }}
                        </div>
                      </div>
                    </div>
                    <div class="btn_state_gro">
                      <div class="state-add">
                        <!-- :disabled="formFlag" -->
                        <el-link type="primary" style="margin-right: 10px" :disabled="formFlag" @click="handleTodayCombinedBilling">今日组合开单</el-link>
                        <el-button type="primary" size="mini" :disabled="formFlag" @click="handleSelectSetMeal">开单套餐</el-button>
                        <el-button type="primary" size="mini" :disabled="formFlag" @click="handleSelectCmbine">开单组合</el-button>
                        <!-- <el-button
                        type="primary"
                        size="mini"
                        :disabled="formFlag"
                      ...
                      <el-button class="discount" size="mini" type="primary" :disabled="formFlag" @click="handleSetDiscount">折扣</el-button>
                ...
                <div class="checkup_btn-in">
                  <div class="checkup_btn">
                    ...
                    <!-- <el-button
                      ...
                      >复制登记</el-button
                    > -->
                    <el-button class="checkin" type="primary" size="mini" plain style="height: 28px; margin-right: 10px" @click="
                        (setmealRadio = ''), handleGetPersonalRegisterList()
                      ">刷新</el-button>
                    <el-button class="checkin" type="primary" size="mini" plain style="height: 28px; margin-right: 10px" @click="handleCopyCheckIn('project')">复制项目</el-button>
                    <el-button class="checkin" type="primary" size="mini" plain style="height: 28px; margin-right: 10px" @click="handleCopyCheckIn('message')">复制信息</el-button>
                    <el-button class="checkin" type="primary" size="mini" plain style="height: 28px; margin-right: 10px" @click="handleLottoAddProgetto">一键添加</el-button>
                    <!-- <el-switch v-model="isGuideSingle" active-text="打印指引单">
                    </el-switch> -->
                  </div>
                  <!-- 登记人员列表 -->
                  <div class="condition_table" @mouseleave="handlemouseLeaveReset">
                    <EasyTable row-id="regid" :loading="loadingCondition" class="condition_table_es" height="100%" ref="personnelTable" :data="conditionData" :tableOptions="conditionOptions" :radio-config="{
      ...
      <!-- 选择组合分组弹框 -->
      <CmbineOnly v-if="showCmbine" :sex="personalForm.sex" :flag="pageState" :groupTableData="groupTableData" @submit="handleProjectSubmit" @close="showCmbine = false" />
      ...
      <el-dialog class="dialog-mini charge_details" width="58%" top="4%" :close-on-click-modal="false" title="收费列表" :visible.sync="showChargeDetails" @close="handleCloseChargeDetails">
        <div>
          ...
          <el-button size="mini" style="margin-left: 20px" type="primary" @click="handleSetAllChargeCancel">取消收费</el-button>
        </div>
        <div class="table_box">
          <!-- row-id="combinecode" -->
          <EasyTable :loading="loadingGroup" class="table_item" height="100%" :data="chargeDataList" :tableOptions="chargeGroupOptions" :checkbox-config="{ rowClick: true, fetchVal: true }" @selected-change="handleSelectSharge" :border="false" :header-cell-style="handleReturnStyle" :row-class-name="handleTableRowClassName" :row-style="{ height: '32px' }" :cell-style="{ padding: '0' }" />
          <div class="charge_form">
            ...
            <div class="form_btn">
              ...
              <el-button size="mini" type="success" @click="handleSetChargeConfirm">收费</el-button>
              <!-- v-if="chargeItems.chargeflag == 1" -->
              <el-button size="mini" type="warning" @click="handleSetChargeCancel">退费</el-button>
      ...
      <!-- 门诊档案弹框 -->
      <el-dialog class="dialog-mini history_pson" width="55%" height="100%" top="10%" :close-on-click-modal="false" :show-close="false" title="门诊档案信息" :visible.sync="showOutpatient" @close="handleCloseOutpatient">
        <div class="table_box">
      ...
      <!-- 批量添加 -->
      <LottoAddProgetto v-if="showLottoAddProgetto" @submit="showLottoAddProgetto = false" />
      <!-- 打印申请单 -->
    ...
    <!-- 删除人员弹框 -->
    <el-dialog title="提示" :visible.sync="showDelPeople" top="15%" width="30%" class="delpeopledia" :show-close="false" :close-on-click-modal="false">
      <div class="content">
        <div class="ctitle">
          <i class="el-icon-warning"></i>
          <span v-if="choosePeople">此操作将永久删除该数据, 是否继续?　{{
              choosePeople.name || ""
            }}</span>
          <span v-else>此操作将永久删除该数据, 是否继续?</span>
        </div>
        <div class="ccform">
          <el-form :inline="true" :model="ccform" class="demo-form-inline" label-width="50px">
            <el-form-item label="账号">
              <el-input size="mini" v-model="ccform.username" placeholder="账号" clearable></el-input>
            </el-form-item>
            <el-form-item label="密码">
              <el-input size="mini" v-model="ccform.password" placeholder="密码" clearable show-password></el-input>
            </el-form-item>
          </el-form>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="handleDelPeopleClose">取 消</el-button>
        <el-button size="mini" type="primary" @click="hadnleDelList">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 调查问卷 -->
...
import LottoAddProgetto from "@/components/LottoAddProgetto/index.vue"; // 批量添加弹框
...
export default {
  ...
  data() {
    ...
    return {
      entryDate: new Date(+new Date() + 8 * 3600 * 1000)
        .toJSON()
        .substr(0, 19)
        .replace("T", " "),
      showDelPeople: false,
  ...
  computed: {
    ...mapGetters(["iframeViews", "visitedViews", "id", "account", "name"]),
    groupPriceTotalCom() {
      let price = 0;
      ...
      return [
            ...
            return (
              <div style="display: flex;">
                <input
                  type="text"
                  style="height:23px"
                  disabled={data.updateqty == 0}
                  class={`discount_qty_${data.$index} discount_input`}
                  onkeyup={(e) => {
                    if (!e) return;
                    switch (e.keyCode) {
                      case 13: // 键盘回车
                        this.handleAloneDiscountQty(data);
                        break;
                      case 38: // 键盘 ↑ 键盘
                        break;
                      case 40: // 键盘 ↓ 键盘
        ...
        },
        {
          ...
          returnVal: (value, data) => {
            let result = data.price * data.qty;
            ...
            return (
              <div style="display: flex;">
                <input
                  ...
                  style="height:23px"
                  class={[
                    `discount_${data.$index}`,
                    "discount_input",
                    data.balanceflag == 1 || data.discountflag == 1
                      ? "inputdisabled"
                      : "",
                  ]}
        ...
        },
        {
          ...
          returnVal: (value, data) => {
            if (data.regid) {
              ...
              let result = data.qty * data.price * data.discount;
              const factor = 10 ** 2;
              return (
                Math.round(Math.sign(result) * Math.abs(result) * factor) /
                factor
              );
              // return result.toFixed(2);
            }
            ...
            return (
              <div style="display: flex;">
                <input
                  type="text"
                  style="height:23px"
                  class={`discount_parts_${data.$index} discount_input`}
                  onkeyup={(e) => {
                    if (!e) return;
                    switch (e.keyCode) {
                      case 13: // 键盘回车
                        break;
                      case 38: // 键盘 ↑ 键盘
                        this.handleDiscountUpperParts(e, data);
                        break;
                      case 40: // 键盘 ↓ 键盘
                        this.handleDiscountLowerParts(e, data);
                        break;
                      default:
                        break;
                    }
  ...
  methods: {
    handleWriteLog(row) {
      if (!row.regid || row.status > 3) return;
      this.WriteLogList.push(row);
      if (this.WriteLogList.length == 2) {
        let WriteLog = `（${this.WriteLogList[0].regid}）${this.WriteLogList[0].name}  切换到  （${this.WriteLogList[1].regid}）${this.WriteLogList[1].name}`;
        WriteLog4({ c: WriteLog }).then((res) => {
          res.code == 200 && this.WriteLogList.shift();
        });
      }
    },
    ...
    /* 一键添加 */
    handleLottoAddProgetto() {
      this.showLottoAddProgetto = true;
    },
    ...
    handleChangeIndustry(value) {
      let flag = this.industryitems.filter((k) => {
        return k.code == value;
      })[0];
      if (flag) {
        this.personalForm.industrytypename = flag.name;
      } else {
        this.personalForm.industrytypename = "";
      }
    },
    handleChangeWork(value) {
      let flag = this.workitems.filter((k) => {
        return k.code == value;
      })[0];
      if (flag) {
        this.personalForm.worktypename = flag.name;
      } else {
        this.personalForm.worktypename = "";
      }
    },
    handleChangeHarmcode(value) {
      let flag = this.harmitems.filter((k) => {
        return k.code == value;
      })[0];
      if (flag) {
        this.personalForm.harmname = flag.name;
      } else {
        this.personalForm.harmname = "";
      }
    },
    /* 获取打印配置 */
    ...
    handleGetDicSetmealList() {
      getDicSetmealList({ tp: "", sex: 0, gp: "1" }).then((res) => {
        ...
        chooseSetmeal.combineitems = chooseSetmeal.combineitems.map((item) => {
          return {
            detid: "",
            regid: "",
            setmealcode: chooseSetmeal.setmealcode,
            setmealname: chooseSetmeal.setmealname,
        ...
        {
          /* chooseSetmeal.combineitems = chooseSetmeal.combineitems.map((k) => {
          return {
            ...k,
            setmealcode: chooseSetmeal.setmealcode || "",
            setmealname: chooseSetmeal.setmealname || "",
        ...
        this.groupTableData = chooseSetmeal.combineitems;
        this.personalForm.combineitems = chooseSetmeal.combineitems; // 赋值对象中
        this.groupPriceTotal = 0;
        localStorage.setItem(
          "groupTableData",
          JSON.stringify(chooseSetmeal.combineitems)
        );
        // 体检项目总金额
    ...
    handleEnterIdcardPf() {
      ...
      if (!this.personalForm.idcard) return;
      let regs =
        /(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[0-9Xx]$)/;
      // 身份证合法性
      if (!regs.test(this.personalForm.idcard)) {
        this.$message.warning("身份证号码不正确!");
        return false;
      }
      let iden = this.personalForm.idcard;
      let val = iden.length;
      let myDate = new Date();
      ...
      this.$set(this.personalForm, "age", age);
    },
    // 获取光标到检索框
    ...
    hadnleDelCombination(item) {
      ...
      let text =
        val == 0
          ...
          : "已退费";
      this.$confirm(
        `${item.combinename}【<span style="color:red">未收费</span>】此操作将删除当前项目, 是否继续?`,
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
        ...
        .then(() => {
          if (this.personalForm.regid) {
            ...
            if (index > -1) {
              ...
              this.personalForm.price = this.groupPriceTotal.toFixed(2);
    ...
    handleCopyCheckIn(flag) {
      ...
      this.isCopyCheckIn = true;
      this.avatarImg = require("./images/lin.jpg");
      // this.handleGetDetail(); // 获取下拉数据
      // this.handlePersonalFormReset(); // 重置表单对象
      if (flag === "project") {
        CopyCombine({ regid: this.personalForm.regid }).then((res) => {
          this.handlePersonalFormReset(); // 重置表单对象
          if (!res.result) return;
          this.groupTableData = res.result.combineitems;
          this.personalForm.actPrice = res.result.actPrice;
          this.personalForm.companyActPrice = res.result.companyActPrice;
          this.personalForm.discountprice = res.result.discountprice;
          this.personalForm.paytotalamount = res.result.paytotalamount;
          this.personalForm.price = res.result.price;
        });
        ...
        //     qty: k.qty,
        //     settlementflag: k.settlementflag,
        //   };
        // });
        ...
        this.personalForm.paytotalamount = 0;
    ...
    // 单项打折键盘↓键回调
    handleDiscountLowerParts(e, data) {
      this.$nextTick(() => {
        if (data.$index === this.groupTableData.length - 1) return;
        let pDom = document.querySelector(`.discount_parts_${data.$index + 1}`);
        if (!pDom) return;
        pDom.focus();
      });
    },
    // 单项打折键盘↑键回调
    handleDiscountUpperParts(e, data) {
      this.$nextTick(() => {
    ...
    handleSetChargeCancel() {
      ...
      let map = new Map();
      for (let item of this.selectSharge) {
        map.set(item.combinecode, item);
      }
      this.selectSharge = [...map.values()];
      this.$confirm(
        `退费后自动删除【${this.selectSharge
          .map((k) => k.combinename)
          .join("，")}】组合`,
        "系统提示",
        {
        ...
        .then(() => {
          setChargeCancel({
            regid: this.personalForm.regid,
            combinecodeitems: this.selectSharge.map((k) => k.combinecode),
          })
            .then((res) => {
              this.$message.success(res.message);
              this.handleCloseChargeDetails();
    ...
    handleHistoryCheck(row) {
      ...
      let company = company && data.companyitems.filter((k) => k.selected)[0];
    ...
    // 点击保存提交回调
    hadnleSubmit() {
      if (!this.personalForm.companycode && !this.personalForm.idcard) {
      ...
      if (!this.personalForm.companycode && this.personalForm.idtype == "01") {
      ...
      let flag = this.groupTableData.filter((k) => k.discount == "")[0] || "";
      if (flag) {
        this.$message.warning("项目折扣不允许为空!");
        return;
      }
      // 图片文件流处理
      if (
    ...
    handleProjectSubmit(obj) {
      ...
      if (this.pageState === "edit") {
        ...
        if (dialogFlag === "combine") {
          if (!this.personalForm.companycode) {
    ...
    handleGetPersonalDetail(regid) {
      ...
      getPersonalDetail({
        ...
        //   this.toUpdateDiscId = "";
        // }
        if (this.isGuideSingle && this.personalForm.regid) {
    ...
    // 新增获取个人登记明细/详情/新增回调
    handleGetDetail() {
      getPersonalDetail().then(async (res) => {
        let {
          ageunititems,
          checktypeitems,
          companyitems,
      ...
      getBatchOrGroup({
        ...
        .then(async (res) => {
          ...
          if (batchitems.length == 0) {
            ...
            await this.handleCheckGroup(this.personalForm.groupcode);
          }
        })
        .catch((err) => {});
    },
    ...
    // 检索条件表信息选中行回调
    async handleCheckCondition(row) {
      if (!row) return false;
      if (
        this.pageState == "add" &&
        ...
        this.$confirm(`登记信息未保存,是否切换?`, "提示", {
          ...
          .then(() => {
            WriteLog4({
              c: `新增人员${this.personalForm.name}-${this.personalForm.idcard}切换到编辑状态,${row.regid}-${row.name}`,
            }).then((res) => {});
    ...
    // 点击读卡保存头像
    async handleCardSaveHeadImage() {
      this.handleResetCondition();
      if (this.cardReaderFlag == "华视") {
        this.handleReadCardHuaShiSaveHeadImage(); // 华视读卡
      } else if (this.cardReaderFlag == "精伦") {
        this.handleReadCardJingLunSaveHeadImage(); // 精伦读卡
      } else if (this.cardReaderFlag == "新中新") {
        this.handleReadCardXinZhongXinSaveHeadImage(); // 精伦读卡
      }
    },
    // 华视读卡
    handleReadCardHuaShiSaveHeadImage() {
      this.handleConnectIdcard();
      ...
      this.$axios
        ...
        .then((res) => {
          if (res.data.resultFlag == 0) {
            let data = res.data;
            let idcard = data.certNumber;
            let filebase64 = data.identityPic;
            SaveHeadImage({
              idcard,
              filebase64,
            }).then((res) => {
    ...
    // 精伦读卡
    handleReadCardJingLunSaveHeadImage() {
    ...
    // 新中新读卡
    handleReadCardXinZhongXinSaveHeadImage() {
      ...
      this.$axios
        ...
        .then((res) => {
          if (res.data.code != -1) {
            ...
            let filebase64 = data.photobase64;
            SaveHeadImage({
              idcard,
              filebase64,
            }).then((res) => {
    ...
    async handleCardReading() {
      this.handleResetCondition();
      // this.isCopyCheckIn = true;
      // this.handlePersonalFormReset(); // 重置表单信息
      if (this.healthcardcode == "idcard") {
        /* let tempArr = this.cardReaderFlag.split("|");
        for (let i = 0; i < tempArr.length; i++) {
          if (tempArr[i] == "华视") {
            await this.handleReadCardHuaShi(); // 华视读卡
            if (this.isReadCardIn == 0) break;
          } else if (tempArr[i] == "精伦") {
            await this.handleReadCardJingLun(); // 精伦读卡
            if (this.isReadCardIn == 0) break;
          } else if (tempArr[i] == "新中新") {
            await this.handleReadCardXinZhongXin(); // 精伦读卡
            if (this.isReadCardIn != -1) break;
          } 
        }*/
        if (this.cardReaderFlag == "华视") {
    ...
    hadnleShowDelPeople() {
      ...
      this.showDelPeople = true;
    },
    // 点击删除个人列表数据
    hadnleDelList() {
      if (
        this.ccform.username.trim() == "" ||
        this.ccform.password.trim() == ""
      ) {
        this.$message.warning("账号或密码不允许为空!");
        return;
      }
      checkUserAcount({
        account: this.ccform.username.trim(),
        password: this.ccform.password.trim(),
      }).then((res) => {
        if (!res.result) return;
        deleteRegister({
          code: this.setmealRadio,
          operatecode: this.ccform.username.trim(),
        }).then((res) => {
          this.pageState = "add"; // 当前编辑状态
          this.formFlag = false; // 解除表单限制可编辑
          this.groupdis = false; // 解除团检表单限制可编辑
        ...
        case "btnGuideSingle": // 指引单
          this.$refs.popoverRefIdc.forEach((d) => {
            if (d.$children[0].$el.innerText == "指引单打印") {
              let repname = "";
              if (this.personalForm.checktype == 12) {
                repname = `${
    ...
    handlePrintGuideSingle() {
      ...
      if (d.$el.innerText == "指引单打印") {
        let repname = "";
        if (this.personalForm.checktype == 12) {
          repname = `${d.$attrs["data-remark"].split("、")[0]}、专项指引单`;
        } else {
          repname = d.$attrs["data-remark"];
        }
        let data = {
        ...
        printBase64(data).then((res) => {
          this.handlePrintApplyAuto(); // 打印申请单
          let flag = this.groupTableData.filter((k) => {
            return this.isCensorShip.split(",").includes(k.combinecode);
          });
          flag.length > 0 && this.handlePrintCensorShip(); // 打印送检单
          if (
            ...
            this.handlePrintGuideKindergarten();
    ...
    // 打印职业指引单回调
    handlePrintGuideSingleOccupation() {
      if (!this.choosePeople) {
        this.$message({
          showClose: true,
          message: "请选择人员再进行打印职业指引单",
          type: "error",
        });
        return false;
      }
      let d = this.$refs.btnGuideSingleOccupation[0];
      if (d.$el.innerText == "职业指引单") {
        let data = {
          fingercode: localStorage.getItem("md5code"),
          codes: [this.choosePeople.regid],
          repname: d.$attrs["data-remark"],
    ...
    // 打印指引单回调-同时打印体格表
    handlePrintGuideSingleAdult(name) {
      if (!this.choosePeople) {
        this.$message({
          showClose: true,
    ...
    hadnleOnBtnClicked(domId) {
      ...
      switch (domId) {
        ...
        case "btnQuestionnaire": // 调查问卷
          ...
          break;
        case "btnSubmission": // 打印送检单
          this.handlePrintCensorShipBtn();
          break;
        ...
        case "btnLabel": // 标签
          ...
          break;
        case "btnGuideSingleOccupation": // 职业指引单打印
          this.handlePrintGuideSingleOccupation();
          break;
