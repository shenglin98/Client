                          ...
                          <el-col :span="6">
                            ...
                            >
                              <el-select
                                filterable
                                clearable
                                @change="handleSelectSetmealCode"
                                v-model="setmealCode"
                              >
                                <el-option
                                  v-for="item in setmealItemsS"
                    ...
                    <div class="message-title">
                      ...
                      <div class="message_content" style="width: 100%">
                        <div class="select_box">
                          <el-checkbox
                            v-model="lockingChecktype"
                            @change="handleChangelockingChecktype"
                            >类型</el-checkbox
                          >
                          <el-checkbox
                            v-model="lockingSetmealCode"
                            @change="handleChangelockingSetmealCode"
                            >套餐</el-checkbox
                          >
                          <el-checkbox
                            v-model="lockingInputcompanyname"
                            @change="handleChangelockingInputcompanyname"
                            >单位</el-checkbox
                          >
                          <el-checkbox
                            v-model="lockingDepartment"
                            @change="handleChangelockingDepartment"
                            >部门</el-checkbox
                          >
                        </div>
                        <el-form
...
export default {
  ...
  data() {
    ...
    return {
      ...
      setmealCode: "",
      setmealItemsS: [],
      lockingChecktype: false,
      lockingSetmealCode: false,
      lockingInputcompanyname: false,
      lockingDepartment: false,
      test: false,
    };
  },
  computed: {
  ...
  mounted() {
    ...
    getDicSetmealList({ tp: "", sex: this.sex, gp: "1" }).then((res) => {
      if (!res.result) return;
      this.setmealItemsS = res.result.setmealitems;
    });
  ...
  methods: {
    ...
    // 勾选类型保留回调
    handleChangelockingChecktype(val) {
      if (!val) {
        localStorage.removeItem("lockingChecktype");
      }
    },
    handleChangelockingSetmealCode(val) {
      if (!val) {
        localStorage.removeItem("lockingSetmealCode");
      }
    },
    handleChangelockingInputcompanyname(val) {
      if (!val) {
        localStorage.removeItem("lockingInputcompanyname");
      }
    },
    handleChangelockingDepartment(val) {
      if (!val) {
        localStorage.removeItem("lockingDepartment");
      }
    },
    handleSelectSetmealCode(value) {
      if (this.setmealCode) {
        let flag = this.setmealItemsS.filter((k) => {
          return k.setmealcode === this.setmealCode;
        })[0];
        if (!flag) return;
        /* let data = flag.combineitems.map((item) => {
          return {
            detid: "",
            regid: "",
            setmealcode: flag.setmealcode,
            setmealname: flag.setmealname,
        ...
        this.groupTableData = [];
        this.personalForm.combineitems = []; // 赋值对象中
        localStorage.setItem("groupTableData", JSON.stringify([]));
        this.groupPriceTotal = 0;
    ...
    hadnleSubmit() {
      ...
      setSubmit(this.personalForm)
        .then((res) => {
          ...
          // 体检类型
          if (this.lockingChecktype) {
            localStorage.setItem(
              "lockingChecktype",
              JSON.stringify(this.personalForm.checktype)
            );
          } else {
            localStorage.removeItem("lockingChecktype");
          }
          // 套餐
          if (this.lockingSetmealCode) {
            let tcobj = {
              setmealCode: this.setmealCode,
              combineitems: this.groupTableData,
            };
            localStorage.setItem("lockingSetmealCode", JSON.stringify(tcobj));
          } else {
            localStorage.removeItem("lockingSetmealCode");
          }
          // 单位
          if (this.lockingInputcompanyname) {
            let dwobj = {
              companycode: this.personalForm.companycode,
              companyname: this.personalForm.companyname,
              inputcompanyname: this.personalForm.inputcompanyname,
            };
            localStorage.setItem(
              "lockingInputcompanyname",
              JSON.stringify(dwobj)
            );
          } else {
            localStorage.removeItem("lockingInputcompanyname");
          }
          // 部门
          if (this.lockingDepartment) {
            localStorage.setItem(
              "lockingDepartment",
              JSON.stringify(this.personalForm.department)
            );
          } else {
            localStorage.removeItem("lockingDepartment");
          }
    ...
    handleSelectBatch(val) {
      ...
      if (data) {
        ...
        if (this.groupitems.length == 1) {
          this.personalForm.groupcode = this.groupitems[0].code;
        }
      }
    },
    // 点击新增按钮回调
    handleCreate() {
      ...
      this.handlePersonalFormReset(); // 重置表单对象
      this.handleGetDetail(); // 获取下拉数据
      if (JSON.parse(localStorage.getItem("lockingChecktype"))) {
        setTimeout(() => {
          let data = JSON.parse(localStorage.getItem("lockingChecktype"));
          if (!data) return;
          this.personalForm.checktype = data;
        }, 700);
      }
      if (JSON.parse(localStorage.getItem("lockingSetmealCode"))) {
        setTimeout(() => {
          let data = JSON.parse(localStorage.getItem("lockingSetmealCode"));
          if (!data) return;
          this.setmealCode = data.setmealCode;
          this.personalForm.combineitems = data.combineitems;
          this.groupTableData = data.combineitems;
        }, 700);
      }
      if (JSON.parse(localStorage.getItem("lockingInputcompanyname"))) {
        setTimeout(() => {
          let data = JSON.parse(
            localStorage.getItem("lockingInputcompanyname")
          );
          if (!data) return;
          this.personalForm.companycode = data.companycode;
          this.personalForm.companyname = data.companyname;
          this.personalForm.inputcompanyname = data.inputcompanyname;
          this.handleSelectSetmealCode();
          this.hadnleGetBatchOrGroup();
        }, 700);
      }
      if (JSON.parse(localStorage.getItem("lockingDepartment"))) {
        setTimeout(() => {
          let data = JSON.parse(localStorage.getItem("lockingDepartment"));
          if (!data) return;
          this.personalForm.department = data;
        }, 700);
      }
    },
    // 获取个人登记明细/详情/新增回调
    ...
    handleHealthCard() {
      ...
      getPatientInfo({
        ...
        .then((res) => {
          if (res.result.length == 0) return;
          if (res.result.length == 1) {
            if (this.readCardInfo) {
              if (
                this.readCardInfo.name != res.result[0].name ||
                this.readCardInfo.sex != res.result[0].sex
              ) {
                this.$message.error(
                  "当前读卡信息与建档信息不符合!请前往门诊处理!"
                );
                this.handlePersonalFormReset(); // 重置表单对象
                return;
              }
    ...
    handlePrintGuideSingle() {
      ...
      if (d.$el.innerText == "指引单打印") {
        let index = d.$attrs["data-remark"].split("、")[0];
        let repname = "";
        if (this.personalForm.checktype == 2) {
          repname = `${index}、公共场所指引单`;
        } else if (this.personalForm.checktype == 3) {
          repname = `${index}、食品从业指引单`;
        } else if (this.personalForm.checktype == 5) {
          repname = `${index}、普通员工指引单`;
        } else if (this.personalForm.checktype == 6) {
          repname = `${index}、自选项目指引单`;
        } else if (this.personalForm.checktype == 7) {
          repname = `${index}、驾驶证指引单`;
        } else if (this.personalForm.checktype == 8) {
          repname = `${index}、特种设备指引单`;
