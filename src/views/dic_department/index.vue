  ...
  <div>
    <sticky :className="'sub-navbar'">
      <div class="filter-container">
        ...
        <permission-btn moduleName="dic_department" size="mini" v-on:btn-event="onBtnClicked"></permission-btn>
      </div>
    </sticky>
    <div class="app-container ">
      <div class="bg-white">
        <el-table ref="mainTable" :key="tableKey" :header-cell-style="tabHeader" :data="list.data" v-loading="listLoading" border fit highlight-current-row style="width: 100%;" @row-click="rowClick" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" prop="name" label="科室名称" show-overflow-tooltip>
          ...
          <el-table-column align="center" prop="hiscode" label="公用编码" show-overflow-tooltip></el-table-column>
          <el-table-column align="center" prop="typecode" label="科室类型" show-overflow-tooltip></el-table-column>
          <el-table-column align="center" prop="interfacecode" label="数据接口" show-overflow-tooltip></el-table-column>
        ...
        <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="130px">
          ...
          <el-form-item size="small" :label="'科室类型'" prop="typecode">
            <el-select v-model="temp.typecode">
              <el-option v-for="(item, index) in list.dic_department_type" :key="index" :value="item.id" :label="item.name"></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-dialog v-el-drag-dialog class="dialog-mini" width="600px" :title="textMap[dialogStatus]" :visible.sync="adddialogFormVisible">
        <el-form :rules="rules" ref="dataForm" :model="addtemp" label-position="right" label-width="130px">
          ...
          <el-form-item size="small" :label="'科室类型'" prop="typecode">
            <el-select v-model="addtemp.typecode">
              <el-option v-for="(item, index) in list.dic_department_type" :key="index" :value="item.id" :label="item.name"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item size="small" :label="'数据接口'" prop="interfacecode">
            <el-input v-model="addtemp.interfacecode"></el-input>
          </el-form-item>
          <el-form-item size="small" :label="'说明'" prop="remark">
...
<script>
import * as dic_department from "@/api/dic_department";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      dic_department.getList(this.listQuery).then((response) => {
        this.list = response.data;
        console.log(this.list);
        this.list.data.forEach((d) => {
          this.list.dic_department_type.forEach((b) => {
            if (d.typecode == b.id) {
              d.typecode = b.name;
            }
          });
    ...
    createData() {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          dic_department.add(this.addtemp).then(() => {
    ...
    handleUpdate(row) {
      ...
      this.list.dic_department_type.forEach((d) => {
        if (d.name == this.temp.typecode) {
          this.temp.typecode = d.id;
        }
      });
    ...
    updateData() {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          dic_department.update(tempData).then(() => {
    ...
    handleDelete(rows) {
      // 多行删除
      this.delrows(dic_department, rows);
    },
  },
};
</script>
